...
if (count($_POST) == 0) {
    ...
    foreach ($_SESSION['lang_param'] as $k => $v) {
        if (isset($_POST['lang|'.(string) $k]) && $_POST['lang|'.(string) $k] != $v[0]) {
            $_SESSION['lang_param'][$k][0] = $_POST['lang|'.(string) $k];
        }
    }
}
foreach ($_POST as $str_key => $str_v) {
    ...
    $sw_break = true;
    switch ($s_k) {
        case 'idle_button': TestLangList($dbh);
            break;
        case 'user_lang_s': if (AfterLangChoice($dbh, 'user_lang_s', 'user_lang', $sw_break)) {
            TestLangList($dbh);
        } break;
        case 'language_OK': if (isset($_SESSION['pre_lang_err'])) {
            unset($_SESSION['pre_lang_err']);
        } TestLangList($dbh);
            RewriteLanguages($dbh, $Mes);
            $_SESSION['lang_del'] = true;
            break;
        case 'lang_exit': $goto = LanguageExit($dbh, $Mes);
            if ($goto != '') {
                header('Location: '.$goto.'.php');
            } break;
        case 'lang_restore': if (isset($_SESSION['pre_lang_err'])) {
            unset($_SESSION['pre_lang_err']);
        } $_SESSION['lang_param'][$k[1]][0] = $_SESSION['lang_param'][$k[1]][1];
            TestLangList($dbh);
            RestoreUserLanguageList();
            break;
        case 'lang_del': LangDel($dbh);
            $_SESSION['del_langs'] = [];
            $_SESSION['lang_del'] = false;
            TestLangList($dbh);
            $Mes[] = '<b>'.Title(182).'</b>';
            RestoreUserLanguageList();
            break;
        case 'lang_no_del': $_SESSION['lang_del'] = false;
            DelLangListRestore();
            TestLangList($dbh);
            break;
    ...
    <table>
	    <tr>
		    ...
		    <?php if ($_SESSION['user_working_mode'] == 1) {
		        echo '<td><button'.$dis_del." name='language_OK' type='submit' title='".Title(269)."' class='save_button'>".Title(30).'</button></td>';
		    } ?>
	    </tr>
    </table>
    <?php if ($fl_del) {
        DBMDelQuestion('modal_form', ['lang_del', 'lang_no_del'], LangSetQuestionString(), 208, 625, 622);
    } ?>
