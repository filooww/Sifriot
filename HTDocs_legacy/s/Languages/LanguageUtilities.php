...
function SetLanguageList($list_type) // 0 - title, 1 - local, 2 - user
{
    ...
    foreach ($_SESSION['common_langs'] as $k => $v) {
        if ($list_type == 0 && $k != 0 || $list_type == 2 && $k != 0 || $list_type == 1 && $k != 1) {
            $langs[$k] = $v;
        }
    }
...
{
    $fl_change = ['special' => false, 'main' => false];
...
function UpdateSystemLanguage($sys_id, $sys_name, &$fl_change)
{
    if (! isset($_SESSION['common_langs'][$sys_id])) {
        ...
        if ($k !== false) {
            ChangeInvalidLanguage($k, $sys_name);
        }
...
function AddInvalidLanguage($k, $v, $sys_name)
{
    ...
    ksort($_SESSION['common_langs']);
    $_SESSION['pre_lang_err'][$new_id] = ['*-181', '<b>'.$v.'</b>', '*201', '*-147', '<b>'.(string) $k.'</b>, ', '*439', '*468', '*82', '*-147', '*624', '<b>'.$new_id.'</b>', '*82', '<b>'.$v.'~</b>', '*534'];
}
function AddSystemLanguage($sys_id, $sys_name)
{
    $_SESSION['common_langs'][$sys_id] = $sys_name;
    $_SESSION['pre_lang_err'][$sys_id] = ['*276'];
}
function ChangeInvalidLanguage($k, $sys_name)
{
    $_SESSION['common_langs'][$k] = $sys_name.'~';
    $_SESSION['pre_lang_err'][$k] = ['*-181', '<b>'.$sys_name.'</b>', '*469', '<b>'.$_SESSION['common_langs'][$k].'</b>'];
}
