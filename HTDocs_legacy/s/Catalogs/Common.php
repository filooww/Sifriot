...
function GetCatalogPortion($dbh, $n, $start_pos, $d_portion)
{
	...
	if ($n == "0" && $_SESSION['Catalog'][$n]['cat_type'] == 1) $cat_arr = GetTreeCatalogPortion($dbh, $start_pos, $d_portion);
	else $cat_arr =  GetNormalCatalogPortion($dbh, $single, $n, $start_pos, $d_portion);
	if ($_SESSION['user_working_mode'] == 1) AddNewRow($single, $cat_arr);
	if ($_SESSION['catalog_param'][$n]['search_on']) GetSearchPosOut($dbh, $n);
	return $cat_arr;
}
function AddNewRow($single, &$cat_arr)
{
	...
	{
		if ($single) $cat_arr['N'] = array(false, false, false, ""); // new??
		else $cat_arr['N'] = array(false, false, false, "", ""); // new??
	}
}
function GetRequest($dbh, $single, $n, $start_pos, $d_portion)
{
	if ($single)
	{
		...
		if ($_SESSION['catalog_param'][$n]['filter_text'] == "") $res = mysqli_query($dbh, "SELECT * FROM ".$_SESSION['Catalog'][$n]['table']." ORDER BY ".$str_common);
		else $res = mysqli_query($dbh, "SELECT * FROM ".$_SESSION['Catalog'][$n]['table']." WHERE ".$_SESSION['catalog_param'][$n]['filter_where']." ORDER BY ".str_common);
	}
	else 
	{
		$f_arr = array(MCV($_SESSION['catalog_param'][$n]['filter_text'], $_SESSION['match_case']), $_SESSION['catalog_param'][$n]['filter_compare']);
		$q_text = QueryTextForSet($dbh, $_SESSION['match_case'], 1, $f_arr, array(""), "f", "", "", (string)$start_pos.",".$d_portion);
		if ($q_text == "") $res = false;
...
function GetNormalCatalogPortion($dbh, $single, $n, $start_pos, $d_portion)
{
	...
	if ($res)
	{
		if (!$single) $max_lev = GetMaxSetLevel($dbh, $_SESSION['Catalog'][$n]['table']);
		while ($row = mysqli_fetch_row($res))
		{
			if ($single) $cat_arr[(string)$row[0]] = array(false, false, false, $row[1]);
			else $cat_arr[(string)$row[0]] = array(false, false, false, $row[1], GetQueryValueSet(2, $row, $max_lev));
		}
...
function GetOffsetInTable($dbh, $n, $value)
{
	...
	if ($single)
	{
		if ($_SESSION['catalog_param'][$n]['filter_where'] == "") $res = mysqli_query($dbh, "SELECT COUNT(*) FROM ".$_SESSION['Catalog'][$n]['table']." WHERE ".$vx." < '".$v."' ORDER BY ".$vx);
		else $res = mysqli_query($dbh, "SELECT COUNT(*) FROM ".$_SESSION['Catalog'][$n]['table']." WHERE ".$vx." < '".$v."' AND ".$_SESSION['catalog_param'][$n]['filter_where']." ORDER BY ".$vx);
		if ($res)
		{
	...
	{
		$q_text = QueryTextForSet($dbh, $_SESSION['match_case'], 1, array(MCV($_SESSION['catalog_param'][$n]['filter_text'], $_SESSION['match_case']), $_SESSION['catalog_param'][$n]['filter_compare']), array(""), "f", "<", $v, "", "COUNT(*)");
		if ($q_text != "")
		{
...
{
	$_SESSION['Catalog'] = array("0"=>array("name"=>"", "tbl"=>"", "id"=>"", "vl"=>"", "in_pub"=>"", "s_n"=>""), "1"=>array("name"=>"", "tbl"=>"", "id"=>"", "vl"=>""));
	InitCatalogParameters("0");
	InitCatalogParameters("1");
	$_SESSION['cat_arr'] = array("0"=>array(), "1"=>array());
	$_SESSION['cat_mark'] = array("b_on"=>false, "bc"=>"");
	$_SESSION['cat_vis'] = array("b_on"=>false, "bc"=>"");
	$_SESSION['copy_paste'] = array("copy_id"=>"", "parent_value"=>"", "parent_value_text"=>"", "copy_value"=>"", "copy_text_value"=>"");
}
...
function CatalogCall($dbh, $conf, $s_n, &$block, $settings_pad, $sel_value, &$cur_value)
{
	...
	if ($s_n != "") $_SESSION['Catalog'] = ReadCatalogFieldDefinitions($dbh, $s_n);
	$block['cat'] = true;
	$ca = array();
...
{
	if ($_SESSION['Catalog']['1']['name'] == "") return Title(399)." <b>".$_SESSION['Catalog']['0']['name']."</b> ".$common_mes;
	else return Title(375)." <b>".$_SESSION['Catalog']['0']['name']."</b> ".Title(82)." <b>".$_SESSION['Catalog']['1']['name']."</b> ".$common_mes;
}
function ExitMessage()
{
	if ($_SESSION['Catalog']['1']['name'] == "") return FTM(Title(399))." <b>".$_SESSION['Catalog']['0']['name']."</b>";
	else return FTM(Title(375))." <b>".$_SESSION['Catalog']['0']['name']."</b>, <b>".$_SESSION['Catalog']['1']['name']."</b>";
}
...
function GoToCatalogPosition($dbh, $pos_go_to, $n)
{
	...
	{
		$_SESSION['catalog_param'][$n]['start_pos'] = (integer)$pos_go_to;
