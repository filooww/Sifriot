...
{
	if (!$to_inf['b_on']) $_SESSION['mes']['0'][] = array("time"=>"", "text"=>Title(495), "status"=>"warning", "now"=>true);
	elseif (!array_key_exists($to_inf['bc'], $_SESSION['cat_arr'])) $_SESSION['mes']['0'][] = array("time"=>"", "text"=>Title(496), "status"=>"warning", "now"=>true);
	elseif ($_SESSION['cat_arr']['1'][$from_code][2]) $_SESSION['mes']['1'][] = array("time"=>"", "text"=>Title(497), "status"=>"error", "now"=>true);
	elseif ($_SESSION['cat_arr']['1'][$from_code][1]) $_SESSION['mes']['1'][] = array("time"=>"", "text"=>Title(498), "status"=>"error", "now"=>true);
	else
	{
		$ed_codes = ($_SESSION['cat_arr']['0'][$to_inf['bc']][3] == "") ? array() : explode(",", $_SESSION['cat_arr']['0'][$to_inf['bc']][3]);
		if (!in_array($from_code, $ed_codes))
		{
			$ed_values = (count($ed_codes) == 0) ? array() : explode($_SESSION['Catalog']['0']['separator'], $_SESSION['cat_arr']['0'][$to_inf['bc']][4]);
			$ed_codes[] = $from_code;
			$ed_values[] = $from_text;
			if ($_SESSION['Catalog']['0']['cat_type'] == 2) array_multisort($ed_values, $ed_codes, SORT_STRING);
...
function DelFromSetList($dbh, &$to_inf, $from_code)
{
	...
	if ($nn !== false)
	{
		...
		{
			$to_inf['b_on'] = false;
			if (is_numeric($to_inf['bc'])) $_SESSION['del_row']['0'][$to_inf['bc']] = array("old_id"=>$cat_arr['0'][$to_inf['bc']][3], "old_value"=>$_SESSION['cat_arr']['0'][$to_inf['bc']][4]);
		}
...
function DelHierarchic($dbh, $del_code)
{
	...
	{
		$_SESSION['del_row']['0'][$del_code] = array("old_id"=>$_SESSION['cat_arr']['0'][$del_code][3], "old_value"=>$_SESSION['cat_arr']['0'][$del_code][4]);
		$sons_inner = GetSonsArray($_SESSION['cat_arr']['0'], $_SESSION['cat_arr']['0'][$del_code][3]);
		$_SESSION['cat_arr']['0'][$del_code] = array(false, true, false, "", "");
		if (count($sons_inner) > 0)
		{
			foreach ($sons_inner as $k => $v)
			{
				$_SESSION['del_row']['0'][$k] = array("old_id"=>$v[0], "old_value"=>$v[1]);
				$_SESSION['cat_arr']['0'][$k] = array(false, true, false, "", "");
			}
		}
		$sons_outer = GetNodeSons($dbh, $sons_inner);
		foreach ($sons_inner as $k => $v) $_SESSION['del_row']['0'][$k] = array("old_id"=>$v[0], "old_value"=>$v[1]);
	}
	TestCatalogs($dbh);
}
...
function CatFill($dbh)
{
	if (!IsChangesAndErrors("1", Title(482)." ", "")) //[] = array("time"=>"", "text"=>"To fill coupled catalog You must save the changes on these pages", "status"=>"warning", "log"=>false);
	{
		...
		$_SESSION['mes']['c'][] = array("time"=>"", "text"=>"<b>".(string)$cnt."</b> ".Title(499)." <b>".$_SESSION['Catalog']['1']['name']."</b> ".Title(500)." <b>".$_SESSION['Catalog']['0']['name']."</b>", "status"=>"statement", "now"=>true);
	}
}
?>
