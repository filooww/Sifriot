...
function OnOffUseButton($k, &$PR, &$Mes)
{
    if ($main_params['const'][$k]['table'] != '') {
        if (! $main_params['var'][$k]['use'] && $main_params['var'][$k]['value'] == '') {
            ...
            $Mes[] = ['time' => '', 'text' => 'The value for <b>'.strtolower($main_params['const'][$k]['name']).'</b> not selected', 'status' => 'error'];
        } elseif (! $main_params['var'][$k]['use'] && $main_params['var'][$k]['value'] != '') {
            $main_params['var'][$k]['use'] = true;
        } elseif ($main_params['var'][$k]['use'] && $main_params['var'][$k]['value'] == '') {
        ...
        if ($main_params['var'][$k]['value'] == '') {
            $Mes[] = ['time' => '', 'text' => 'The value for <b>'.strtolower($main_params['const'][$k]['name']).'</b> not selected', 'status' => 'error'];
        } else {
            $main_params['var'][$k]['use'] = ! $main_params['var'][$k]['use'];
        }
    }
}
...
{
    $fl_select = false;
    if ($_SESSION['cat_arr']['0'][$f][1] || $_SESSION['cat_arr']['0'][$f][2]) {
        $Mes[] = ['time' => '', 'text' => 'To select this value from catalog You must '.(($_SESSION['cat_arr']['0'][$f][2]) ? 'correct' : 'save').' it', 'status' => 'error'];
    } else {
        $ff = ($_SESSION['cat_arr']['0'][$f][1]) ? NoUpdateSelectedValue($dbh, $f, $Mes) : false;
        if (! $ff) {
            if ($_SESSION['Catalog']['0']['cat_type'] == 1 && ! SingleNode($dbh, $_SESSION['cat_arr']['0'][$f][3])) {
                $Mes[] = ['time' => '', 'text' => 'This value is a group value and cannot be selected', 'status' => 'error'];
            } else {
                $l_selected['value'] = end($_SESSION['cat_arr']['0'][$f]);
                $l_selected['code'] = $f;
                if (isset($l_selected['use'])) {
                    $l_selected['use'] = true;
                }
