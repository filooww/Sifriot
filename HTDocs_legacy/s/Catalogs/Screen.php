...
function CaseMinus($dbh)
{
    ...
    if ($max_count < $_SESSION['portion']) {
        ...
        if ($k0 != '' && $k1 != '') {
            if ($k0 != '-') {
                ReduceScreenTable('0', $k0);
            }
            if ($k1 != '-') {
                ReduceScreenTable('1', $k1);
            }
            $_SESSION['portion']--;
        } else {
            $_SESSION['mes']['c'][] = ['time' => '', 'text' => IsChangesAndErrors('', Title(237).' ', ' '.Title(470)), 'status' => 'warning', 'now' => true];
        }
    }
}
function CasePost($dbh)
{
    if (! is_numeric($_POST['cat_height_input'])) {
        $_SESSION['mes']['c'][] = ['time' => '', 'text' => "<font color='#FF0000'>".Title(177).' '.Title(77).'</font>', 'status' => 'error', 'now' => true];
    } elseif ((int) $_POST['cat_height_input'] < 1) {
        $_SESSION['mes']['c'][] = ['time' => '', 'text' => "<font color='#FF0000'>".Title(178).'</font>', 'status' => 'error', 'now' => true];
    } elseif ((int) $_POST['cat_height_input'] != $_SESSION['portion']) {
        $fl = ((int) $_POST['cat_height_input'] < $_SESSION['portion']) ? ! IsChangesAndErrors('', Title(237).' ', '') : true;
        if ($fl) {
            $_SESSION['portion'] = (int) $_POST['cat_height_input'];
            $_SESSION['cat_arr']['0'] = GetCatalogPortion($dbh, '0', $_SESSION['catalog_param']['0']['start_pos'], $_SESSION['portion']);
...
{
    switch ($offs) {
        case 'plus': CasePlus($dbh);
            break;
        case 'minus': if ($_SESSION['portion'] > 1) {
            CaseMinus($dbh);
        } break;
        case 'button': if (isset($_POST['cat_height_input'])) {
            CasePost($dbh);
        } break;
    }
}
...
{
    if (count($_SESSION['cat_arr'][$n]) < $max_count) {
        return '-';
    }
    if (isset($_SESSION['cat_arr'][$n]['N']) && $_SESSION['cat_arr'][$n]['N'][3] != '') {
        return '';
    } // new??
    if (count($_SESSION['cat_arr'][$n]) == 1) {
        return '';
    }
    $arr = array_keys($_SESSION['cat_arr'][$n]);
    $nn = count($arr) - ((isset($_SESSION['cat_arr'][$n]['N'])) ? 2 : 1); // new??
    $k = $arr[$nn];
    if (! $_SESSION['cat_arr'][$n][$k][1] && ! $_SESSION['cat_arr'][$n][$k][2]) {
        return $k;
    } else {
        return '';
    }
...
function ReduceScreenTable($n, $arr_key)
{
    if ($_SESSION['catalog_param'][$n]['search_on']) {
        $_SESSION['catalog_param'][$n]['next_search_out'] = ScreenNextOutPos($_SESSION['cat_arr'][$n][$arr_key], $_SESSION['catalog_param'][$n]['search_text'], GetRadioType($_SESSION['catalog_param'][$n]['search_where']));
    }
    ...
    if (count($next_arr) > 0) {
        ...
        if ($next_row[3] != '') {
            if (isset($_SESSION['cat_arr'][$n]['N'])) { // new??
                $new_row = [];
                foreach ($_SESSION['cat_arr'][$n]['N'] as $z) {
                    $new_row[] = $z;
                } // new??
