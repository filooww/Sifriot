...
{
    if ($s_text == '') {
        $_SESSION['mes'][$n][] = ['time' => '', 'text' => Title(83).' '.Title(226), 'status' => 'error', 'now' => true];
    } else {
        ...
        if ($_SESSION['Catalog']['1']['name'] == '' && $n == '0' || $n == '1') {
            $_SESSION['catalog_param'][$n]['search_where'] = GetWhere(MCF($_SESSION['Catalog'][$n]['value'], $_SESSION['match_case']), MCV($s_text, $_SESSION['match_case']), $_SESSION['catalog_param'][$n]['search_compare']);
        } else {
            $_SESSION['catalog_param'][$n]['search_where'] = WhereSet($dbh, $_SESSION['match_case'], $s_text, $_SESSION['catalog_param'][$n]['search_compare'], 'f');
        }
        ...
        if ($_SESSION['catalog_param'][$n]['found_count'] > 0) {
            ...
            if ($first_value['id'] != '') {
                if ($n == '0' && $_SESSION['Catalog']['1']['table'] != '' && $_SESSION['Catalog'][$n]['cat_type'] == 1) {
                    SetAllCollapse($dbh, '-', $_SESSION['cat_arr']['0'], $settings_pad);
                }
                if (array_key_exists($first_value['id'], $_SESSION['cat_arr'][$n])) {
...
{
    $first_value = ['id' => '', 'value' => ''];
    $single = ($_SESSION['Catalog']['1']['table'] == '' || $_SESSION['Catalog']['1']['table'] != '' && $n == '1');
    if ($single) {
        if ($_SESSION['catalog_param'][$n]['filter_text'] != '') {
            $_SESSION['catalog_param'][$n]['search_where'] .= ' AND '.$_SESSION['catalog_param'][$n]['filter_where'];
        }
        ...
        if (mysqli_errno($dbh) > 0) {
            $_SESSION['mes'][$n][] = ['time' => '', 'text' => mysqli_error($dbh), 'status' => 'error', 'now' => true];
        }
    } else {
        $s_arr = [MCV($_SESSION['catalog_param'][$n]['search_text'], $_SESSION['match_case']), $_SESSION['catalog_param'][$n]['search_compare']];
    ...
    if ($res) {
        if ($row = mysqli_fetch_row($res)) {
            if ($single) {
                $first_value = ['id' => $row[0], 'value' => $row[1]];
            } else {
                $first_value = ['id' => $row[0], 'value' => GetQueryValueSet(2, $row, GetMaxSetLevel($dbh, $_SESSION['Catalog']['0']['table']))];
            }
        }
...
function SearchMoving($dbh, $dir, $n, $portion, $settings_pad)
{
    if (! IsChangesAndErrors($n, Title(480).' ', ' '.Title(470))) { // [] = array("time"=>"", "text"=>"To perform searching You must save changes on this page", "status"=>"warning", "log"=>false);
        ...
        if ($dir == 'back') {
            $_SESSION['catalog_param'][$n]['start_pos'] = max($_SESSION['catalog_param'][$n]['start_pos'] - $_SESSION['portion'] + 1, 0);
        }
...
function GetSearchPosOut($dbh, $n)
{
    ...
    if ($single) {
        ...
        if ($p[0] != '') {
            $q_prev = 'SELECT '.$_SESSION['Catalog'][$n]['value'].' FROM '.$_SESSION['Catalog'][$n]['table'].' WHERE '.WhereSingle($n, $_SESSION['match_case'], $p, $waw, '<').' ORDER BY '.MCF($_SESSION['Catalog'][$n]['value'], $_SESSION['match_case']).' DESC LIMIT 1';
        }
        if ($p[1] != '') {
            $q_next = 'SELECT '.$_SESSION['Catalog'][$n]['value'].' FROM '.$_SESSION['Catalog'][$n]['table'].' WHERE '.WhereSingle($n, $_SESSION['match_case'], $p, $waw, '>').' ORDER BY '.MCF($_SESSION['Catalog'][$n]['value'], $_SESSION['match_case']).' LIMIT 1';
        }
    } else {
        $f_a = [MCV($_SESSION['catalog_param'][$n]['filter_text'], $_SESSION['match_case']), $_SESSION['catalog_param'][$n]['filter_compare']];
        $s_a = [MCV($_SESSION['catalog_param'][$n]['search_text'], $_SESSION['match_case']), $_SESSION['catalog_param'][$n]['search_compare']];
        if ($p[0] != '') {
            $q_prev = QueryTextForSet($dbh, $_SESSION['match_case'], -1, $f_a, $s_a, 'f', '<', MCV($p[0], $_SESSION['match_case']), '1');
        }
        if ($p[1] != '') {
            $q_next = QueryTextForSet($dbh, $_SESSION['match_case'], 1, $f_a, $s_a, 'f', '>', MCV($p[1], $_SESSION['match_case']), '1');
        }
    }
...
{
    if (count($_SESSION['cat_arr'][$n]) == 0) {
        return [];
    }
    $lim_arr = reset($_SESSION['cat_arr'][$n]);
    if (! is_numeric(key($_SESSION['cat_arr'][$n]))) {
        return [];
    } // new??
    $res_lim[] = end($lim_arr);
    ...
    if (! is_numeric(key($_SESSION['cat_arr'][$n]))) { // new??
        ...
        $res_lim[] = end($_SESSION['cat_arr'][$n]);
    }
