...
if (count($_POST) == 0) {
    ...
    if ($_SESSION['priority'] < 11) {
        ExitSession(Title(509).' <b>'.$_SESSION['Catalog'][$n]['name'].'</b> '.FTM(Title(296)).'. '.Title(510).'.'.'|FF0000', $_SESSION['db_info']['id']);
    }
    $_SESSION['cur_cat'] = '';
}
$line_code = [];
...
foreach ($_POST as $k => $v) {
    ...
    switch ($ak[0]) {
        ...
        case 'user_lang_s': CatalogLangChoice($dbh_sys, $dbh, $sw_break);
            break;
        case 'cat_exit': CatExitAction($dbh_sys, $sel, $_SESSION['cat_view'], $_SESSION['block'], $_SESSION['cur_cat'], $_SESSION['PR']['var'], $_SESSION['cat_flag']);
            break;
        case 'cat_rest': CatRestoreAction($dbh, $sel_value);
            break;
        case 'cat_save': UpdateCatalogs($dbh);
            TestCatalogs($dbh, false, false);
            if ($_SESSION['user_working_mode'] == 1) {
                $_SESSION['cat_arr_init'] = $_SESSION['cat_arr'];
            } break;
        case 'cat_test': TestCatalogs($dbh, false, true, ($_SESSION['Catalog']['1']['name'] == '') ? Title(215) : Title(126));
            break;
        case 'cat_height': ChangeScreenHeight($dbh, $ak[1]);
            break;
        case 'cat_view_messages': $_SESSION['view_err_mes'] = ! $_SESSION['view_err_mes'];
            TestCatalogs($dbh);
            break;
        case 'answer_yes': CatYesDelAction($dbh);
            break;
        case 'answer_no': CatNoDelAction($dbh, $_SESSION['block']);
            break;
        case 'topos': GoToCatalogPosition($dbh, $ak[1], $ak[2]);
            $_SESSION['block']['cat_goto'] = false;
            break;
        case 'cat_tree': $_SESSION['cat_tree'] = ! $_SESSION['cat_tree'];
            break;
        case 'cat_tree_collapse': SetAllCollapse($dbh, '+', $_SESSION['cat_arr']['0'], $_SESSION['set_pad']);
            break;
        case 'cat_tree_extend': SetAllCollapse($dbh, '-', $_SESSION['cat_arr']['0'], $_SESSION['set_pad']);
            break;
        case 'cat_filter': FFAction($dbh, 'filter_on', 'filter_compare', 'filter_anywhere', $ak[1], Title(110), $_SESSION['set_pad']);
            break;
        case 'cat_search': FFAction($dbh, 'search_on', 'search_compare', 'search_anywhere', $ak[1], Title(111), $_SESSION['set_pad']);
            break;
        case 'cat_search_move': SearchMoving($dbh, $ak[1], $ak[2], $_SESSION['set_pad']);
            break;
        case 'cat_search_hide': $_SESSION['catalog_param'][$ak[1]]['view_search'] = ! $_SESSION['catalog_param'][$ak[1]]['view_search'];
            break;
        case 'cat_fill': CatFill($dbh);
            break;
        case 'filter_start': ApplyFilter($dbh, $ak[1], $_POST['filter_text|'.$ak[1]], $_POST['filter_mode|'.$ak[1]], $_SESSION['set_pad']);
            break;
        case 'search_start': StartSearch($dbh, $ak[1], $_POST['search_text|'.$ak[1]], $_POST['search_mode|'.$ak[1]], $_SESSION['set_pad']);
            break;
        case 'cat_nav': NavigationActions($dbh, $ak[1], $ak[2], $_SESSION['set_pad']);
            break;
        case 'cat_code': $line_code = (($ak[1] == '0') ? ['select', $ak[2]] : ['insert', $ak[2]]);
            break; // new??
        case 'collapse': SetCollapse($dbh, $ak[2], $ak[3], $_POST['collapse|0|'.$ak[2].'|'.$ak[3]], $_SESSION['set_pad']);
            break;
        case 'set_vis': SetOnOffButton($_SESSION['cat_vis'], $ak[2]);
            break;
        case 'cat_mark': SetOnOffButton($_SESSION['cat_mark'], $ak[2]);
            break;
        case 'cat_copy': $_SESSION['copy_paste'] = CopyBranch($ak[3], $ak[2], $_SESSION['cat_arr']['0'][$ak[2]], $_SESSION['Catalog']['0']['separator']);
            break;
        case 'cat_paste': CatBranchAction($dbh, $ak[2], $_SESSION['copy_paste']);
            break;
        case 'cat_paste_root': CatBranchAction($dbh, '', $_SESSION['copy_paste']);
            break;
        case 'del_hier_set': DelHierarchic($dbh, $ak[2]);
            break;
        case 'del_from_set': if (is_numeric($ak[2])) {
            DelFromSetList($dbh, $_SESSION['cat_vis'], $ak[2]);
        } break; // new??
...
if ($line_code == 'select') {
    CatLineSelectAction($dbh, $line_code[1]);
} elseif ($line_code == 'insert') {
    InsMarked($dbh, $_SESSION['cat_mark'], $line_code[1], $_POST['cat_text|1|'.$line_code[1]]);
}
...
<form method="post" id="catalog_form" name="catalog_form">
	...
	<?php ViewNowMessages();
if ($_SESSION['view_err_mes']) {
    ViewMessages(false, '', $_SESSION['number_warn'], $_SESSION['Catalog'], $_SESSION['del_row'], $_SESSION['block'], true);
}?>
	<table>
		<tr valign="top">
