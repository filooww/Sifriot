...
function CatalogSelectCall($dbh, $conf, $k, $user_id, $sel_value, &$cur_value)
{
    $_SESSION['Catalog']['0'] = [];
    switch ($k) {
        case 'Authors':		$C['0'] = ['name' => 'Author sets', 'tbl' => 'author_groups', 'id' => 'id_author_group', 'vl' => 'author_set', 'spr' => GetTableValue($dbh, 'author_groups', 'separators', false), 'cat_type' => 'abc'];
            $C['1'] = ['name' => 'Authors', 'tbl' => 'authors', 'id' => 'id_author', 'vl' => 'author', 'cat_type' => ''];
            break;
        case 'Themes':		$C['0'] = ['name' => 'Theme sets', 'tbl' => 'theme_sets', 'id' => 'id_theme_set', 'vl' => 'theme_set', 'spr' => GetTableValue($dbh, 'author_groups', 'separators', false), 'cat_type' => 'ordinary'];
            $C['1'] = ['name' => 'Themes', 'tbl' => 'themes', 'id' => 'id_theme', 'vl' => 'theme', 'cat_type' => ''];
            break;
        case 'Publishing':	$C['0'] = ['name' => 'Publishings', 'tbl' => 'publishings', 'id' => 'id_publishing', 'vl' => 'publishing', 'spr' => '', 'cat_type' => ''];
            $C['1'] = ['name' => '', 'tbl' => '', 'id' => '', 'vl' => '', 'cat_type' => ''];
            break;
        case 'Series':		$C['0'] = ['name' => 'Series sets', 'tbl' => 'part_sets', 'id' => 'id_part_set', 'vl' => 'part_set', 'spr' => GetTableValue($dbh, 'part_sets', 'separators', false), 'cat_type' => 'hierarchic'];
            $C['1'] = ['name' => 'Series', 'tbl' => 'parts', 'id' => 'id_part', 'vl' => 'part', 'cat_type' => ''];
            break;
        case 'IssueType':	$C['0'] = ['name' => 'Issue types', 'tbl' => 'issue_types', 'id' => 'id_issue_type', 'vl' => 'issue_type', 'spr' => '', 'cat_type' => ''];
            $C['1'] = ['name' => '', 'tbl' => '', 'id' => '', 'vl' => '', 'cat_type' => ''];
            break;
        case 'Magazine':	$C['0'] = ['name' => 'Magazines', 'tbl' => 'magazines', 'id' => 'id_magazine', 'vl' => 'magazine', 'spr' => '', 'cat_type' => ''];
            $C['1'] = ['name' => '', 'tbl' => '', 'id' => '', 'vl' => '', 'cat_type' => ''];
            break;
    }
    if (count($_SESSION['Catalog']['0']) > 0) {
        ...
        $_SESSION['cat_arr'] = ['0' => GetCatalogPortion($dbh, '0', $_SESSION['catalog_param']['0']['start_pos'], $_SESSION['portion'], true), '1' => []];
        $_SESSION['catalog_param']['0']['total_count'] = GetTableLimit($dbh, 'total', '0');
        if ($_SESSION['Catalog']['0']['cat_type'] == 1) {
            $_SESSION['catalog_param']['tree_total_count'] = TreeCounts($_SESSION['catalog_param']['total_count']);
        }
        $cur_value = $sel_value;
...
function ViewTableForSelect($cat_vis, $cg, $block, $tree, $moda, $sel_modes, $cur_value, $ccv)
{
    if ($_SESSION['catalog_param']['0']['filter_on']) {
        FormForSelection(Title(99), 'FFCC00', 'filter_text0', 'filter_start0', 'apply filter', 'filter_mode0', '0', 'filter_text', 'filter_mode_values', 'filter_compare');
    }
    if ($_SESSION['catalog_param']['0']['search_on']) {
        FormForSelection(Title(83), '00FFFF', 'search_text0', 'search_start0', 'start search', 'search_mode0', '0', 'search_text', 'search_mode_values', 'search_compare');
    }
    ViewTitleMenu('0');
    echo "<table frame='border'>";
    echo "<tr valign='top'>";
...
function ViewPortionForSelect($s_type, $conf, $cat_vis, $tree, $cur_value, $ccv)
{
    ...
    foreach ($_SESSION['cat_arr']['0'] as $k_ref => $v_ref) {
        ...
        if ($_SESSION['Catalog']['0']['cat_type'] == 1 && $tree) {
            echo "<td><input type='text' name='cat_text".$cc."' value='".TreeViewRow(end($v_ref))."' size=".$conf['w_08']."' readonly ".$cl.'></td>';
        } else {
            echo "<td><input type='text' name='cat_text".$cc."' value='".end($v_ref)."' size=".$conf['w_08']."' readonly ".$cl.'></td>';
        }
        if ($_SESSION['catalog_param']['0']['found_count'] > 0 && $_SESSION['catalog_param']['0']['view_search']) {
            echo '<td>'.RowForOutput(end($v_ref), '0', $s_type).'</td>';
        }
        echo '</tr>';
        if ($cat_vis['b_on'] && $k_ref == $cat_vis['bc'] && end($v_ref) != '') {
            SetByRows($k_ref, $v_ref, $blks);
        }
    }
