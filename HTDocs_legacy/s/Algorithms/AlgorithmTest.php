...
{
    $_SESSION['alg_flag'][$k]['id'] = ((int) $k == 0);
    $_SESSION['alg_flag'][$k]['offset'] = (! is_numeric($v['offset']) || is_numeric($v['offset']) && (int) $v['offset'] < 0);
...
function AlgorithmRemarkTest($k, $check_value)
{
    ...
    if (strpos($check_value, chr(39)) === false) {
        $fl_apostr = false;
    } else {
...
function TestAlgorithmCheck($k, $param_name, $check_value)
{
    if ($check_value == 0 || $check_value == 1) {
        $_SESSION['alg_flag'][$k][$param_name] = false;
    } else {
