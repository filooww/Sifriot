...
{
$l_f = fopen("D:\\Test.txt", "a"); fwrite($l_f, "\r\ntest UpdateOnAlgorithm 00 ***".$d_file."===\r\n"); fclose($l_f); ArrayToFile("D:\\Test.txt", "", $arr_alg_seq, 0);
	$arr_group = array();
	foreach ($arr_alg_seq as $v) 
	{
$l_f = fopen("D:\\Test.txt", "a"); fwrite($l_f, "\r\ntest UpdateOnAlgorithm 01 ***\r\n"); fclose($l_f); ArrayToFile("D:\\Test.txt", "", $v, 0);
$l_f = fopen("D:\\Test.txt", "a"); fwrite($l_f, "\r\ntest UpdateOnAlgorithm 02 ***\r\n"); fclose($l_f); ArrayToFile("D:\\Test.txt", "", $main_params, 0);
		$k_alg = FindAlgInFV($v, $PR['var']);
$l_f = fopen("D:\\Test.txt", "a"); fwrite($l_f, "\r\ntest UpdateOnAlgorithm 03 ***".$k_alg."===\r\n"); fclose($l_f); ArrayToFile("D:\\Test.txt", "", $arr_group, 0);
...
$l_f = fopen("D:\\Test.txt", "a"); fwrite($l_f, "\r\ntest UpdateOnAlgorithm 05 ***\r\n"); fclose($l_f); ArrayToFile("D:\\Test.txt", "", $file_arr, 0);
...
$l_f = fopen("D:\\Test.txt", "a"); fwrite($l_f, "\r\ntest UpdateOnAlgorithm 10 ***".TestBool(isset($main_params['const'][$k]['table']))."==="); fclose($l_f);
		if ($main_params['const'][$k]['table'] != "")
		{
$l_f = fopen("D:\\Test.txt", "a"); fwrite($l_f, "\r\ntest UpdateOnAlgorithm 11 ***".TestBool(isset($main_params['const'][$k]['table1']))."==="); fclose($l_f);
...
$l_f = fopen("D:\\Test.txt", "a"); fwrite($l_f, "\r\ntest UpdateOnAlgorithm 12 ***\r\n"); fclose($l_f); ArrayToFile("D:\\Test.txt", "", $ref_codes, 0);
...
$l_f = fopen("D:\\Test.txt", "a"); fwrite($l_f, "\r\ntest UpdateOnAlgorithm 13 ***".$main_params['var'][$k]['code']."==="); fclose($l_f);
				if ($main_params['var'][$k]['code'] != "") SaveMaxLevel($dbh, count($ref_codes));
			}
			else 
{
...
$l_f = fopen("D:\\Test.txt", "a"); fwrite($l_f, "\r\ntest UpdateOnAlgorithm 14 ***".$main_params['var'][$k]['code']."==="); fclose($l_f);
}
		}
		else 
{
		$main_params['var'][$k]['code'] = ltrim(rtrim($v[0]));
$l_f = fopen("D:\\Test.txt", "a"); fwrite($l_f, "\r\ntest UpdateOnAlgorithm 15 ***".$main_params['var'][$k]['code']."==="); fclose($l_f);
}
	}
	if ($main_params['var']['Title']['code'] == "") $main_params['var']['Title']['code'] = $file_arr[0];
$l_f = fopen("D:\\Test.txt", "a"); fwrite($l_f, "\r\ntest UpdateOnAlgorithm 16 ***".$main_params['var']['Title']['code']."==="); fclose($l_f);
}
...
function GetFieldPart($v, &$source_field)
{
	...
	{
		$pos1 = ($pos_beg['delim'] == "") ? 0 : $pos_beg['pos'] + (($v['beg_inc'] == 1) ? 0 : strlen($pos_beg['delim']));
		...
		if ($v['del_from_source'] == 1) $source_field = str_replace($selit, "", $source_field);
		if ($v['beg_scr'] == 1 && $pos_beg['delim'] != "") $source_field = substr($source_field, 0, $pos_beg['pos']).substr($source_field, $pos_beg['pos'] + strlen($pos_beg['delim']));
	}
	return $selit;
}
function PosSearchIndex($pos_arr, $init_pos, $delim_searched)
{
	if (count($pos_arr) == 0) return array("pos" => -1, "delim" => "");
	foreach ($pos_arr as $z) if ($z > $init_pos) return array("pos" => $z, "delim" => $delim_searched);
	return array("pos" => -1, "delim" => "");
}
