...
function ChangeAlgorithmScreen($dbh, $offs)
{
    ...
    if (gettype($offs) == 'integer') {
        if ($offs == 1) {
            ...
            if ($_SESSION['portion'] > count($_SESSION['arr_alg'])) {
                $_SESSION['portion']--;
            } elseif ($_SESSION['portion'] == 1) {
                $mes_screen = "<font color='#FF0000'>".Title(178).'</font>';
            } else {
                end($_SESSION['arr_alg']);
                if (PermitRowAction(key($_SESSION['arr_alg']))) {
                    if (count($_SESSION['arr_alg']) == $_SESSION['portion']) {
                        array_pop($_SESSION['arr_alg']);
                    }
                    $_SESSION['portion']--;
                } else {
                    $mes_screen = "<font color='#FF0000'>".Title(237).' '.Title(471).'</font>';
                }
            }
        }
    } else {
        if (! is_numeric($offs)) {
            $mes_screen = "<font color='#FF0000'>".Title(177).' '.Title(77).'</font>';
        } elseif ((int) $offs < 1 || strpos($offs, '-') !== false || strpos($offs, '.') != false) {
            $mes_screen = "<font color='#FF0000'>".Title(177).' '.Title(512).' ('.Title(299).' <b>'.$offs.'</b>)</font>';
        } elseif ((int) $offs > count($_SESSION['arr_alg'])) {
            $old = $_SESSION['portion'];
            $_SESSION['portion'] = (int) $offs;
            if (count($_SESSION['arr_alg']) >= $old) {
                GetAlgorithmPortion($dbh);
            }
        } elseif ($_SESSION['alarm']) {
            SetAlarmAlgorithmPortionSize((int) $offs);
        } else {
            $mes_screen = SetAlgorithmPortionSize((int) $offs);
        }
    }
...
{
    if (is_numeric($_POST['algorithm_height']) && (int) $_POST['algorithm_height'] > 0) {
        SaveUserScreenPortion($dbh, $_POST['algorithm_height']);
    }
    if ($_SESSION['alarm']) {
        ResetAlgorithmParse();
        AlgorithmsCheck($dbh);
...
{
    if (AlgorithmErrorsFree($k, $err_types)) {
...
{
    if ($_SESSION['algorithm_info'] == $k && $_SESSION['algorithm_info'] != '') {
        return $unhide_img;
    }
    if (AlgorithmErrorsFree($k, $err_types)) {
        return $no_err_img;
...
{
    if ($_SESSION['alarm']) {
        if (in_array($k, $_SESSION['alg_del'])) {
            return false;
        }
...
{
    if ($flag_name == 'id') {
        return ($_SESSION['alg_flag'][$k]['id']) ? Title(175) : '';
    }
    if ($flag_name == 'beg_del') {
        return (isset($_SESSION['update_replace'][$k]['beg_del'])) ? Title(120).' '.Title(469).' '.FTM(Title(277)).' '.$_SESSION['apostrophe_replace'] : '';
    }
    if ($flag_name == 'inn_del') {
        return (isset($_SESSION['update_replace'][$k]['inn_del'])) ? Title(120).' '.Title(469).' '.FTM(Title(277)).' '.$_SESSION['apostrophe_replace'] : '';
    }
    if ($flag_name == 'end_del') {
        return (isset($_SESSION['update_replace'][$k]['end_del'])) ? Title(120).' '.Title(469).' '.FTM(Title(277)).' '.$_SESSION['apostrophe_replace'] : '';
    }
    if ($flag_name == 'del_sym') {
        return (isset($_SESSION['update_replace'][$k]['del_sym'])) ? Title(120).' '.Title(469).' '.FTM(Title(277)).' '.$_SESSION['apostrophe_replace'] : '';
    }
    if ($flag_name == 'ins_sym') {
        return (isset($_SESSION['update_replace'][$k]['ins_sym'])) ? Title(120).' '.Title(469).' '.FTM(Title(277)).' '.$_SESSION['apostrophe_replace'] : '';
    }
    if ($flag_name == 'reg_exp') {
        return (isset($_SESSION['update_replace'][$k]['reg_exp'])) ? Title(120).' '.Title(469).' '.FTM(Title(277)).' '.$_SESSION['apostrophe_replace'] : '';
    }
    if ($flag_name == 'remarks') {
        if (isset($_SESSION['update_replace'][$k]['remarks'])) {
            return Title(120).' '.Title(469).' '.FTM(Title(277)).' '.$_SESSION['apostrophe_replace'];
        }
...
function HideUnhideAlgorithmInfo($dbh, $k)
{
    if ($_SESSION['algorithm_info'] == '') {
        ...
        if ($k == $_SESSION['algorithm_info']) {
            ...
            if (! $fl && isset($_SESSION['arr_alg'][$_SESSION['algorithm_info']])) {
                unset($_SESSION['arr_alg'][$_SESSION['algorithm_info']]);
            }
            ResetAlgorithmParse();
...
function AlgorithmCheckBox($k, $v_check, $flag_name, $dis_alg)
{
    ...
    if (isset($_SESSION['update_replace'][$k][$flag_name]) && $_SESSION['update_replace'][$k][$flag_name] != '') {
        echo "<font color='#FFFFFF'>:</font><font color='#FF0000'><b>".$_SESSION['update_replace'][$k][$flag_name].'</b></font>';
    }
}
...
{
    $alg_err['full'] = [['id', 'offset', 'del_from_source', 'beg_del', 'beg_num', 'beg_inc', 'beg_scr', 'inn_del', 'end_del', 'end_num', 'end_inc', 'end_scr', 'del_sym', 'ins_sym', 'field_only', 'reg_exp', 'reg_scr', 'remarks'], ['beg_del', 'inn_del', 'end_del', 'del_sym', 'ins_sym', 'reg_exp', 'remarks']];
    $alg_err['id'] = [['id'], []];
    $alg_err['remarks'] = [['remarks'], ['remarks']];
    $alg_err['add'] = [['offset', 'del_from_source', 'beg_del', 'beg_num', 'beg_inc', 'beg_scr', 'inn_del', 'end_del', 'end_num', 'end_inc', 'end_scr', 'del_sym', 'ins_sym', 'field_only', 'reg_exp', 'reg_scr'], ['beg_del', 'inn_del', 'end_del', 'del_sym', 'ins_sym', 'reg_exp']];
    $alg_err['replace_fields'] = ['del_from_source', 'beg_del', 'beg_inc', 'beg_scr', 'inn_del', 'end_del', 'end_inc', 'end_scr', 'del_sym', 'ins_sym', 'field_only', 'reg_exp', 'reg_scr'];
...
function GetAlgorithmTableLimits($init_k, $end_k, $ark)
{
    ...
    if ($init_k == '') {
        ...
        $i = array_search((int) $init_k, $ark);
        if ($i !== false && $i < count($ark) - 1) {
            $i_lim[0] = $i + 1;
        }
    }
    if ($end_k == '') {
...
function AlgorithmTableRows($init_k, $end_k, $t_del)
{
    ...
    if ($i_lim[0] > -1 && $i_lim[1] > -1) {
        ...
        if ($end_k == '') {
            for ($i = count($_SESSION['arr_alg']); $i < $_SESSION['portion']; $i++) {
...
{
    if ($_SESSION['user_working_mode'] == 0 || ! $_SESSION['algorithm_delete'] && $k != $_SESSION['edit_algorithm']) {
        return true;
    }
...
{
    if ($_SESSION['alg_flag'][$k][$flag_name]) {
        return " class='".$err_class."'";
    }
    if (isset($_SESSION['update_replace'][$k][$flag_name]) && $_SESSION['update_replace'][$k][$flag_name]) {
        return " class='".$err_class."'";
    }
...
{
    if (isset($_SESSION['algorithm_info'])) {
        unset($_SESSION['algorithm_info']);
    }
    if (isset($_SESSION['alg_del'])) {
        unset($_SESSION['alg_del']);
    }
    if (isset($_SESSION['arr_alg'])) {
        unset($_SESSION['arr_alg']);
    }
    if (isset($_SESSION['path_parse'])) {
        unset($_SESSION['path_parse']);
    }
    if (isset($_SESSION['perform_parse'])) {
        unset($_SESSION['perform_parse']);
    }
    if (isset($_SESSION['path_example'])) {
        unset($_SESSION['path_example']);
    }
    if (isset($_SESSION['file_example'])) {
        unset($_SESSION['file_example']);
    }
    if (isset($_SESSION['edit_algorithm'])) {
        unset($_SESSION['edit_algorithm']);
    }
    if (isset($_SESSION['algorithm_insert'])) {
        unset($_SESSION['algorithm_insert']);
    }
    if (isset($_SESSION['alg_flag'])) {
        unset($_SESSION['alg_flag']);
    }
    if (isset($_SESSION['algorithm_size'])) {
        unset($_SESSION['algorithm_size']);
    }
    if (isset($_SESSION['algorithm_err'])) {
        unset($_SESSION['algorithm_err']);
    }
    if (isset($_SESSION['update_replace'])) {
        unset($_SESSION['update_replace']);
    }
}
