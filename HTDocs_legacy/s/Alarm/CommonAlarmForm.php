...
foreach (array_keys($_POST) as $str_key) {
    ...
    switch ($str_key) {
        case 'user_lang_s': AfterLangChoice($dbh, 'user_lang_s', 'user_lang', $sw_break);
            break;
        case 'sys_exit': ExitSession();
            break;
        case 'coding_table': $_SESSION['language_messages'] = [];
            header('Location: ../Codings/CodingForm.php');
            break;
        case 'interface_texts': GoToTitles($dbh);
            header('Location: ../Titles/TitleForm.php');
            break;
        case 'languages': header('Location: ../Languages/LanguageForm.php');
            break;
        case 'translate_table': header('Location: ../LocalLanguages/LocalForm.php');
            break;
        case 'user_ident': header('Location: ../UserList/UserListForm.php');
            break;
        case 'visits': header('Location: ../Visits/VisitForm.php');
            break;
        case 'db_list': $_SESSION['lang_param'] = ReadLanguages($dbh, true);
            header('Location: ../DataBases/DataBasesForm.php');
            break;
        case 'db_s_configs': $_SESSION['old_number_warn'] = $_SESSION['number_warn'];
            $_SESSION['common_config'] = true;
            $_SESSION['config_list'] = GetAllConfigs($dbh, 'db_s_configs');
...
if (count($Mes) > 0) {
    foreach ($Mes as $z) {
        echo "<div><font color='#FF0000'><b>".$z.'</b></font></div>';
    }
}
?>
<form method="post" id="alarm_form" name="alarm_form">
	...
	<hr align="left" size="1" noshade="noshade" color="#000000" >
	<?php if (IsInvalidReferences()) {
	    require_once 'ReferenceAlarmHTML.php';
	} ?>
</form>
