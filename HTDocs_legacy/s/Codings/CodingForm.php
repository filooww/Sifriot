...
if (count($_POST) == 0) {
    ...
    foreach ($_SESSION['coding_list'] as $k => $v) {
        if (isset($_POST['coding|'.(string) $k]) && $_POST['coding|'.(string) $k] != $v[0]) {
            $_SESSION['coding_list'][$k][0] = $_POST['coding|'.(string) $k];
        }
    }
}
foreach ($_POST as $str_key => $str_v) {
    ...
    $sw_break = true;
    switch ($s_k) {
        case 'idle_button': TestCodingList($dbh);
            break;
        case 'user_lang_s': if (AfterLangChoice($dbh, 'user_lang_s', 'user_lang', $sw_break)) {
            TestCodingList($dbh);
        } break;
        case 'coding_OK': if (isset($_SESSION['pre_coding_err'])) {
            unset($_SESSION['pre_coding_err']);
        } TestCodingList($dbh);
            RewriteCoding($dbh, $Mes);
            $_SESSION['coding_del'] = true;
            break;
        case 'coding_exit': $goto = CodingExit($dbh, $Mes);
            if ($goto != '') {
                header('Location: '.$goto.'.php');
            } break;
        case 'coding_restore': if (isset($_SESSION['pre_coding_err'])) {
            unset($_SESSION['pre_coding_err']);
        } $_SESSION['del_coding'][$k[1]][0] = $_SESSION['del_coding'][$k[1]][1];
            TestCodingList($dbh);
            break;
        case 'coding_del': CodingDel($dbh);
            $_SESSION['del_coding'] = [];
            $_SESSION['coding_del'] = false;
            TestCodingList($dbh);
            $Mes[] = '<b>'.Title(694).'</b>';
            break;
        case 'coding_no_del': $_SESSION['coding_del'] = false;
            DelCodingListRestore();
            TestCodingList($dbh);
            break;
    ...
    <table>
	    <tr>
		    ...
		    <?php if ($_SESSION['user_working_mode'] == 1) {
		        echo "<td><button name='coding_OK'".$dis_del." type='submit' class='save_button'>".Title(30).'</button></td>';
		    } ?>
	    </tr>
    </table>
    <?php
    if ($fl_del) {
        DBMDelQuestion('modal_form', ['coding_del', 'coding_no_del'], CodingSetQuestionString(), 208, 702, 703);
    } ?>
    <table>
        <tr>
            <td>
