...
function SetDoubleCodingParameters($v)
{
    for ($i = 0; $i < count($v); $i++) {
        ...
        if ($_SESSION['coding_list'][$v[$i]][2] == '') {
            $_SESSION['coding_list'][$v[$i]][2] = "<font color='#FF0000'>".Title(700).' '.FTM(Title(330)).' '.((count($arr_v) == 1) ? Title(297) : Title(298)).' <b>'.implode(', ', $arr_v).'</b></font>';
        } else {
            $_SESSION['coding_list'][$v[$i]][2] .= "; <font color='#FF0000'>".Title(700).' '.FTM(Title(330)).' '.((count($arr_v) == 1) ? Title(297) : Title(298)).' <b>'.implode(', ', $arr_v).'</b></font>';
        }
    }
...
function TestCodingList($dbh, $init = false)
{
    ...
    foreach ($_SESSION['coding_list'] as $k => $v) {
        TestCodingRow($dbh, $k, $v[0], $init);
    }
    if (! $init) {
        $dd = TestCodingDouble();
        if (count($dd) > 0) {
            foreach ($dd as $v) {
                SetDoubleCodingParameters($v);
            }
        }
    }
}
function TestCodingRow($dbh, $k, $v0, $init)
{
    ...
    if ($v0 == '' && gettype($k) == 'integer') {
        TestEmptyCoding($dbh, $k, $init, $arr_mark);
    }
    ...
    if (isset($_SESSION['pre_coding_err'][$k])) {
        $arr_mark[] = "<font color='#0000FF'>".DigitsToTitle($_SESSION['pre_coding_err'][$k]).'</font>';
    }
    if (count($arr_mark) > 0) {
        $_SESSION['coding_list'][$k][2] = implode('; ', $arr_mark);
    } else {
        $_SESSION['coding_list'][$k][2] = '';
    }
}
...
function InvalidCodinglMessage($k, $coding_refs)
{
    ...
    if ($_SESSION['coding_list'][$k][0] != $_SESSION['coding_list'][$k][1]) {
        $str .= " <button name='coding_restore|".(string) $k."' type='submit'>".FTM(Title(224)).' '.Title(490).'</button>';
    }
    ...
    foreach ($_SESSION['coding_list'] as $k => $v) {
        if ($v[0] != '' && ! $v[2]) {
            if (gettype($k) == 'string' && $v[0] != '') {
                InsertNewCoding($dbh, $v[0], $fl_change);
...
function InsertNewCoding($dbh, $v0, &$fl_change)
{
    ...
    $_SESSION['coding_list'][$new_coding_id] = [$v0, $v0, '', false];
    unset($_SESSION['coding_list']['new']);
    ksort($_SESSION['coding_list']);
    $_SESSION['coding_list']['new'] = ['', '', '', false];
...
{
    if (isset($_SESSION['pre_coding_err'])) {
        unset($_SESSION['pre_coding_err']);
    }
    ...
    foreach ($_SESSION['del_coding'] as $z) {
        unset($_SESSION['coding_list'][$z]);
    }
}
function CodingSetQuestionString()
{
    $arr = [];
    foreach ($_SESSION['del_coding'] as $k_del) {
        $arr[] = $_SESSION['coding_list'][$k_del][1].' ('.$k_del.')';
    }
    ...
    if ($res) {
        while ($row = mysqli_fetch_row($res)) {
            ...
            if (count($arr) > $_SESSION['number_warn']) {
                $coding_refs[$row[0]] = implode(', ', array_slice($arr, 0, $_SESSION['number_warn'])).', ...';
            } else {
                $coding_refs[$row[0]] = $row[1];
            }
        }
...
{
    foreach ($_SESSION['coding_list'] as $v) {
        if ($v[3]) {
            return true;
        }
    }
...
function CodingExit($dbh, &$Mes)
{
    ...
    if ($ret_addr != '') {
        ...
        if (isset($_SESSION['del_coding'])) {
            unset($_SESSION['del_coding']);
        }
        if (isset($_SESSION['pre_coding_err'])) {
            unset($_SESSION['pre_coding_err']);
        }
...
function DelCodingListRestore()
{
    ...
    foreach ($_SESSION['del_coding'] as $k) {
        if ($_SESSION['coding_list'][$k][0] != $_SESSION['coding_list'][$k][1]) {
            $_SESSION['coding_list'][$k][0] = $_SESSION['coding_list'][$k][1];
        }
    }
    $_SESSION['del_coding'] = [];
}
