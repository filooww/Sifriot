...
if (! $dbh_sys) {
    ExitSession(Title(252).'|FF0000', '0');
}
$Mes = [];
if (count($_POST) == 0) {
    ...
    $_SESSION['login'] = ['name' => '', 'password' => '', 'password_d' => ''];
...
foreach ($_POST as $str_key => $str_v) {
    ...
    switch ($str_key) {
        case 'user_lang_s': AfterLangChoice($dbh_sys, 'user_lang_s', 'user_lang', $sw_break);
            break;
        case 'register_OK': $sw_break = false;
            $_SESSION['registration'] = true;
            $_SESSION['try_count'] = 0;
            break;
        case 'login_OK': case 'log_OK': $sw_break = false;
            $goto = UserLogin($dbh_sys, $Mes);
            if ($goto != '') {
                header('Location: '.$goto);
            } break;
        case 'trial_OK': $goto = UserGuest($dbh_sys);
            if ($goto != '') {
                header('Location: '.$goto);
            } break;
        case 'user_exit': ExitSession();
            break;
...
<form method="post" id="login_form" name="login_form">
    ...
    <?php
    if (count($_SESSION['structure_errors']) > 0 || $_SESSION['alarm']) {
        require_once $_SERVER['DOCUMENT_ROOT'].'/s/Alarm/SystemTitleErrors.php';
    } else {
        require_once 'LoginButtons.php';
    }
?>
	<table width="100%">
		<tr>
						...
						<td>
							...
							<table bgcolor="#CCFFFF" frame="border">
								...
								<?php if ($_SESSION['registration']) {
								    echo '<tr><td><b>'.Title(112)."</b></td><td><input name='user_password_d' type='password' value='".$_SESSION['login']['password_d']."'></td></tr>";
								}?>
							</table>
							<table>
								<tr>
                            ...
                            if (isset($_POST['login_OK']) || isset($_POST['log_OK'])) {
                                ...
                                for ($i = 0; $i < count($Mes); $i++) {
                                    echo "<tr><td><font color='#FF0000'><b>".$Mes[$i].'</b></font></td></tr>';
                                }
                                echo '</table>';
                            }
?>
