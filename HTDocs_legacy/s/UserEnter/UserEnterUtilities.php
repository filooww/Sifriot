...
{
    $u_data = ['id' => 0, 'db' => ''];
    $_SESSION['try_count'] = (int) $_POST['try_count'] + 1;
    if (! $_SESSION['registration'] && $_SESSION['try_count'] > $_SESSION['try_limit']) {
        ExitSession(Title(9).'|FF0000');
    } elseif (TestLoginFill($Mes)) {
        $u_data = GetUserData($dbh_sys, $Mes);
    }
    if ($u_data['id'] > 0) {
        $_SESSION['user_id'] = (string) $u_data['id'];
        if ($_SESSION['alarm'] && $_SESSION['priority'] < 99) {
            ExitSession(Title(610).'. '.Title(613).'.');
        } else {
            return SessionPreparation($dbh_sys, $u_data['db']);
        }
    } elseif ($u_data['id'] == 0) {
        return '';
    } else {
...
function TestLoginFill(&$Mes)
{
    if ($_SESSION['registration']) {
        ...
        } elseif ($_POST['user_password'] != $_POST['user_password_d']) {
            $Mes[] = Title(118);
        }
    } else {
        if ($_POST['user_name'] == '' && $_POST['user_password'] == '') {
            $Mes[] = Title(2);
        } elseif ($_POST['user_name'] == '' && $_POST['user_password'] != '') {
            $Mes[] = Title(3);
        } elseif ($_POST['user_name'] != '' && $_POST['user_password'] == '') {
            $Mes[] = Title(4);
        }
...
{
    if (strlen($user_data) < 3) {
        return false;
    }
    $arr_str = str_split($user_data);
    for ($i = 0; $i < count($arr_str); $i++) {
        if (ord($arr_str[$i]) > 32 && ord($arr_str[$i]) < 48) {
            return false;
        }
        if (ord($arr_str[$i]) > 57 && ord($arr_str[$i]) < 97) {
            return false;
        }
        if (ord($arr_str[$i]) > 122) {
            return false;
        }
    }
...
{
    if (strlen($user_data) < 6 || strlen($user_data) > 12) {
        return false;
    }
    $arr_str = str_split($user_data);
    for ($i = 0; $i < count($arr_str); $i++) {
        ...
        if (ord($arr_str[$i]) == 34 || ord($arr_str[$i]) == 39) {
            return false;
        }
        if (ord($arr_str[$i]) > 126) {
            return false;
        }
    }
...
function SetRegistration($dbh_sys, &$Mes)
{
    ...
    if ($res) {
        while ($row = mysqli_fetch_row($res)) {
            if ($row[1] == $_POST['user_name'] && $row[2] == $_POST['user_password']) {
                $Mes[] = Title(117);
                break;
            }
            if ($row[1] == $_POST['user_name'] && $row[2] != $_POST['user_password']) {
                $Mes[] = Title(113).' '.Title(587);
                break;
            }
            if ($row[1] != $_POST['user_name'] && $row[2] == $_POST['user_password']) {
                $Mes[] = Title(114).' '.Title(587);
                break;
            }
        }
...
function GetUserData($dbh_sys, &$Mes)
{
    $u_data = ['id' => 0, 'db' => ''];
    if ($_SESSION['registration']) {
        $id = SetRegistration($dbh_sys, $Mes);
    } else {
...
function SetSessionParameters($dbh_sys, $row)
{
    ...
    if ($_SESSION['user_lang'][0] != $row[4]) {
        if (isset($_SESSION['user_langs'][$row[4]])) {
            $_SESSION['user_lang'] = [$row[4], $_SESSION['user_langs'][$row[4]]];
            $_SESSION['titles'] = GetTitlesByLanguage($dbh_sys, (int) $_SESSION['user_lang'][0]);
        }
    }
...
function SessionFromPost()
{
    ...
    if (! $_SESSION['alarm']) {
        if (isset($_POST['user_password_d'])) {
            $_SESSION['login']['password_d'] = $_POST['user_password_d'];
        } else {
            $_SESSION['login']['password_d'] = '';
        }
    }
    if (isset($_POST['user_lang'])) {
        $lang_key = array_search($_POST['user_lang'], $_SESSION['user_langs']);
        if ($lang_key !== false) {
            $_SESSION['user_lang'] = [$lang_key, $_POST['user_lang']];
        }
    } else {
        $_SESSION['user_lang'] = [1, 'English'];
    }
}
...
function SessionSettings($dbh_sys)
{
    ...
    $_SESSION['compare_img'] = ['AtBegin', 'AnyWhere', 'Exact'];
...
function UserAlarmLogin($dbh_sys, &$Mes)
{
    $id = 0;
    $_SESSION['try_count'] = (int) $_POST['try_count'] + 1;
    if ($_SESSION['try_count'] > $_SESSION['try_limit']) {
        ExitSession('You have exceeded maximum number of login attempts');
    } elseif (TestAlarmLoginFill($Mes)) {
        $id = GetAlarmUserData($dbh_sys, $Mes);
    }
    if ($id > 0) {
        $_SESSION['user_id'] = (string) $id;
        if ($_SESSION['priority'] != 99) {
            ExitSession('Errors were found in the system tables. Contact system administrator.');
        } else {
            return SessionAlarmPreparation($dbh_sys);
        }
    } elseif ($id == 0) {
        return '';
    } else {
...
{
    if ($_POST['user_name'] == '' && $_POST['user_password'] == '') {
        $Mes[] = 'You didn`t fill in user name and password';
    } elseif ($_POST['user_name'] == '' && $_POST['user_password'] != '') {
        $Mes[] = 'You didn`t fill in user name';
    } elseif ($_POST['user_name'] != '' && $_POST['user_password'] == '') {
        $Mes[] = 'You didn`t fill in password';
    }
    if (count($Mes) == 0) {
        $fl_name = TestUserName($_POST['user_name']);
        $fl_pass = TestUserPassword($_POST['user_password']);
        if (! $fl_name && $fl_pass) {
            $Mes[] = 'User name rules violated';
        } elseif ($fl_name && ! $fl_pass) {
            $Mes[] = 'Password rules violated';
        } elseif (! $fl_name && ! $fl_pass) {
            $Mes[] = 'user name and password rules violated';
        }
    }
...
function GetAlarmUserData($dbh_sys, &$Mes)
{
    ...
    if ($res) {
        if ($row = mysqli_fetch_row($res)) {
            ...
            $Mes[] = 'User name and(or) password You entered is incorrect';
        }
    } else {
        $Mes[] = 'User name and(or) password You entered is incorrect';
    }
