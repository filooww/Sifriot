...
function ChangeUserListScreen($dbh, $offs)
{
    ...
    if (gettype($offs) == 'integer') {
        if ($offs == 1) {
            ...
            if ($_SESSION['portion'] > count($_SESSION['user_list'])) {
                $_SESSION['portion']--;
                ...
                if (PermitRowAction(key($_SESSION['user_list']))) {
                    if (count($_SESSION['user_list']) == $_SESSION['portion']) {
        ...
        if (! is_numeric($offs)) {
        ...
        } elseif ((int) $offs > count($_SESSION['user_list'])) {
            ...
            SetAlarmUserListPortionSize((int) $offs);
        } else {
            $mes_screen = SetUserListPortionSize((int) $offs);
        }
    }
...
function ShowUserCategoriesCounts()
{
    foreach ($_SESSION['categories'] as $k => $v) {
        ...
        if ($k != 'invalid') {
            echo (string) $v[3].'</b>';
            if ((int) $v[4] > 0) {
                echo " (<b><font color='#FF0000'>".$v[4].'</font></b>)';
            }
        } else {
            if ($v[3] == 0) {
                echo '0</b>';
            } else {
                echo "<font color='#FF0000'>".(string) $v[3].'</font></b>';
            }
        }
        echo '</td>';
...
function UserDataColor($k, $err_class, $norm_class, $flag_num)
{
    ...
    if (isset($_SESSION['update_replace'][$k][$flag_num]) && $_SESSION['update_replace'][$k][$flag_num]) {
        return " class='".$err_class."'";
    }
    if (isset($_SESSION['update_double'][$k][$flag_num]) && $_SESSION['update_double'][$k][$flag_num] != '') {
        return " class='".$err_class."'";
    }
...
{
    if ($_SESSION['user_info'] == $k && $_SESSION['user_info'] != '') {
        return $unhide_img;
    }
    if (UserErrorsFree($k, [7, 8, 9], [], 4)) {
        return $no_err_img;
    }
...
{
    $user_categories['all'] = [Title(169), 0, 99, 0, 0];
    $user_categories['admins'] = [Title(135), 11, 99, 0, 0];
    $user_categories['users'] = [Title(136), 1, 10, 0, 0];
    $user_categories['guests'] = [Title(137), 0, 0, 0, 0];
    $user_categories['invalid'] = [Title(232), -1, -1, 0, 0];

    return $user_categories;
}
function StringFieldErrors($k, $flag_num, &$arr, $double = true)
{
    if ($_SESSION['user_list'][$k][2] == 0 && $flag_num == 1) {
        $arr[] = Title(301);
    }
    if ($_SESSION['user_flag'][$k][$flag_num]) {
        $arr[] = Title(31).(($double && $_SESSION['user_list'][$k][2] == 0) ? ' ('.Title(618).')' : '');
    }
    if (isset($_SESSION['update_replace'][$k][$flag_num]) && $_SESSION['update_replace'][$k][$flag_num]) {
        $arr[] = Title(120).' '.Title(469).' '.FTM(Title(277)).' '.$_SESSION['apostrophe_replace'];
    }
    if ($double && isset($_SESSION['update_double'][$k][$flag_num]) && $_SESSION['update_double'][$k][$flag_num] != '') {
        $arr[] = Title(430);
    }
}
function GetDataTitle($k, $flag_num)
{
    $arr = [];
    switch ($flag_num) {
        case 0: if ($_SESSION['user_flag'][$k][0]) {
            $arr[] = Title(171);
        } break;
        case 1: case 2: StringFieldErrors($k, $flag_num, $arr);
            break;
        case 3: if ($_SESSION['user_flag'][$k][$flag_num]) {
            $arr[] = FTM(Title(232));
        } break;
        case 4: if ($_SESSION['user_flag'][$k][$flag_num]) {
            $arr[] = Title(546);
        } break;
        case 5: if ($_SESSION['user_flag'][$k][$flag_num]) {
            $arr[] = Title(547);
        } break;
        case 6: if ($_SESSION['user_flag'][$k][$flag_num]) {
            $arr[] = Title(548);
        } break;
        case 7: StringFieldErrors($k, $flag_num, $arr, false);
            break;
        case 8: if ($_SESSION['user_flag'][$k][$flag_num]) {
            $arr[] = Title(600).' '.chr(34).Title(49).chr(34);
        } break;
        case 9: if ($_SESSION['user_flag'][$k][$flag_num]) {
            $arr[] = Title(600).' '.chr(34).Title(595).chr(34);
        } break;
    }
...
function HideUnhideUserInfo($k)
{
    if ($_SESSION['user_info'] == '') {
        $_SESSION['user_info'] = $k;
    } else {
        if ($k == $_SESSION['user_info']) {
            $_SESSION['user_info'] = '';
        } else {
            $_SESSION['user_info'] = $k;
        }
...
function UserCheckBox($k, $v_check, $check_prefix, $flag_num, $t_user)
{
    ...
    if ($_SESSION['update_replace'][$k][$flag_num] != '') {
        echo "<font color='#FFFFFF'>:</font><font color='#FF0000'><b>".$_SESSION['update_replace'][$k][$flag_num].'</b></font>';
    }
}
function GetUserNameArray()
{
    $name_arr = [];
    foreach ($_SESSION['user_list'] as $v) {
        $name_arr[] = $v[0];
    }
...
function UserListMessages($mes_screen, $edit_mes, $Mes)
{
    echo '<table>';
    if ($mes_screen != '') {
        echo "<tr valign='top'><td>".$mes_screen.'</td></tr>';
    }
    if (count($edit_mes) > 0) {
        echo "<tr valign='top'><td>".ViewEditMessages($edit_mes).'</td></tr>';
    }
    if (count($Mes) > 0) {
        foreach ($Mes as $z) {
