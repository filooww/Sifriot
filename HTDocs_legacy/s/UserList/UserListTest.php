...
{
 	if (!is_numeric($k)) $_SESSION['user_flag'][$k][0] = true;
	elseif ((integer)$k == 0) $_SESSION['user_flag'][$k][0] = true;
	if (!is_numeric($v[2]) || is_numeric($v[2]) && (strpos($v[2], ".") !== false || (integer)$v[2] < 0 || (integer)$v[2] > 99)) $_SESSION['user_flag'][$k][3] = true;
	$_SESSION['user_flag'][$k][1] = !UserParamTest($v[0], $k, $v[2], 0, 1);
	$_SESSION['user_flag'][$k][2] = !UserParamTest($v[1], $k, $v[2], 1, 2);
    if (!isset($_SESSION['user_langs'][$v[3]])) $_SESSION['user_flag'][$k][4] = true;
	if (!is_numeric($v[4]) || is_numeric($v[4]) && (strpos($v[2], ".") !== false || (integer)$v[4] < 1)) $_SESSION['user_flag'][$k][5] = true;
	$_SESSION['user_flag'][$k][6] = PrefDBTest($k, $v[2], $v[5]);
    $_SESSION['user_flag'][$k][7] = !PrefDateTest($v[6], $k);
	if ($v[7] != 0 && $v[7] != 1) ReplaceUserCheck($k, 7, 8, $v[7]);
	if ($v[8] != 0 && $v[8] != 1) ReplaceUserCheck($k, 8, 9, $v[8]);
}
...
function PrefDBTest($k, $prior, $db)
{
    ...
    {
        if (!isset($_SESSION['list_db'][$db])) $fl = true;
    }
    else
    {
        ...
        elseif (!isset($_SESSION['list_db'][$db]) || $db == "0") $fl = true;
    }
    return $fl;
}
function PrefDateTest($pref_date, $k)
{
    if (strpos($pref_date, chr(39)) !== false) ReplaceUserApostrophe($k, 6, 7);
...
function UserParamTest($param, $k, $priority, $param_num, $flag_num)
{
    if (strpos($param, chr(39)) === false)
    {
        if ($priority == 0)
        {
            if ($param_num == 0) return ($param == "_".$k);
            else return ($param == "#####".$k);
        }
        else
        {
            if ($param_num == 0) return TestUserName($param);
            else return TestUserPassword($param);
        }
    }
    else
    {
...
{
	foreach ($_SESSION['user_list'] as $k => $v) if ($k != $k_edit && $v[$param_num] == $param_value) return $k;
	$test_result = "";
