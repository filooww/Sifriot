...
$Mes = array();
if (count($_POST) == 0) UserListInitial($dbh);
else UserSavePost();
$mes_screen = "";
$edit_mes = array();
foreach ($_POST as $str_key => $str_v)
{
	...
	switch ($s_k)
	{
		...
		case "user_lang_s"			: if (AfterUserListLangChoice($dbh, $sw_break)) UserFilter($dbh); break;
        case "replace_s"            : if (AfterReplaceSelect(array("li"=>$_SESSION['user_langs'], "db"=>$_SESSION['list_db']), $sw_break)) UserFilter($dbh); break;
		case "filter_category_s"	: AfterCategoryFilterChoice($dbh, $sw_break); break;
		case "list_errors_s"	    : if (AfterErrorsFilterChoice($dbh, $Mes, $sw_break)) UserFilter($dbh); break;
		case "user_exit"			: $_SESSION['user_info'] = ""; $goto = UserListExit($dbh, $Mes); if ($goto != "") header("Location: ".$goto.".php"); break;
		case "user_height_minus"	: $_SESSION['user_info'] = ""; $mes_screen = ChangeUserListScreen($dbh, -1); break;
		case "user_height_plus"	    : $_SESSION['user_info'] = ""; $mes_screen = ChangeUserListScreen($dbh, 1); break;
		case "user_height_button"	: $_SESSION['user_info'] = ""; $mes_screen = ChangeUserListScreen($dbh, $_POST['user_height']); break;
		case "user_navigation"		: $sw_break = false; $_SESSION['user_info'] = ""; UserListNavigation($dbh, $arr_key[1], $Mes); break;
		case "yes_del"				: $sw_break = false; DeleteUser($dbh); $_SESSION['user_delete'] = array("", "", array()); $_SESSION['user_info'] = ""; break;
		case "no_del"				: $sw_break = false; $_SESSION['user_info'] = ""; $_SESSION['user_list'][$_SESSION['edit_user']][0] = $_SESSION['user_delete'][1]; $_SESSION['user_delete'] = array("", "", array()); UserFilter($dbh); break;
		case "user_filter"			: $sw_break = false; $_SESSION['inv_ref'] = false; $_SESSION['user_info'] = ""; UserFilter($dbh); break;
		case "user_edit"			: $sw_break = false; $edit_mes = EditUser($dbh, $arr_key[1], $_SESSION['user_list'][$arr_key[1]], $_SESSION['user_flag'][$arr_key[1]], $Mes); break;
		case "user_info"	        : $sw_break = false; HideUnhideUserInfo($arr_key[1]); break;
		case "user_references"	    : $_SESSION['user_info'] = ""; GetInvRef("user_ident", array("li"=>$_SESSION['user_langs'], "db"=>$_SESSION['list_db'])); UserFilter($dbh); break;
		case "replace_id"           : ReplaceUserIDs($dbh, $arr_key[1], $arr_key[2]); break;
...
<form method="post" id="user_list_form" name="user_list_form">
	...
	<?php if ($_SESSION['inv_ref'] && PermitInvRef("user_ident", array("li", "db"))) InvRefTable("user_ident", "emph_data", array("li"=>$_SESSION['user_langs'], "db"=>$_SESSION['list_db'])); ?>
	<table>
		<tr>
			...
			<td><font color='#FFFFFF'>X</font></td>
			<?php
			if (PermitInvRef("user_ident", array("li", "db"))) echo "<td><button name='user_references' type='submit' value='*' class='i_r'".(($f_block || $_SESSION['user_filter'] != "") ? " disabled" : "").">".Title(541)."</button></td>";
	...
	<table>
		<tr>
            ...
            <td><font color="#FFFFFF">X</font></td>
			<td><?php echo FTM(Title(134))." "; SelectTag("user_filter_category", $_SESSION['categories'], $_SESSION['category'][0], "filter_category_s", true, 0, "filter_category_on", $f_block);?></td>
            <td><font color="#FFFFFF">X</font></td>
			<td>
	...
	<?php
	if ($_SESSION['user_delete'][0] != "") DBMDelQuestion("modal_form", array("yes_del", "no_del"), array($_SESSION['user_delete'][1]." (".$_SESSION['user_delete'][0].")"), 208, 211, 212);
	if (count($_SESSION['user_list']) == 0) echo "<div><center><b><font size=+1>".Title(12)." ".Title(226)."</b></font></center></div>";
	else
	{
        if ($mes_screen != "" || count($edit_mes) + count($Mes) > 0 || $_SESSION['user_delete'][0] != "") UserListMessages($mes_screen, $edit_mes, $Mes);
