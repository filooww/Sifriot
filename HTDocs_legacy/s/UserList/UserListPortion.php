...
function GetUserListPortion($dbh)
{
    ...
    $_SESSION['user_flag'] = array();
	$_SESSION['update_replace'] = array();
	$_SESSION['update_double'] = array();
    $ord = ($_SESSION['filter_id'] != "" && $_SESSION['filter_name'] == "") ? "id_user" : "name";
    if ($_SESSION['start'] >= $_SESSION['user_size']) $_SESSION['start'] = 0;
	...
	if ($res)
	{
		...
		{
			$_SESSION['user_list'][(string)$row[0]] = array($row[1], $row[2], (string)$row[3], $row[4], (string)$row[5], is_null($row[6]) ? "nu" : (string)$row[6], $row[7], $row[8], $row[9]);
			$_SESSION['user_flag'][(string)$row[0]] = array(false, false, false, false, false, false, false, false, false, false, false);
	        $_SESSION['update_replace'][(string)$row[0]] = array(1=>false, 2=>false, 7=>false, 8=>"", 9=>"");
	        $_SESSION['update_double'][(string)$row[0]] = array(1=>"", 2=>"");
		}
...
function UserListNavigation($dbh, $act, &$Mes)
{
    ...
    else $Mes[] = "<font color='#FF0000'><b>".Title(240)." ".Title(239)."</b></font>";
}
function StrG($k)
{
    if ($_SESSION['categories'][$k][1] == $_SESSION['categories'][$k][2]) return "user_priority = ".(string)$_SESSION['categories'][$k][1];
...
function StrN($user_category)
{
    ...
    if ($user_category != "guests") $cases[] = "LENGTH(name) < 3 OR name NOT REGEXP ".UserRegExp(0); // name
    else $cases[] = "name <> CONCAT('_',id_user)"; // name
    if ($user_category != "guests") $cases[] = "LENGTH(pass) < 6 OR LENGTH(pass) > 12 OR pass REGEXP ".UserRegExp(1); // pass
    else $cases[] = "pass <> CONCAT('#####',id_user)"; // pass
    ...
    if ($user_category == "admins") $cases[] = "(SELECT db_list.db_id FROM db_list WHERE db_list.db_id = user_ident.preffered_db LIMIT 1) IS NULL AND preffered_db IS NOT NULL"; // preffered_db
    else $cases[] = "((SELECT db_list.db_id FROM db_list WHERE db_list.db_id = user_ident.preffered_db LIMIT 1) IS NULL OR preffered_db = 0) AND preffered_db IS NOT NULL"; // preffered_db
...
{
    $cat_cond = array("admins"=>StrG("admins"), "users"=>StrG("users"), "guests"=>StrG("guests"));
    $arr_counts['admins'] = array($cat_cond['admins'], $cat_cond['admins']." AND ".StrN("admins"));
    $arr_counts['users'] = array($cat_cond['users'], $cat_cond['users']." AND ".StrN("users"));
    $arr_counts['guests'] = array($cat_cond['guests'], $cat_cond['guests']." AND ".StrN("guests"));
	...
	if ($res)
	{
		while ($row = mysqli_fetch_row($res))
        {
            $_SESSION['categories'][$row[1]][(integer)$row[2]] = $row[0];
            if ((integer)$row[2] == 3) $_SESSION['categories']['all'][3] += $row[0];
            else $_SESSION['categories']['all'][4] += $row[0];
        }
		mysqli_free_result($res);
...
function SetUserListPortionSize($int_offs)
{
	...
	for ($i = count($_SESSION['user_flag']) - 1; $i >= $int_offs && UserErrorsFree($k, array(1, 2, 7, 8, 9), array(1, 2)); $i--)
	{
	...
	if ($i >= $int_offs) $mes_screen = "<font color='#FF0000'>".Title(563)." ".Title(513)."</font>";
	return $mes_screen;
}
...
{
	for ($i = $f_beg; $i < $f_end; $i++) if ($_SESSION['user_flag'][$k][$i]) return false;
	for ($i = 0; $i < count($r_ind); $i++) if ($_SESSION['update_replace'][$k][$r_ind[$i]] || $_SESSION['update_replace'][$k][$r_ind[$i]] != "") return false;
	for ($i = 0; $i < count($d_ind); $i++) if ($_SESSION['update_double'][$k][$d_ind[$i]] != "") return false;
	return true;
}
function UserRegExp($field_number)
{
    if ($field_number == 0) return "'^[a-z0-9 ]+$'";
    else return "'[''".chr(34)." ]'";
}
