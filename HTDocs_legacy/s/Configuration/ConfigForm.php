...
foreach ($_POST as $str_key => $str_v)
{
	...
	switch ($s_k)
	{
        case "idle_button"  : TestConfigs($Mes, true); break;
		case "user_lang_s"	: if (AfterLangChoice($dbh_sys, "user_lang_s", "user_lang", $sw_break)) $arr_warn = BeforeSave($Mes, true); break;
		case "config_exit"	: $goto = ActionConfigExit($dbh_sys, $Mes); if ($goto != "") header("Location: ".$goto.".php"); break;
		case "config_save"	: $arr_warn = BeforeSave($Mes, false); break;
		case "config_fill"	: FillConfigs($dbh, $dbh_sys, ($_SESSION['common_config']) ? "db_s_configs" : "db_configs", $Mes); break;
		case "conf_type"	: TestConfigs($Mes, false, $arr_key[1]); break;
		case "yes_save"		: $_SESSION['config_list'] = RewriteConfigs(($_SESSION['common_config']) ? $dbh_sys : $dbh, ($_SESSION['common_config']) ? "db_s_configs" : "db_configs", $Mes); break;
		case "no_save"		: $_SESSION['config_list'] = ListCopy($_SESSION['config_list_copy']); TestConfigs($Mes, true); break;
...
<form method="post" id="config_form" name="config_form">
	...
	<table>
		<tr>
			...
			if ($_SESSION['user_working_mode'] == 1)
			{
				...
				if (!$_SESSION['common_config']) echo "<td><button name='config_fill'".$dis." type='submit' title='".Title(306)."' class='refresh_button'>".Title(306)."</button></td>";
			}
			?>
	...
	<?php if ($fl_save) QuestionForm("q_form", Title(418), array("yes_save", "no_save"), array(Title(209), Title(210)));?>
	<table>
		<tr>
			<td width="3%"></td>
			...
			<?php if (count($invalid_config_types) > 0) echo "<td width='3%'></td>";?>
		...
		foreach ($_SESSION['config_list'] as $k => $v)
		{
			echo "<tr>";
				...
				{
					if (in_array($k, $invalid_config_types)) echo "<td><input size='2' type='text' disabled class='data_numeric' value='".(string)$v[7]."'></td>";
					else echo "<td></td>";
				}
