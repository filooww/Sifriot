...
foreach ($_POST as $str_key => $str_v) {
    ...
    switch ($s_k) {
        case 'db_view_exit': ExitFromViewMode($dbh);
            break;
        case 'list_minus': $sw_break = false;
            ChangeMTScreenHeight($dbh, $_SESSION['db_info']['t_main'], -1, $_SESSION['item_arr'], $_SESSION['p_start'], $_SESSION['PR'], $_SESSION['mes']['m'], (int) $_SESSION['user_id']);
            break;
        case 'list_plus': $sw_break = false;
            ChangeMTScreenHeight($dbh, $_SESSION['db_info']['t_main'], 1, $_SESSION['item_arr'], $_SESSION['p_start'], $_SESSION['PR'], $_SESSION['mes']['m'], (int) $_SESSION['user_id']);
            break;
        case 'list_height_b': $sw_break = false;
            ChangeMTScreenHeight($dbh, $_SESSION['db_info']['t_main'], $_POST['list_height_v'], $_SESSION['item_arr'], $_SESSION['p_start'], $_SESSION['PR'], $_SESSION['mes']['m'], (int) $_SESSION['user_id']);
            break;
        case 'match_case_find': $sw_break = false;
            $_SESSION['item_arr'] = ChangeMatchCase($dbh, (int) $_SESSION['user_id']);
            break;
        case 'hide_list_flag': $sw_break = false;
            $_SESSION['hide_list'] = ! $_SESSION['hide_list'];
            break;
        case 'sort_find': $sw_break = false;
            SortFind();
            break;
        case 'multi_col': $sw_break = false;
            ChangeColumnsView($dbh, $_SESSION['m_col_v'], $_SESSION['f_view'], $_SESSION['p_code'], $_SESSION['URL_p'], (int) $_SESSION['user_id'], $_SESSION['p_files']['e'], $_SESSION['PR']['const']);
            break;
        case 'item_beg': $sw_break = false;
            MTNavigation($dbh, 'beg', $_SESSION['db_info']['t_main'], $_SESSION['p_count'], $_SESSION['p_start'], $_SESSION['item_arr'], $_SESSION['p_code'], (int) $_SESSION['user_id']);
            break;
        case 'item_pg_up': $sw_break = false;
            MTNavigation($dbh, 'pgup', $_SESSION['db_info']['t_main'], $_SESSION['p_count'], $_SESSION['p_start'], $_SESSION['item_arr'], $_SESSION['p_code'], (int) $_SESSION['user_id']);
            break;
        case 'item_ln_up': $sw_break = false;
            MTNavigation($dbh, 'lnup', $_SESSION['db_info']['t_main'], $_SESSION['p_count'], $_SESSION['p_start'], $_SESSION['item_arr'], $_SESSION['p_code'], (int) $_SESSION['user_id']);
            break;
        case 'item_ln_dn': $sw_break = false;
            MTNavigation($dbh, 'lndn', $_SESSION['db_info']['t_main'], $_SESSION['p_count'], $_SESSION['p_start'], $_SESSION['item_arr'], $_SESSION['p_code'], (int) $_SESSION['user_id']);
            break;
        case 'item_pg_dn': $sw_break = false;
            MTNavigation($dbh, 'pgdn', $_SESSION['db_info']['t_main'], $_SESSION['p_count'], $_SESSION['p_start'], $_SESSION['item_arr'], $_SESSION['p_code'], (int) $_SESSION['user_id']);
            break;
        case 'item_end': $sw_break = false;
            MTNavigation($dbh, 'end', $_SESSION['db_info']['t_main'], $_SESSION['p_count'], $_SESSION['p_start'], $_SESSION['item_arr'], $_SESSION['p_code'], (int) $_SESSION['user_id']);
            break;
        case 'view_item': $sw_break = false;
            ViewItem($dbh, $_SESSION['URL_p'], $_SESSION['f_view'], $_SESSION['p_code'], (int) $arr_key[1], $_SESSION['p_files']['e'], (int) $_SESSION['user_id'], $_SESSION['PR']['const']);
            break;
        case 'sel_lang': $sw_break = false;
            SetLanguage($dbh, $_POST['view_language'], $_SESSION['langs'], (int) $_SESSION['user_id'], $_SESSION['cur_lang'], $_SESSION['el_titles']);
            break;
        case 't_flag': $sw_break = false;
            $suspend = TestSessionSuspend($dbh_sys, $_SESSION['db_info']['id'], $_SESSION['user_id']);
            break;
        case 'end_flag': $sw_break = false;
            ExitSession(Title(1).' <b>'.$_SESSION['db_info']['name'].'</b>`'.implode('`', $Mes).'|FF0000', $_SESSION['db_info']['id']);

            ExitFromViewMode($dbh, $_SESSION['user_id']);
            break;
        default: $sw_break = false;
    }
    $sw_cat_break = ListSettingActions($dbh, $arr_key, $_SESSION, $_SESSION['mes']['m'], (int) $_SESSION['user_id']);
    if ($sw_cat_break) {
        break;
    }
}
...
<body onLoad="start_prompt_exit()">
	...
	<form method="post" id="list_form" name="list_form">
		...
		<select name="view_language"> <!-- // SelectTag -->
			<?php foreach ($_SESSION['langs'] as $v) {
			    echo OptionTag($_SESSION['cur_lang'][1], $v);
			}?>
		</select>
...
if ($_SESSION['set_pad']) {
    ListSettingsPad((int) $_SESSION['user_id']);
}
if ($_SESSION['hide_list']) {
    if (! $_SESSION['set_pad']) {
        require_once 'ItemPortion.php';
    }
    ViewMessages(false, '', $_SESSION['number_warn'], [], [], $_SESSION['block'], false);
} else {
    ViewMessages(false, '', $_SESSION['number_warn'], [], [], $_SESSION['block'], false);
    require_once 'ItemPortion.php';
}
