...
function GetFileNum($p_files, $f_n)
{
    if ($f_n == '') {
        $max_num = 0;
        foreach (array_keys($p_files) as $k) {
            if ((int) $k > $max_num) {
                $max_num = (int) $k;
            }
        }
...
function UpdateOneItemFile($dbh, $p_con, $item_field_key, $item_id, $k, $v, $vi, $attach, $file_field_key, &$Mes)
{
    ...
    foreach ($v as $k0 => $v0) {
        if ($v0 != $vi[$k0] && $k0 != $_SESSION['spec_fld'][2]['URL_link']) { // ~~url
            ...
            if ($v0['f_type'] == 'date') {
                $arr_query[] = ($v0 == '') ? $k0.' = 0' : $k0.' = '.(string) $v0;
            } else {
                $arr_query[] = $k0." = '".$v0."'";
            }
            if ($v0['f_lower']) {
                $arr_query[] = $k0."_low = '".MCV($v0, false)."'";
            }
        }
    }
...
function InsertOneItemFile($dbh, $p_con, $item_id, $k, $v, &$Mes)
{
    ...
    foreach ($p_con as $k0 => $v0) {
        if ($v0['own_table'] == 0 && $v0['f_type'] != $_SESSION['spec_fld'][2]['URL_link']) {
            if ($v0['f_type'] == 'date') {
                $arr_fields[] = ($v0 == '') ? $k0.' = 0' : $k0.' = '.(string) $v0;
            } else {
                $arr_fields[] = $v['f_ref'];
            }
            if ($v['f_key']) {
            ...
            if ($v['f_typr'] == 'integer') {
                $_SESSION['p_files']['e'][$_SESSION['load_file_number']][$k] = 0;
            } else {
                $_SESSION['p_files']['e'][$_SESSION['load_file_number']][$k] = '';
            }
        }
    }
    /*
...
function ChangeItemFiles($dbh, $item_id, &$p_files, $URL_path, &$Mes)
{
    ...
    foreach ($p_files['e'] as $k => $v) {
        if (array_key_exists($k, $p_files['i'])) {
            UpdateOneItemFile($dbh, $item_id, $k, $v, $p_files['i'][$k], $Mes);
        } elseif (! GetItemFile($dbh, $item_id, $k)) {
            InsertOneItemFile($dbh, $item_id, $k, $v, $Mes);
        }
    }
    foreach ($p_files['i'] as $k => $v) {
        if (! array_key_exists($k, $p_files['e'])) {
            DeleteOneItemFile($dbh, $item_id, $k, $v, $v['URL_p'], $Mes);
        }
    }
...
function SetFileRows(&$p_files)
{
    foreach ($_POST as $str_key => $str_v) {
        ...
        if (count($arr_key) > 1 && substr($arr_key[0], 0, 5) == 'file_' && $p_files[$arr_key[1]][$arr_key[0]] != $str_v) {
            $p_files[$arr_key[1]][$arr_key[0]] = $str_v;
        }
    }
}
        ...
        if ($dir) {
            while (($file = readdir($dir)) !== false) {
                if ($file != '.' && $file != '..') {
                    ...
                    $arr_file = explode('-', $ff);
                    if (count($arr_file) == 2 && $arr_file[0] == (string) $p_code && ! array_key_exists($arr_file[1], $p_files)) {
                        unlink($dirname.'/'.$file);
                    }
                }
            }
...
{
    foreach ($p_files as $f_num => $f_inf) {
        if ($f_inf['file_name'] == $file_name) {
            return $f_num;
        }
    }
...
{
    if (empty($s['URL_p'])) {
        $Mes[] = ['time' => '', 'text' => 'Destination directory is not specified', 'status' => 'error'];
    } else {
        $s['file_update'] = ($file_number != '');
        if ($file_number == '') {
            $s['load_file_number'] = GetFileNum($s['p_files']['e'], $file_number);
        } else {
            $s['load_file_number'] = $file_number;
        }
        if ($s['p_code'] > 0) {
            return true;
        }
