...
function TestItemFilling($dbh, $p_code, $p_con, $item_row, &$Mes)
{
	...
	foreach (array_keys($item_row) as $k)
	{
		if (strpos($k, "_code") === false && $k != "del_mark")
		{
			...
			if ($v == "" && !$p_con[$k]['blank']) TestBlank($p_con[$k]['name'], $v, $p_con[$k]['type'], $Mes);
			if ($on_mes == count($Mes) && $p_con[$k]['table'] == "") 
			{
				$illegal_mes = SetTestMessages(StringTest($p_con[$k]['illegals'], $v, "replace_text"), $v);
				if ($illegal_mes != "") $Mes[] = array("time"=>"", "text"=>$illegal_mes, "status"=>"error", );
			}
			if ($on_mes == count($Mes) && $p_con[$k]['type'] != "")
			{
				if ($p_con[$k]['type'] == "integer" && $v != "" && !is_numeric($v)) $Mes[] = array("time"=>"", "text"=>"The <b>".$p_con[$k]['name']."</b> of item is not numeric", "status"=>"error");
				if ($p_con[$k]['type'] == "date")
				{
					$date_err = CalendTestDate($v);
					if (count($date_err) > 0) $Mes[] = array("time"=>"", "text"=>"The <b>".$p_con[$k]['name']."</b> of item: ".implode("; ", $date_err), "status"=>"error");
				}
			}
	...
	if (!$f_unique) TestUnique($dbh, $p_con, $p_code, $item_row, $Mes);
...
function InsertItem($dbh, &$p_code, $p_con, $item_row, &$Mes)
{
	...
	if ($q_txt != "")
	{
		...
		else 
		{
			...
			$p_code = ($item_id == "") ? 0 : (integer)$item_id;
		}
	}
	return (count($Mes) == $c_mes);
}
...
function SaveItem($dbh, &$s)
{
	...
	{
		if ($s['p_code'] == 0) $no_err = InsertItem($dbh, $s['p_code'], $s['PR']['const'], $s['item_row'], $s['mes']['m']);
		else $no_err = UpdateItem($dbh, $s['p_code'], $s['PR']['const'], $s['item_row'], $s['mes']['m']);
		if ($no_err)
		{
...
{
	if ($v['table'] != "") return $item_row[$k."_code"];
	elseif ($v['type'] != "" && $$v['type'] == "integer") return $item_row[$k];
	return "'".$item_row[$k]."'";
}
function TestUnique($dbh, $main_table, $p_con, $p_code, $item_row, &$Mes, $unique_field = "", $unique_name = "", $unique_value = "")
{
	...
	{
		$arr_compar = array();
		foreach ($p_con as $k => $v) if (!$v['key'] && $v['field_check']) $arr_compar[] = $v['ref']." = ".TestValue($k, $v, $item_row);
	...
	else $res = mysqli_query($dbh, "SELECT ".$_SESSION['spec_fld'][1]['key']." FROM ".$main_table." WHERE ".$unique_field." = '".$unique_value."' AND ".$_SESSION['spec_fld'][1]['key']." <> ".(string)$p_code);
	if ($res)
	{
		$arr_id = array();
		...
		{
			$s = (count($arr_id) == 1) ? "" : "s";
			if ($unique_field == "") $Mes[] = array("time"=>"", "text"=>"The same data have follow item ID".$s.": <b>".implode(", ", $arr_id)."</b>", "status"=>"error");
			else $Mes[] = array("time"=>"", "text"=>"The same <b>".$unique_name."</b> have follow item ID".$s.": <b>".implode(", ", $arr_id)."</b>", "status"=>"error");
		}
	}
}
function TestBlank($test_name, $test_value, $ref_type, &$Mes)
{
	if ($test_value == "") $Mes[] = array("time"=>"", "text"=>"The <b>".$test_name."</b> is empty", "status"=>"error");
	else
	{
		if ($ref_type == "integer" && $test_value == "0") $Mes[] = array("time"=>"", "text"=>"The <b>".$test_name."</b> is empty", "status"=>"error");
		elseif ($ref_type == "date" && is_null($test_value)) $Mes[] = array("time"=>"", "text"=>"The <b>".$test_name."</b> is empty", "status"=>"error");
	}
}
function CopyItemRowFiles(&$item_row, &$p_files)
{
	foreach ($item_row['e'] as $k => $v) $item_row['i'][$k] = $v;
	foreach ($p_files['e'] as $k => $v) foreach ($v as $k0) $p_files['i'][$k][$k0] = $p_files['e'][$k][$k0];
/*
	{
...
{
	foreach ($p_con as $k => $v) if (isset($_POST[$k]) && $v['table'] == "") $item_row[$k] = $_POST[$k];
	foreach ($p_files as $k => $v)
	{
		if (isset($_POST['file_issue_year-'.$k])) $p_files[$k]['file_issue_year'] = (($_POST['file_issue_year-'.$k] == "") ? 0 : $_POST['file_issue_year-'.$k]);
		if (isset($_POST['file_volume-'.$k])) $p_files[$k]['file_volume'] = $_POST['file_volume-'.$k];
		if (isset($_POST['file_number-'.$k])) $p_files[$k]['file_number'] = $_POST['file_number-'.$k];
		if (isset($_POST['file_page-'.$k])) $p_files[$k]['file_page'] = $_POST['file_page-'.$k];
		if (isset($_POST['file_description-'.$k])) $p_files[$k]['file_description'] = $_POST['file_description-'.$k];
	}
}
function ItemFormCompare($item_row, $p_files)
{
	foreach ($item_row['e'] as $k => $v) if ($item_row['i'][$k] != $v) return false;
	if (count($p_files['e']) != count($p_files['i'])) return false;
	foreach ($p_files['e'] as $k => $v) foreach ($v as $k0 => $v0) if ($p_files['i'][$k][$k0] != $v0) return false;
	return true;
}
function MaxSize($p_con)
{
	$max_size = 0;
	foreach ($p_con as $k => $v) if (array_key_exists($k, $v) && $v['screen_order'] > 0 && strpos($v['using'], "update") !== false && $v['table'] != "" && $v['size'] > $max_size) $max_size = $v['size'];
	return $max_size;
}
function ItemHTMLForm($p_con, $main_table, $item_row, $block, $init_v, $end_v) //!!!!!!! $init_v, $end_v
{
    $arr = array("");
    for ($i = $init_v; $i <= $end_v; $i++) $arr[] = $i;
...
//							echo "</select>";
					...
					else
					{
						...
						{
							if ($_SESSION['spec_fld'][1]['key'] == "") echo "<td></td>";
						...
						else echo "<td><input size='".$v['size']."' name='".$k."' readonly type='text' style='style='text-align:".$sel_align[$v['f_align']]."' value='".$item_row[$k]."' ".FormExPar($block)."></td>";
					}
				}
				else
				{
