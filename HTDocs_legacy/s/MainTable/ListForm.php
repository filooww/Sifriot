...
{
	if (!array_key_exists($k_v, $_SESSION['main_params']['const'][1])) return false;
	if ($_SESSION['main_params']['const'][1][$k_v]['screen_order'] == 0) return false;
	if ($URL_path == "" && strpos($_SESSION['main_params']['const'][1][$k_v]['using'], "update") !== false) return true;
	if ($URL_path != "" && strpos($_SESSION['main_params']['const'][1][$k_v]['using'], "view") !== false) return true;
	return false;
}
function ViewMTPortion($main_table, $p_files, $item_mark_del_question, $URL_path = "", $user_priority = 0, $view_on = false)
{
	...
	foreach ($_SESSION['item_arr'] as $k => $v)
	{
		...
		if ($k == (string)$_SESSION['p_code']) $bgcl = "class='cur_row'";
		...
		echo "</tr>";
		$cl = !$cl;
		if ($URL_path != "" && $user_priority > 0 && $k == (string)$_SESSION['p_code'] && $view_on) ViewPubFiles($p_files, $URL_path);
		if ($URL_path == "" && $k == (string)$_SESSION['p_code'] && $item_mark_del_question) ViewMarkQuestion($v['del_mark'][0], $n_max, count($v));
	}
	for ($i = count($_SESSION['item_arr']); $i < $_SESSION['portion']; $i++) 
	{
		echo "<tr>";
			if ($URL_path == "") echo "<td></td><td></td><td></td>";
...
{
	if ($mark_del_flag) return "MarkDelete";
	else return "Blank";
}
function PutValue($col_value, $p_con_k)
{
	...
	else $col_v = $col_value;
	if ($p_con_k['type'] == "integer" && $col_v == "0") return "";
	else return $col_v;
}
...
// p_con!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	foreach ($p_files as $k => $v)
	{
		echo "<tr valign='top'  class='file_bgc'>";
			...
			echo "<td></td>";
			//~~url
			if ($v['url_file'] == "") echo "<td width='30%'><font color='#FF0000'><< ".Title(72)." >></font></td>";
			elseif (!ResExists($v['url_file'])) echo "<td width='50%'><font color='#FF0000'><< ".Title(72)." >></font></td>";
			else echo "<td width='30%'><a name='file_name-".$k."' href='".RefToRes($v['url_file'], $conf_root.'/')."' target='_blank'>".$v['file_name']."</a></td>"; //~~url
...
function ViewMarkQuestion($p_mark, $n_max, $count_v)
{
	...
	echo "<tr>";
		...
		echo "</td>";
		for ($i = 0; $i < $count_v - $n_max - 1; $i++) echo "<td></td>";
	echo "</tr>";
}
