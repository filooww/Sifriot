		...
		<td>
			<table>			
				<tr valign="top">
					<?php
                    if ($_SESSION['pri'] > 0) {
                        echo "<th width='2%' class='header_button'>X</th>";
                    }
foreach ($_SESSION['PR']['con'] as $k => $v) {
    if ($v['screen_order'] > 0 && strpos($v['using'], 'view') !== false) {
        echo "<th width='".$v['t_prc']."' class='header_text' align='left'><b>".$v['name'].'</b></th>';
    }
}
?>
				</tr>
