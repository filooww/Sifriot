...
function RewriteLocalCodes($dbh, &$Mes)
{
    ...
    if ($fl) {
        $Mes[] = "<font color='#0000FF'><b>".Title(199).'</b></font>';
    }
...
function TestSymbolUnique($dbh, &$arr_err)
{
    ...
    if ($res) {
        ...
        if ($fl[1] != '') {
            $arr_err[] = "<font color='#FF0000'><b>".$_POST['new_symbol'].'</b> - '.Title(579).' '.Title(587).' '.FTM(Title(231)).' <b>'.$fl[1].'</b> ('.Title(614).' <b>'.$fl[0].'</b>)</font>';
        }
    }
}
function AddNewSymbol($dbh)
{
    ...
    if (count($arr_err) == 0) {
        $arr_sym = [ord(substr($_POST['new_symbol'], 0)), ord(substr($_POST['new_symbol'], 1)), ord(substr($_POST['new_symbol'], 2))];
        $_SESSION['trans_codes'][$_POST['new_symbol']] = ['lang_code' => (int) $_SESSION['sel_local_lang'][0], 'b_0' => $arr_sym[0], 'b_1' => $arr_sym[1], 'b_2' => $arr_sym[2], 'translit_set' => '', 'new' => true, 'del' => false, 'err' => $arr_err];
        ksort($_SESSION['trans_codes']);
        $_SESSION['change_lang_letter'] = [$_POST['new_symbol'], false];
        $_SESSION['new_symb_param'][1] = "<font color='#0000FF'>".FTM(Title(277)).' '.Title(586).'</font>';
    } else {
        $_SESSION['new_symb_param'][1] = implode('; ', $arr_err);
    }
}
function TestNewSymbol($dbh)
{
    ...
    if ($_POST['new_symbol'] == '') {
        ...
        if (strpos($_POST['new_symbol'], chr(39)) !== false) {
            $arr_err[] = "<font color='#FF0000'>".FTM(Title(277)).' '.Title(572).'</font>';
        }
        if (mb_strlen($_POST['new_symbol'], 'utf-8') > 1) {
            $arr_err[] = "<font color='#FF0000'>".Title(578).'</font>';
        } elseif (ord($_POST['new_symbol']) > 31 && ord($_POST['new_symbol']) < 127) {
            $arr_err[] = "<font color='#FF0000'>".Title(671).'</font>';
        } else {
            TestSymbolUnique($dbh, $arr_err);
        }
    }
...
{
    $arr_err = [];
    $_SESSION['trans_codes'][$k]['err'] = [];
    if ($k == '') {
        $arr_err[] = "<font color='#FF0000'>".FTM(Title(277)).' '.Title(573).'</font>';
    } else {
        if (strpos($k, chr(39)) !== false) {
            $arr_err[] = "<font color='#FF0000'>".FTM(Title(277)).' '.Title(572).'</font>';
        }
        if (mb_strlen($k, 'utf-8') > 1) {
            $arr_err[] = "<font color='#FF0000'>".Title(578).'</font>';
        } elseif (ord($k) > 31 && ord($k) < 127) {
            $arr_err[] = "<font color='#FF0000'>".Title(671).'</font>';
        }
    }
    if (isset($_SESSION['local_langs'][$v['lang_code']])) {
        if ($v['lang_code'] > 1 && $v['translit_set'] == '') {
            $arr_err[] = "<font color='#FF0000'>".FTM(Title(285)).' '.Title(573).'</font>';
        }
        if ($v['lang_code'] == 0 && $v['translit_set'] != '') {
            $arr_err[] = "<font color='#FF0000'>".Title(659).'</font>';
        }
    }
    if (! isset($_SESSION['local_langs'][$v['lang_code']])) {
        $arr_err[] = "<font color='#FF0000'>".FTM(Title(602)).' '.Title(542).' (<b>'.(string) $v['lang_code'].'</b>)</font>';
    }
    if (gettype($_SESSION['sel_local_lang'][0]) == 'string' && ! isset($_SESSION['coding_list'][$v['id_code']])) {
        $arr_err[] = "<font color='#FF0000'>".Title(331).' (<b>'.(string) $v['id_code'].'</b>)</font>';
    }
    if (strpos($v['translit_set'], chr(39)) !== false) {
        $arr_err[] = "<font color='#FF0000'>".FTM(Title(285)).' '.Title(572).'</font>';
    }
    if (TransliteratonSetError($v['translit_set'])) {
...
function AfterChoiceCoding(&$Mes, &$sw_break)
{
    ...
    $_SESSION['sel_coding'] = [$new_coding, $_POST['sel_coding']];
...
function TestAllLocalCodes(&$Mes, $mes_del = false)
{
    ...
    $arr_del = [];
    foreach ($_SESSION['trans_codes'] as $k => $v) {
        if (in_array($k, $_SESSION['del_local']) !== false) {
            $arr_del[] = "<font class='del_symb'><b>".$k.'</b></font>';
        } else {
    ...
    if ($mes_del && count($arr_del) > 0) {
        $Mes[] = MesDelTitles($arr_del);
    }
...
{
    $invalid_letters = [];
    foreach ($_SESSION['pre_ref']['translate_table']['p'] as $type_arr) {
        foreach ($type_arr['v'] as $letters) {
...
function MoveSymbol($dbh, $letter, &$Mes)
{
    if (gettype($_SESSION['sel_local_lang'][0]) == 'string') {
        $old_coding_id = $_SESSION['trans_codes'][$letter]['id_code'];
    } else {
        $old_coding_id = $_SESSION['sel_coding'][0];
    }
    if (gettype($_SESSION['sel_local_lang'][0]) == 'string') {
        $new_coding_id = array_search($_POST['change_local_coding|'.$letter], $_SESSION['coding_list']);
    } else {
        $new_coding_id = $_SESSION['sel_coding'][0];
    }
    ...
    if ($old_coding_id == $new_coding_id && $old_lang_id == $new_lang_id) {
        $Mes[] = "<font color='#0000FF'>".Title(244).'</font>';
    } else {
        ...
        if ($_SESSION['pre_ref']['translate_table']['over']) {
            ...
            if (isset($_SESSION['pre_ref']['translate_table']['p']['cd']['v'][(string) $old_coding_id])) {
                ChangeInvalidRefTable($dbh, 'translate_table', 'cd', $old_coding_id, $letter);
            }
            if (isset($_SESSION['pre_ref']['translate_table']['p']['li']['v'][(string) $old_lang_id])) {
                ChangeInvalidRefTable($dbh, 'translate_table', 'li', $old_lang_id, $letter);
            }
        }
        $_SESSION['new_symb_param'] = ['', ''];
        if (isset($_SESSION['trans_codes'][$letter]['del']) && $_SESSION['trans_codes'][$letter]['del']) {
            $_SESSION['trans_codes'][$letter]['del'] = false;
        }
        $Mes[] = "<font color='#0000FF'>".Title(277)."</font> <font class='emph_data'><b>".$letter."</b></font> <font color='#0000FF'>".Title(586).'</font>';
        $_SESSION['sel_coding'] = [$new_coding_id, $_SESSION['coding_list'][$new_coding_id]];
        $_SESSION['sel_local_lang'] = [$new_lang_id, $_SESSION['local_langs'][$new_lang_id]];
        ...
        $_SESSION['change_lang_letter'] = [$letter, false];
        TestAllLocalCodes($Mes);
    }
}
function AfterChoiceLocalLang($dbh, &$sw_break)
{
    if (AfterLangChoice($dbh, 'user_lang_s', 'user_lang', $sw_break)) {
        if (PermitInvRef('translate_table', ['cd', 'li'])) {
            $_SESSION['local_langs_for_page']['nu'] = '-- '.FTM(Title(602)).' '.Title(542).' '.Title(82).'/'.Title(567).' '.Title(281).' --';
        }
...
function DeleteMarkedLetters($dbh)
{
    ...
    foreach ($_SESSION['del_local'] as $k) {
        if (gettype($_SESSION['sel_local_lang'][0]) == 'string') {
            $coding_id = $_SESSION['trans_codes'][$k]['id_code'];
        } else {
            $coding_id = $_SESSION['sel_coding'][0];
        }
        $arr_del[$k] = [$coding_id, $_SESSION['trans_codes'][$k]['lang_code']];
    }
    ...
    foreach ($arr_del as $k => $v) {
        if ($_SESSION['pre_ref']['translate_table']['over']) {
            ...
            if (isset($_SESSION['pre_ref']['translate_table']['p']['cd']['v'][(string) $v[0]])) {
                ChangeInvalidRefTable($dbh, 'translate_table', 'cd', $v[0], $k);
            }
            if (isset($_SESSION['pre_ref']['translate_table']['p']['li']['v'][(string) $v[1]])) {
                ChangeInvalidRefTable($dbh, 'translate_table', 'li', $v[1], $k);
            }
        }
