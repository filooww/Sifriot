...
if (count($_POST) == 0) {
    ...
    $_SESSION['sel_coding'] = [GetFirstKey($_SESSION['coding_list']), $_SESSION['coding_list'][key($_SESSION['coding_list'])]];
    $_SESSION['sel_coding_for_change'] = $_SESSION['sel_coding'][0];
    $_SESSION['local_langs'] = SetLanguageList(1);
    $_SESSION['sel_local_lang'] = [0, $_SESSION['local_langs'][0]];
...
foreach ($_POST as $str_key => $str_v) {
    ...
    switch ($s_k) {
        ...
        case 'sel_coding_s': if (AfterChoiceCoding($dbh, $sw_break)) {
            SetLocalLanguage($dbh, $Mes);
        } break;
        case 'user_lang_s': if (AfterChoiceLocalLang($dbh, $sw_break)) {
            TestAllLocalCodes($Mes);
        } break;
        case 'local_lang_s': if (AfterLocalLangChoice($Mes, $sw_break)) {
            SetLocalLanguage($dbh, $Mes);
        } break;
        case 'replace_s': if (AfterReplaceSelect(['cd' => $_SESSION['coding_list'], 'li' => $_SESSION['local_langs']], $sw_break)) {
            TestAllLocalCodes($Mes);
        } break;
        case 'change_local_ids': MoveSymbol($dbh, $k[1], $Mes);
            $_SESSION['inv_ref'] = false;
            break;
        case 'local_exit': $goto = ExitLocals($dbh, $Mes);
            if ($goto != '') {
                header('Location: '.$goto.'.php');
            } break;
        case 'local_save': if (count($_SESSION['del_local']) > 0) {
            $_SESSION['local_delete'] = true;
        } PostToTransliterations();
            TestAllLocalCodes($Mes);
            SetLocalLanguage($dbh, $Mes);
            break;
        case 'local_mark_delete': MarkLocalDelete($k[1]);
            TestAllLocalCodes($Mes, true);
            break;
        case 'symbol_del': DeleteMarkedLetters($dbh);
            $_SESSION['del_local'] = [];
            $_SESSION['local_delete'] = false;
            TestAllLocalCodes($Mes);
            break;
        case 'symbol_no_del': $_SESSION['del_local'] = [];
            $_SESSION['local_delete'] = false;
            break;
        case 'add_new_symbol': AddNewSymbol($dbh);
            break;
        case 'local_references': $_SESSION['new_symb_param'] = ['', ''];
            GetInvRef('translate_table', ['cd' => $_SESSION['coding_list'], 'li' => $_SESSION['local_langs_for_page']]);
            TestAllLocalCodes($Mes);
            break;
        case 'replace_id': ReplaceLocalIDs($dbh, $k[1], $k[2]);
            break;
...
?>
	...
	<?php if ($_SESSION['inv_ref'] && PermitInvRef('translate_table', ['cd', 'li'])) {
	    InvRefTable('translate_table', 'emph_data', ['cd' => $_SESSION['coding_list'], 'li' => $_SESSION['local_langs']]);
	} ?>
	<table>
		<tr>
...
?>
    ...
    <?php
    if ($_SESSION['local_delete']) {
        DBMDelQuestion('modal_form', ['symbol_del', 'symbol_no_del'], LocalSetQuestionString(), 208, 277, 584, "(<font color='#FF0000'><b>".Title(132).'</b></font>)');
    }
if (count($Mes) > 0) {
    foreach ($Mes as $z) {
        echo '<br>'.$z;
...
if ($_SESSION['user_working_mode'] == 1 && ! $_SESSION['local_delete']) {
    require_once 'LocalNewRow.php';
}
if (count($_SESSION['trans_codes']) == 0) {
    echo '<div><center><b><font size=+1>'.Title(76).'</b></font></center></div>';
} else {
    require_once 'LocalTable.php';
}
?>
