...
if (! $dbh) {
    ExitSession(Title(252).'|FF0000', $_SESSION['login']['id']);
}
if (count($_POST) == 0) {
    $_SESSION['lang_param'] = ['0' => '', '1' => 'English', '2' => 'Русский', '3' => 'עברית'];
    $_SESSION['title_lang'] = ['0', ''];
    $_SESSION['title_id'] = '';
    $_SESSION['title_text'] = '';
    ...
    $_SESSION['title_lang'] = [(string) $ll, $_POST['title_lang']];
...
foreach ($_SESSION['lang_param'] as $v) {
    echo '<option'.(($v == $_SESSION['title_lang'][1]) ? ' selected' : '').'>'.$v.'</option>';
}
echo '</select>';
?>
			</td>
...
<?php
if ($_SESSION['title_id'] == '' && $_SESSION['title_lang'][0] == '0' && $_SESSION['title_text'] == '') {
    $w = '';
} elseif ($_SESSION['title_id'] == '' && $_SESSION['title_lang'][0] == '0' && $_SESSION['title_text'] != '') {
    $w = "title_text LIKE '%".$_SESSION['title_text']."%'";
} elseif ($_SESSION['title_id'] == '' && $_SESSION['title_lang'][0] != '0' && $_SESSION['title_text'] == '') {
    $w = 'id_language = '.$_SESSION['title_lang'][0];
} elseif ($_SESSION['title_id'] == '' && $_SESSION['title_lang'][0] != '0' && $_SESSION['title_text'] != '') {
    $w = 'id_language = '.$_SESSION['title_lang'][0]." AND title_text LIKE '%".$_SESSION['title_text']."%'";
} elseif ($_SESSION['title_id'] != '' && $_SESSION['title_lang'][0] == '0' && $_SESSION['title_text'] == '') {
    $w = 'id_title = '.$_SESSION['title_id'];
} elseif ($_SESSION['title_id'] != '' && $_SESSION['title_lang'][0] == '0' && $_SESSION['title_text'] != '') {
    $w = 'id_title = '.$_SESSION['title_id']." AND title_text LIKE '%".$_SESSION['title_text']."%'";
} elseif ($_SESSION['title_id'] != '' && $_SESSION['title_lang'][0] != '0' && $_SESSION['title_text'] == '') {
    $w = 'id_title = '.$_SESSION['title_id'].' AND id_language = '.$_SESSION['title_lang'][0];
} else {
    $w = 'id_title = '.$_SESSION['title_id'].' AND id_language = '.$_SESSION['title_lang'][0]." AND title_text LIKE '%".$_SESSION['title_text']."%'";
}
