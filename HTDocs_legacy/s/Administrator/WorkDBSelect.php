...
foreach ($_POST as $str_key => $str_v) {
    ...
    switch ($arr_key[0]) {
        case 'user_lang_s': AfterLangChoice($dbh_sys, 'user_lang_s', 'user_lang', $sw_break);
            break;
        case 'work_db_select_exit': DBSelectExit();
            break;
        case 'db_list': $goto = GoToPrefferedDB($dbh_sys, $arr_key[1], $Mes);
            if ($goto != '') {
...
if (count($_SESSION['db_pre_flags']['no_existed_tables']) > 0) {
    echo "<div align='center'><h3><b>".Title(632).' '.Title(613).' '.Title(631).'.</b></h3></div>';
}
if (count($Mes) > 0) {
    foreach ($Mes as $z) {
        echo '<br><b>'.$z.'</b>';
