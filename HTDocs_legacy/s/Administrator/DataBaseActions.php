...
foreach ($_POST as $str_key => $str_v) {
    ...
    switch ($str_key) {
        case 'user_lang_s': if (AfterLangChoice($dbh_sys, 'user_lang_s', 'user_lang', $sw_break)) {
            TestUserDB($_SESSION['db_info']['name'], $_SESSION['db_info']['coding'], $_SESSION['db_info']['id']);
        } break;
        case 'w_mode_s': SwitchWorkingMode($dbh_sys, $_SESSION['db_info']['id'], (int) $_POST['w_mode_s'], $sw_break);
            break;
        case 'db_action_exit': DataBaseActionExit($dbh_sys);
            header('Location: ../Administrator/MainForm.php');
            break;
        case 'db_action_configs': $_SESSION['common_config'] = false;
            DBConfiguration($dbh_sys);
            header('Location: ../Configuration/ConfigForm.php');
            break;
        case 'db_action_tables': header('Location: ../Tables/TableForm.php');
            break;
        case 'db_action_fields': header('Location: '.GoToFields());
            break;
        case 'db_action_catalogs': $_SESSION['catalog_list'] = GetCatalogList($dbh_sys);
            header('Location: ../Catalogs/CatalogListForm.php');
            break;
        case 'db_action_primary': header('Location: ../Primary/PrimaryMain.php');
            break;
        case 'db_action_update': $_SESSION['p_start'] = 0;
            header('Location: ../MainTable/List.php');
            break;
        case 'db_action_delete': header('Location: ../DeleteMarked/List.php');
            break;
...
<form method="post" id="DB_actions" name="DB_actions">
	<table width="100%">
		<tr>
			...
			<td width="20%" valign="middle"><?php RadioTag('user_working_mode', $_SESSION['user_working_mode'], [Title(154), Title(155)], 'hidden_button', false, 'working_mode_on', 'w_mode_s'); ?></td>
			<td align="right"><button name="db_action_exit" type="submit" value="*" class="exit_button"><?php echo Title(8); ?></button></td>
		</tr>
	</table>
    <?php if (isset($_SESSION['db_errors']) && count($_SESSION['db_errors']) > 0) {
        ViewDBErrors();
    } ?>
