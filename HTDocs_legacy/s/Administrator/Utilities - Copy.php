...
{
    if (count($_SESSION['no_title']) > 0) FillNoTitles();
...
function GoToUserDB($dbh, $work_db)
{
    ...
    $_SESSION['table_definitions'] = GetTableDefinitions($dbh, $Mes);
    $t_flags = DBTesting($dbh);
    if ($t_flags['empty'] || IsTablesErrors() || count($t_flags['errors']) + count($_SESSION['field_definitions']) + count($_SESSION['db_pre_flags']['table_errors']) + count($_SESSION['no_table']['no_definition']) + count($_SESSION['no_table']['no_DB']) > 0) ExitSession(Title(632)."|FF0000`[".$_SESSION['arr_db'][$work_db]['db_name']."]|0000FF`".Title(613), $work_db);
    if (count($Mes) == 0)
    {
        $_SESSION['db_info'] = DBInfoSelect($work_db);
        if ($_SESSION['priority'] < 11) return "../MainTable/List";
    }
...
function GoToDB($dbh_sys, $work_db, &$Mes)
{
    ...
    $_SESSION['db_pre_flags'] = array("table_errors"=>array(), "no_existed_tables"=>array());
	TestDataBaseTablesExist($dbh);
    if (count($_SESSION['db_pre_flags']['no_existed_tables']) > 0)
    {
        if ($_SESSION['priority'] < 11) ExitSession(Title(632)."|FF0000`[".$_SESSION['arr_db'][$work_db]['db_name']."]|0000FF`".Title(613).((count($_SESSION['arr_db']) == 2) ? "" : " ".Title(631)), $work_db);
        else $dbh = RestoreDBHandler($work_db);
    }

        if ($_SESSION['priority'] > 10) VisitParameters($dbh_sys, 0, -1, $_SESSION['priority']);
    ...
    {
        if ($_SESSION['priority'] < 11) ExitSession(Title(632)."|FF0000`[".$_SESSION['arr_db'][$work_db]['db_name']."]|0000FF`".Title(613), $work_db);
        $_SESSION['user_structure_form'] = $work_db;
        return "../Alarm/UserStructureForm";
    }
    $_SESSION['db_info'] = DBInfoSelect($_SESSION['arr_db'][$work_db]);
    if ($_SESSION['priority'] < 11) return GoToUserDB($dbh, $work_db);
