...
foreach ($_POST as $str_key => $str_v)
{
	...
	switch ($arr_key[0])
	{
		...
		case "w_mode_s"				: SwitchWorkingMode($dbh_sys, 0, (integer)$_POST['w_mode_s'], $sw_break); break;
		case "DBM_exit"				: ExitSession(); break;
		case "coding_table"		    : $_SESSION['language_messages'] = array(); header("Location: ../Codings/CodingForm.php"); break;
		case "common_config"		: $_SESSION['common_config'] = true; $_SESSION['config_list'] = GetAllConfigs($dbh_sys, "db_s_configs"); header("Location: ../Configuration/ConfigForm.php"); break;
		case "user_list"			: header("Location: ../UserList/UserListForm.php"); break;
		case "visit_list"			: header("Location: ../Visits/VisitForm.php"); break;
		case "languages"			: $_SESSION['language_messages'] = array(); header("Location: ../Languages/LanguageForm.php"); break;
		case "local_languages_form"	: header("Location: ../LocalLanguages/LocalForm.php"); break;
		case "data_bases_list"		: header("Location: ../DataBases/DataBasesForm.php"); break;
		case "interface_texts"		: GoToTitles($dbh_sys); header("Location: ../Titles/TitleForm.php"); break;
		case "algorithms"	        : header("Location: ../Algorithms/AlgorithmForm.php"); break;
		case "db_list"				: $goto = GoToDB($dbh_sys, (integer)$arr_key[1]); header("Location: ".$goto.".php"); break;
...
<form method="post" id="main_form" name="main_form">
	<table width="100%">
		<tr>
            ...
            if ($_SESSION['priority'] > 10)
            {
                ...
                RadioTag("user_working_mode", $_SESSION['user_working_mode'], array(Title(154), Title(155)), "hidden_button", false, "working_mode_on", "w_mode_s");
                echo "</td>";
            }
            else echo "<td width='20%' valign='middle'></td>";
            ?>
	...
	{
	    echo "<table>";
	    foreach ($Mes as $z) echo "<tr valign='top'><td>".$z."</td></tr>";
        echo "</table>";
	}
	?>
	...
	<table width="100%">
		<tr valign="top">
			...
			{
				echo "<td width='50%' align='right'>"; require_once("MainFormSuper.php"); echo "</td>";
				echo "<td width='10%'><font class='invisible_row'>X</font></td>";
				echo "<td width='40%'>"; require_once("DBList.php"); echo "</td>";
			}
			else {echo "<td width='100%' align='center'>"; require_once("DBList.php"); echo "</td>";}
			?>
		</tr>
	</table>
<form>
