...
function TestSpecialType($special_type, $save)
{
	...
	for ($i = 0; $i < count($_SESSION['special_interface'][$special_type]['numbers']); $i++)
	{
	    if ($r_type)
	    {
		    ...
		    if (!$save && $_SESSION['special_interface'][$special_type]['numbers'][$i][1] > 0 || $save && $_SESSION['special_interface'][$special_type]['numbers'][$i][1] > 1) return false;
        }
	}
	return true;
}
...
{
	foreach (array_keys($_SESSION['special_interface']) as $k) if (!TestSpecialType($k, $save)) return false;
	return true;
}
function GetSpecialNumbers($dbh)
{
    ...
    if ($res)
    {
        while ($row = mysqli_fetch_row($res))
        {
		    ...
		    if (strlen($row[1]) == 0) $spec_numbers[$row[0]]['numbers'] = array();
		    else
		    {
                $str_arr = explode(",", $row[1]);
		        for ($i = 0; $i < count($str_arr); $i++) $spec_numbers[$row[0]]['numbers'][] = array($str_arr[$i], TestSpecialNumber($str_arr[$i]));
		    }
        }
    ...
    foreach (array_keys($_SESSION['spec_titles']) as $k_type)
    {
        if (!isset($spec_numbers[$k_type]['illegal']))
        {
            ...
            $spec_numbers[$k_type]['numbers'] = array();
        }
    }
    return $spec_numbers;
}
function SwitchSpecialTexts($dbh, &$Mes)
{
	if (isset($_SESSION['special_interface']) && count($_SESSION['special_interface']) > 0)
	{
        ...
        {
            if ($_SESSION['user_working_mode'] == 1) SaveSpecialTexts($dbh);
  	        $_SESSION['special_interface'] = array();
  	        $_SESSION['special_interface_copy'] = array();
	        $_SESSION['spec_title'] = Title(465);
		}
        else $Mes[] = "<font color='#FF0000'><b>".Title(471)."</b></font>";
	}
	else
    {
        $_SESSION['special_interface'] = GetSpecialNumbers($dbh);
        $_SESSION['special_interface_copy'] = $_SESSION['special_interface'];
        if ($_SESSION['user_working_mode'] == 0) $_SESSION['spec_title'] = Title(640);
        else $_SESSION['spec_title'] = Title(466);
    }
}
function FindSpecialNumber($sn)
{
    foreach ($_SESSION['special_interface_copy'] as $k_type => $v) foreach ($v['numbers'] as $z) if (abs($z[0]) == $sn && $z[1] == 3) return false;
    return true;
}
...
function SaveSpecialTexts($dbh)
{
	...
	foreach ($_SESSION['special_interface'] as $k => $v)
	{
        ...
        {
            $arr = array();
	        foreach ($v['numbers'] as $z) if ($z[1] == 0) $arr[] = $z[0];
            if (count($arr) == 0) $arr_ins[] = "('".$k."','')";
	        else $arr_ins[] = "('".$k."','".implode(",", $arr)."')";
        }
	}
	if (count($arr_ins) > 0)
    {
...
{
	$n_t = abs((integer)$spec_num);
	if (isset($_SESSION['titles'][$n_t]))
	{
		if ((integer)$spec_num < 0) return FTM(Title($n_t));
		return Title($n_t);
	}
	else return "";
}
function SpecialTextRow($spec_type, $v, $block_flag, $add_button, $col0)
{
	if (!$block_flag)
	{
		...
		if ($add_button == "" || $v['illegal']) echo "<td width='1%'></td>";
		else echo "<td width='1%'><button name='".$add_button."' title='".Title(456)."' type='submit' value='*'".$dis.">>></button></td>";
	}
	...
	for ($i = $col0; $i < $last_number; $i++)
	{
		...
		if ($v['numbers'][$i][1] == 1) $title = Title(455)." (".Title(581).")";
		elseif ($v['numbers'][$i][1] == 2) $title = Title(458);
		elseif ($v['numbers'][$i][1] == 3) $title = Title(459);
		else $title = TitleTitles($v['numbers'][$i][0]);
...
{
	if ($tested_value == "") return 1;
	elseif (!is_numeric($tested_value)) return 2;
	elseif ((integer)$tested_value != 0)
	{
		$n_t = abs((integer)$tested_value);
		if (!isset($_SESSION['titles'][$n_t]) || isset($_SESSION['titles'][$n_t]) && $_SESSION['titles'][$n_t] == "Missing text ".(string)$n_t) return 3;
		else return 0;
	}
	return 0;
}
function AllSpecialTitleNumbers($dbh)
{
	...
	if ($res)
	{
		while ($row = mysqli_fetch_row($res))
		{
            ...
            {
			    $arr = explode(",", $row[0]);
			    if (count($arr) > 0) foreach ($arr as $z) if (is_numeric($z) && $z != "0" && !in_array(abs((integer)$z), $arr_res)) $arr_res[] = abs((integer)$z);
            }
		}
		mysqli_free_result($res);
	}
...
{
	$spec_num_arr = array();
	$spec_keys = array("table_types", "compare_mode", "sort_mode", "field_align", "field_using", "field_types", "group_types", "z_o");
	foreach ($spec_keys as $special_type) $spec_num_arr[$special_type] = array_chunk($_SESSION['special_interface'][$special_type]['numbers'], $_SESSION['max_table_columns']);
	return $spec_num_arr;
}
?>
