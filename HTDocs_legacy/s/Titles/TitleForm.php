...
if (count($_POST) == 0)
{
    ...
    if (PermitInvRef("interface_texts", array("li"))) $_SESSION['replace_mes'] = array();
...
foreach ($_POST as $str_key => $str_v)
{
	...
	switch ($s_k)
	{
		...
		case "user_lang_s"		: AfterTitleLangChoice($dbh, $sw_break); break;
		case "title_lang_s"		: AfterTitleSelLangChoice($dbh, $sw_break); break;
		case "title_modes"		: SwitchTitleMode($dbh,  $sw_break); break;
        case "replace_s"        : if (AfterReplaceSelect(array("li"=>$_SESSION['title_langs']), $sw_break)) TitleFilter($dbh); break;
		case "title_exit"		: $goto = TitleExit($dbh, $Mes); if ($goto != "" && count($Mes) == 0) header("Location: ".$goto.".php"); break;
		case "special_texts"	: SwitchSpecialTexts($dbh, $Mes); TitleFilter($dbh); break;
		case "integrity_check"	: IntegrityCheck($dbh); TitleFilter($dbh); break;
		case "title_identifier"	: TitleEdit($dbh, $k[1], $k[2]); TitleFilter($dbh); break;
		case "title_lang_insert": TitleLangInsert($dbh, (integer)$k[1], (integer)$k[2]); TitleFilter($dbh); break;
		case "list_minus"		: ChangeTitleScreen($dbh, -1, $page_mes); break;
		case "list_plus"		: ChangeTitleScreen($dbh, 1, $page_mes); break;
		case "list_height_b"	: ChangeTitleScreen($dbh, $_POST['list_height'], $page_mes); break;
		case "title_navigation"	: TitleNavigation($dbh, $k[1]); break;
		case "title_filter"		: TitleFilter($dbh); break;
		case "add_new_title"	: AddNewTitle($dbh); break;
		case "add_special"		: $_SESSION['special_interface'][$k[1]]['numbers'][] = array("", 0); $_SESSION['new_special_number'] = $k[1]; TitleFilter($dbh); break;
		case "title_references"	: GetInvRef("interface_texts", array("li"=>$_SESSION['title_langs'])); TitleFilter($dbh); break;
		case "title_mark_del"	: $_SESSION['del_mes'] = TitleMarkDelete($k[1], $k[2]); if ($_SESSION['del_mes'][1] == "") TitleFilter($dbh); break;
		case "title_del"		: TitleDelete($dbh); break;
		case "title_no_del"		: unset($_SESSION['title_del']); TitleFilter($dbh); break;
        case "replace_id"       : $_SESSION['replace_mes'] = array(); ReplaceTitleLangIDs($dbh, $k[1], $k[2]); break;
...
<form method="post" id="title_form" name="title_form">
    ...
    <?php
    if ($_SESSION['inv_ref'] && $_SESSION['title_filter'] == "" && PermitInvRef("interface_texts", array("li"))) InvRefTable("interface_texts", "emph_data", array("li"=>$_SESSION['title_langs']));
	...
	if (isset($_SESSION['mes_integrity']))
	{
		foreach ($_SESSION['mes_integrity'] as $z)
        {
            ...
            if ($str != "") echo "<div>".$str."</div>";
        }
		echo "<hr align='left' size='1' noshade='noshade' color='#000000' >";	
	}
	if (count($Mes) > 0) foreach ($Mes as $z) echo "<div>".$z."</div>";
	if (isset($_SESSION['special_interface']) && count($_SESSION['special_interface']) > 0) require_once("SpecialTextsForm.php");
	...
	<table frame="border" width="100%">
		<tr valign="top">
			...
			<td>
			<?php
			if (count($_SESSION['title_param']) == 0) echo "<table align='center'><tr><td><font size='+2'><b>".Title(553)."</b></font></td></tr></table>";
			else
            {
                if (isset($_SESSION['title_del'])) DBMDelQuestion("modal_form", array("title_del", "title_no_del"), array(), 208, 362);
