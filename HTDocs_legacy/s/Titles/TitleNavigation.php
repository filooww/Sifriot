...
{
    if (count($_SESSION['title_param']) == 0) return 0;
...
function EndTitlePosition()
{
	...
	{
        if (count($_SESSION['title_param']) == 0) return $_SESSION['start'];
		$curr = reset($_SESSION['title_param']);
		...
		if ($curr === false) return $_SESSION['start'];
...
function TitleNavigation($dbh, $nav)
{
	...
	switch ($nav)
	{
		...
		case "pgup": $s_p = ($_SESSION['selected_title_lang'][0] == 0 && $_SESSION['view_title_mode'][0] != 2) ? PrevTitlePagePosition() : max($_SESSION['start'] - $_SESSION['portion'] + 1, 0); break;
		case "tlup": case "lnup": $s_p = max($_SESSION['start'] - 1, 0); break;
		case "lndn": $s_p = min($_SESSION['start'] + 1, $_SESSION['title_count'] - 1); break;
		case "tldn": $s_p = ($_SESSION['selected_title_lang'][0] == 0 && $_SESSION['view_title_mode'][0] != 2) ? NextTitlePosition() : min($_SESSION['start'] + 1, $_SESSION['title_count'] - 1); break;
		case "pgdn": $s_p = ($_SESSION['selected_title_lang'][0] == 0 && $_SESSION['view_title_mode'][0] != 2) ? NextTitlePagePosition() : min($_SESSION['start'] + $_SESSION['portion'] - 1, $_SESSION['title_count'] - 1); break;
		case "end" : $s_p = ($_SESSION['selected_title_lang'][0] == 0 && $_SESSION['view_title_mode'][0] != 2) ? EndTitlePosition() : $_SESSION['title_count'] - 1; break;
	}
