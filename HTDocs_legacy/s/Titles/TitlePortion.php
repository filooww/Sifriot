...
function GetTitlePortion($dbh, $nav = "")
{
	...
	if ($res)
	{
		...
		{
			$title_param[TitleKey($row[0], $row[1])] = array($row[2], "", (string)$row[0], $row[1]);
		}
		...
		if ($_SESSION['selected_title_lang'][0] == 0 && $_SESSION['view_title_mode'][0] != 2 && count($title_param) > 0 && substr($nav, 0, 2) != "ln")
		{
			...
			if ($_SESSION['start'] > 0)
			{
				...
				if (count($title_param) > $_SESSION['portion']) $_SESSION['portion'] += $shift_back;
			}
            if (count($title_param) > 0) TitleExtentForward($dbh, $w_w, $title_param);
		}
		if (count($title_param) > $_SESSION['portion']) $_SESSION['portion'] = count($title_param);
	}
	return $title_param;
}
function TitleExtentBack($dbh, $w_w, &$title_param)
{
	...
	if ($res)
	{
		while ($row = mysqli_fetch_row($res))
		{
			...
			{
				$title_param[TitleKey($row[0], $row[1])] = array($row[2], "", $row[0], $row[1]);
				$sh++;
			}
		}
		mysqli_free_result($res);
...
function TitleExtentForward($dbh, $w_w, &$title_param)
{
	...
	if ($res)
	{
		while ($row = mysqli_fetch_row($res))
		{
			if (!array_key_exists(TitleKey($row[0], $row[1]), $title_param)) $title_param[TitleKey($row[0], $row[1])] = array($row[2], "", $row[0], $row[1]);
		}
		mysqli_free_result($res);
	}
}
