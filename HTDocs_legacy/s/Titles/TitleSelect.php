...
{
    if (isset($_SESSION['titles'][$title_id])) {
        return $_SESSION['titles'][$title_id];
    } else {
...
function SplitTitle($title_id)
{
    ...
    echo '<table>';
    foreach ($title_split as $t_p) {
        echo '<tr><td>'.$t_p.'</td></tr>';
    }
    echo '</table>';
}
            ...
            while (($file = readdir($dir)) !== false) {
                if ($file != '.' && $file != '..') {
                    if (is_dir($dir_name.'/'.$file)) {
                        TitleByDirs($dir_name.'/'.$file);
                    } elseif (pathinfo($dir_name.'/'.$file, PATHINFO_EXTENSION) == 'php') {
                        AddTitleIds(file_get_contents($dir_name.'/'.$file));
                    }
                }
            }
            closedir($dir);
    ...
    while ($i !== false) {
        ...
        if ($i !== false) {
            ...
            if ($j !== false) {
                ...
                if (is_numeric($title_id) && ! in_array((int) $title_id, $_SESSION['scripts_title_ids'])) {
                    $_SESSION['scripts_title_ids'][] = (int) $title_id;
                }
            }
            $p = $j + 1;
        }
