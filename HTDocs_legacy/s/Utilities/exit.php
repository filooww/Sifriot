...
function GetColor($get_par)
{
    ...
    switch ($color_par) {
        case '1': $cl = " class='mess_red'";
            break;
        case '2': $cl = " class='mess_green'";
            break;
        case '3': $cl = " class='mess_blue'";
            break;
...
$exit_str = '';
foreach ($_GET as $v) {
    $exit_str .= "<div align='center'".GetColor($v).'><h'.substr($v, 1, 1).'>'.substr($v, 2).'</h'.substr($v, 1, 1).'></div>';
}
session_destroy();
if ($exit_str == '') {
    $exit_str = "<div align='center' class='mess_blue'>Session completed</div>";
} // titles #16
exit($exit_str);
?>
