...
function CalendarSelect($arr_key, $sy, &$calend_date, &$p_var, &$calend_mes)
{
    ...
    $sw_break = true;
    switch ($arr_key[0]) {
        case 'to_calendar':		$calend_mes = CalendSetDate('filter_text-'.$arr_key[1], 'text', $sy, $calend_date);
            break;
        case 'to_calendar_to':	$calend_mes = CalendSetDate('filter_max-'.$arr_key[1], 'to', $sy, $calend_date);
            break;
        case 'calendar_OK':		CalendarSetValue($calend_date, $p_var);
            break;
        case 'calendar_exit':	$calend_date = ['id_key' => 'UploadDate', 'id_filter' => '', 'value' => ''];
            break;
...
function CalendarSetValue(&$calend_date, &$p_var)
{
    ...
    $p_var[$id_key]['filter'][$id_filter] = CalendGetDate([$_POST['calendar_day'], $_POST['calendar_month'], $_POST['calendar_year']]);
    $calend_date = ['id_key' => 'UploadDate', 'id_filter' => '', 'value' => ''];
}
...
function CalendSetDate($post_name, $filter_name, $start_y, &$calend_date)
{
    ...
    if (isset($_POST[$post_name]) && $_POST[$post_name] != '') {
        ...
        if (count($arr_date) == 2) {
            $calend_mes[] = ['time' => '', 'text' => Title(21), 'status' => 'warning'];
            $arr_date[] = '';
        } elseif (count($arr_date) == 1) {
            $calend_mes[] = ['time' => '', 'text' => Title(21), 'status' => 'warning'];
            $arr_date[] = '';
            $arr_date[] = '';
        } elseif (count($arr_date) > 3) {
            $calend_mes[] = ['time' => '', 'text' => Title(22), 'status' => 'warning'];
        }
        ...
        $calend_mes[] = ['time' => '', 'text' => Title(23), 'status' => 'warning'];
        switch (substr($_SESSION['date_format'], 1)) {
...
function TestYear(&$sy, $starty, $cury, &$calend_mes)
{
    if ($sy == '' || ! is_numeric($sy)) {
        $sy = (string) $cury;
        $calend_mes[] = ['time' => '', 'text' => Title(27).' '.Title(535).'. '.Title(24), 'status' => 'warning'];
    } elseif ((int) $sy < $starty || (int) $sy > $cury) {
        $sy = (string) $cury;
        $calend_mes[] = ['time' => '', 'text' => Title(27).' '.Title(535).'. '.Title(24), 'status' => 'warning'];
    }
}
function TestMonth(&$sm, &$calend_mes)
{
    if ($sm == '' || ! is_numeric($sm)) {
        $sm = '01';
        $calend_mes[] = ['time' => '', 'text' => Title(28).' '.Title(535).'. '.Title(25), 'status' => 'warning'];
    } elseif ((int) $sm < 1 || (int) $sm > 12) {
        $sm = '01';
        $calend_mes[] = ['time' => '', 'text' => Title(28).' '.Title(535).'. '.Title(25), 'status' => 'warning'];
    }
}
function TestDay(&$sd, $sm, $sy, &$calend_mes)
{
    if ($sd == '' || ! is_numeric($sd)) {
        $sd = '01';
        $calend_mes[] = ['time' => '', 'text' => Title(29).' '.Title(535).'. '.Title(26), 'status' => 'warning'];
    } elseif ((int) $sd < 1 || (int) $sd > CalendMaxDay((int) $sm, (int) $sy)) {
        $sd = '01';
        $calend_mes[] = ['time' => '', 'text' => Title(29).' '.Title(535).'. '.Title(26), 'status' => 'warning'];
    }
}
...
function CalendTestDate($test_date, $start_y = 0)
{
    ...
    if (count($arr_date) == 3) {
        ...
        $Mes[] = Title(22).' (<b>'.$_SESSION['date_format'][0].'</b>)';
    } else {
        $Mes[] = Title(21).' (<b>'.$_SESSION['date_format'][0].'</b>)';
    }
...
{
    if ($sy == '' || ! is_numeric($sy)) {
        $Mes[] = Title(27).' '.Title(535);
    } elseif (strlen($sy) != 4) {
        $Mes[] = Title(27).' '.Title(535);
    } elseif ($sy[0] == '+' || $sy[0] == '-') {
        $Mes[] = Title(27).' '.Title(535);
    } elseif ($starty > 0 && ((int) $sy < $starty || (int) $sy > $cury)) {
        $Mes[] = Title(27).' '.Title(535);
    }
}
...
function StringToDateFormat($date_str, $date_delim, &$Mes)
{
    ...
    if (count($arr_date) == 3) {
        ...
        TestD($arr_date[$ad[0]], $arr_date[1], $arr_date[2], $Mes);
        if (count($Mes) == 0) {
            return GetDatePart($arr_date[$ad[2]], 4).'-'.GetDatePart($arr_date[$ad[1]], 2).'-'.GetDatePart($arr_date[$ad[0]], 4);
        } else {
            return null;
        }
    } else {
...
{
    if (strlen($date_part) == $date_part_length) {
        return $date_part;
    } elseif (strlen($date_part) < $date_part_length) {
        return str_repeat('0', $date_part_length - strlen($date_part)).$date_part;
    } else {
        return substr($date_part, -1, $date_part_length);
    }
}
function ToDate($dt, $date_delim, $to_space = false)
{
    ...
    if (is_null($dt) && $to_space) {
        return '';
    }
    $arr_date_time = explode(' ', $dt);
    $arr_date = explode('-', $arr_date_time[0]);
    $str_date = $arr_date[2].$date_delim.$arr_date[1].$date_delim.$arr_date[0];
    if (count($arr_date_time) == 1) {
        return $str_date;
    } else {
        return $str_date.' '.$arr_date_time[1];
    }
}
function TestDateFormat($d_format)
{
    if (substr($d_format, 1) == 'dmy' || substr($d_format, 1) == 'mdy' || substr($d_format, 1) == 'ydm' || substr($d_format, 1) == 'ymd') {
        return true;
    }
...
{
    switch (substr($_SESSION['date_format'], 1)) {
        case 'dmy': return [0, 1, 2];
        case 'mdy': return [1, 0, 2];
        case 'ydm': return [1, 2, 0];
        case 'ymd': return [2, 1, 0];
    }
