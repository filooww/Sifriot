...
function SetExPar($block, $cl, $use = false)
{
    ...
    if (count($arr_par) == 0) {
        ...
        return ' '.implode(' ', $arr_par);
    }
}
function OptionTag($selected_value, $option_value, $disables_value = false)
{
    if ($option_value != $selected_value && ! $disables_value) {
        return '<option>'.$option_value.'</option>';
    }
    if ($option_value != $selected_value && $disables_value) {
        return '<option disabled>'.$option_value.'</option>';
    }
    if ($option_value == $selected_value && ! $disables_value) {
        return '<option selected>'.$option_value.'</option>';
    }
...
function QuestionForm($table_class, $text, $button_name_arr, $button_text_arr)
{
    ...
    if (is_array($text)) {
        foreach ($text as $v) {
            echo "<tr align='center'><td><p align='center'><b>".$v.'</b></p></td></tr>';
        }
    } else {
        echo "<tr align='center'><td><p align='center'><b>".$text.'</b></p></td></tr>';
    }
    echo "<tr align='center'>";
    echo "<td align='center'>";
    for ($i = 0; $i < count($button_name_arr); $i++) {
        echo "<input size='10' name='".$button_name_arr[$i]."' type='submit' value='".$button_text_arr[$i]."'>";
    }
    echo '</td>';
    echo '</tr>';
    echo '</table>';
...
{
    if ($flag) {
        return ImgV('CheckBorder', $w, $h, $block);
    } else {
        return ImgV('BlankBorder', $w, $h, $block);
    }
}
function BSize($w, $h)
{
...
function SelectTag($select_name, $options, $selected_value, $h_name = '', $by_key = false, $value_key = '', $on_func = '', $disabled_user = false, $display_invalid = true, $tag_class = '', $tag_title = '')
{
    ...
    if ($on_func == '') {
        echo '<select '.$ttl.$tag_class." name='".$select_name."'".$dis.'>';
    } else {
        if (strpos($on_func, '(') === false) {
            echo '<select'.$ttl.$tag_class." name='".$select_name."' id='".$select_name."' onchange='".$on_func."()'".$dis.'>';
        } else {
            echo '<select'.$ttl.$tag_class." name='".$select_name."' id='".$select_name."' onchange='".$on_func."'".$dis.'>';
        }
    }
    if ($display_invalid) {
        if ($by_key) {
            ...
            if (! in_array($selected_value, $options)) {
                $invalid_value = '<<'.$selected_value.'>>';
            }
        }
    }
    ...
    if (isset($invalid_value)) {
        echo '<option selected disabled>'.$invalid_value.'</option>';
    }
    echo '</select>';
    if ($h_name != '') {
        echo "<input type='hidden' name='".$h_name."' id='".$h_name."' value=''>";
    }
}
function RadioTag($radio_name, $checked_value, $checked_texts, $bg_class, $after = false, $on_func = '', $flag_name = '', $dis = false)
{
    ...
    for ($i = 0; $i < count($checked_texts); $i++) {
        if ($after) {
            echo $checked_texts[$i]."<input type='radio' name='".$radio_name."'".$on_part." value='".(string) $i."'".(($i == $checked_value) ? ' checked' : '').(($dis) ? ' disabled' : '').'>';
        } else {
            echo "<input type='radio' name='".$radio_name."'".$on_part." value='".(string) $i."'".(($i == $checked_value) ? ' checked' : '').(($dis) ? ' disabled' : '').'>'.$checked_texts[$i];
        }
        echo "<font class='".$bg_class."'>X</font>";
    }
    if ($flag_name != '') {
        echo "<input type='hidden' name='".$flag_name."' value=''>";
    }
}
function AfterLangChoice($dbh, $hide_name, $lang_name, &$sw_break)
{
    ...
    if ($_POST[$hide_name] == '') {
        ...
        if ($lang_key !== false) {
            $_SESSION['user_lang'] = [$lang_key, $_POST[$lang_name]];
            $_SESSION['titles'] = GetTitlesByLanguage($dbh, (int) $lang_key);
            if (isset($_SESSION['admin_mes'])) {
                TextAdminMessage($_SESSION['admin_mes']);
            }
            $cf = true;
        }
    }
...
{
    switch ($struct_mes[0]) {
        case 1: case 3: $struct_mes[1] = Title(53).'. '.Title(198);
            break;
        case 2: $struct_mes[1] = Title(92).'. '.Title(198);
            break;
        case 4: $struct_mes[1] = Title(161).'. '.Title(198);
            break;
        case 5: $struct_mes[1] = Title(206).'. '.Title(236);
            break;
        case 6: $struct_mes[1] = Title(188).'. '.Title(236);
            break;
