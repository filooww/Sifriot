...
function GetManagerDBFile($dbname, $pass_file, $coding = '')
{
    ...
    if (file_exists($pass_file)) {
        ...
        if (count($arrAuth) > 2) {
            $_SESSION['serv'] = ['host' => $arrAuth[0], 'user' => $arrAuth[1], 'pass' => $arrAuth[2]];
            $dbh = GetOnlyDB($dbname, $coding);
            $fl = true;
        }
    }
...
function GetTableLimit($dbh, $wh, $n, $mc = false, $alias_name = '')
{
    if ($wh == 'total') {
        ...
        if ($_SESSION['Catalog']['1']['name'] == '' && $n == '0' || $n == '1') {
            return SingleCount($dbh, $wh, $n);
        } else {
            return SetsCount($dbh, $mc, $wh, $alias_name);
        }
    }
}
...
function SetsCount($dbh, $mc, $wh, $alias_name)
{
    ...
    if ($wh == 'filter') {
        ...
        $f_where = [MCV($_SESSION['catalog_param']['0']['filter_text'], $mc), $_SESSION['catalog_param']['0']['filter_compare']];
        $s_where = [''];
    } else {
        if ($_SESSION['catalog_param']['0']['search_text'] == '') {
            return 0;
        }
        if ($_SESSION['catalog_param']['0']['filter_text'] == '') {
            $f_where = [''];
            $s_where = [MCV($_SESSION['catalog_param']['0']['search_text'], $mc), $_SESSION['catalog_param']['0']['search_compare']];
        } else {
            $f_where = [MCV($_SESSION['catalog_param']['0']['filter_text'], $mc), $_SESSION['catalog_param']['0']['filter_compare']];
            $s_where = [MCV($_SESSION['catalog_param']['0']['search_text'], $mc), $_SESSION['catalog_param']['0']['search_compare']];
        }
...
{
    if ($max_lev == 0) {
        $req_text = ' CASE WHEN '.$main_table_ref.' IS NULL THEN '.$default_valus.' ELSE '.$main_table_ref.' END AS '.$alias_name;
    } elseif ($tableSet == '') {
        $int_sel = '(SELECT '.$tableRef.'.'.$valueRef.' FROM '.$tableRef.' WHERE '.$tableRef.'.'.$idRef.' = '.$main_table_ref.' LIMIT 1)';
        $req_text = ' CASE';
        if ($main_table_ref != '') {
            $req_text .= ' WHEN '.$main_table_ref.' IS NULL THEN '.$default_valus;
        }
        ...
        $req_arr = [];
        for ($i = 0; $i < $max_lev; $i++) {
            $req_arr[] = SetCaseText($tableRef, $idRef, $valueRef, $to_from, $to_where, $tableSet, $valueSet, $i, $main_table_ref, $default_valus, $alias_name);
        }
...
function SetCaseText($tableRef, $idRef, $valueRef, $to_from, $to_where, $tableSet, $valueSet, $i, $main_table_ref, $default_valus, $alias_name)
{
    $str0 = ' CASE';
    if ($i == 0) {
        $int_sel = '(SELECT '.$tableRef.'.'.$valueRef.' FROM '.$tableRef.$to_from.' WHERE '.$to_where.$tableRef.'.'.$idRef.' = SUBSTRING_INDEX('.$tableSet.'.'.$valueSet.",',',1) LIMIT 1)";
    } else {
    ...
    if ($main_table_ref != '') {
        $str0 .= ' WHEN '.$main_table_ref.' IS NULL THEN '.$default_valus;
    }
...
function OrderForSet($max_lev, $ord_dir, $alias_name, $k, $m_case, $separator)
{
    ...
    $arr_req_text = [];
    for ($i = 0; $i < $max_lev; $i++) {
        $arr_req_text[] = MCF($alias_name.$k, $m_case).(string) ($i + 1);
    }
    if (count($arr_req_text) == 0) {
        return '';
    } elseif (count($arr_req_text) == 1) {
        return $arr_req_text[0].$add_sort;
    }
...
function ConcatPart($max_lev, $alias_name, $m_case)
{
    $concat_arr = [];
    for ($i = 0; $i < $max_lev; $i++) {
        $concat_arr[] = MCF($alias_name, $m_case).(string) ($i + 1);
    }
    if (count($concat_arr) == 0) {
        return '';
    } elseif (count($concat_arr) == 1) {
        return $concat_arr[0];
    }
...
function QueryTextForSet($dbh_sys, $mc, $ord_dir, $f_arr, $s_arr, $alias_name, $compr = '', $value = '', $limit = '', $f_list = '')
{
    ...
    if (! $mc) {
        $rt .= ', '.VF($max_lev, $_SESSION['Catalog']['0']['table'], $_SESSION['Catalog']['0']['id'], $_SESSION['Catalog']['0']['value'], $_SESSION['Catalog']['1']['table'], $_SESSION['Catalog']['1']['id'], $_SESSION['Catalog']['1']['value'].'_low', '', chr(34).chr(34), $alias_name.'_low', false);
    }
    $flist = ($f_list == '') ? '*' : $f_list;
    ...
    if ($f_arr[0] != '' || $s_arr[0] != '' || $compr != '') {
        $concat_cl = ConcatPart($max_lev, $alias_name, $mc);
    }
    if ($f_arr[0] != '' && $s_arr[0] != '') {
        $where_fs = GetWhere($concat_cl, $f_arr[0], $f_arr[1]).' AND '.GetWhere($concat_cl, $s_arr[0], $s_arr[1]);
    } elseif ($f_arr[0] != '' && $s_arr[0] == '') {
        $where_fs = GetWhere($concat_cl, $f_arr[0], $f_arr[1]);
    } elseif ($f_arr[0] == '' && $s_arr[0] != '') {
        $where_fs = GetWhere($concat_cl, $s_arr[0], $s_arr[1]);
    } else {
        $where_fs = '';
    }
    if ($compr != '') {
        $where_compr = $concat_cl.' '.$compr." '".$value."'";
    } else {
        $where_compr = '';
    }
    if ($where_fs != '' && $where_compr != '') {
        $rt .= 'WHERE '.$where_fs.' AND '.$where_compr;
    } elseif ($where_fs != '' && $where_compr == '') {
        $rt .= 'WHERE '.$where_fs;
    } elseif ($where_fs == '' && $where_compr != '') {
        $rt .= 'WHERE '.$where_compr;
    }
    if ($ord_dir != 0) {
        $rt .= ' ORDER BY '.OrderForSet($max_lev, $ord_dir, $alias_name, '', $mc, $_SESSION['Catalog']['0']['separator']).','.$_SESSION['Catalog']['0']['id'];
    }
    if ($limit != '') {
        $rt .= ' LIMIT '.$limit;
    }
...
function SaveMaxLevel($dbh, $cur_max_level)
{
    ...
    if ($reqMax) {
        if ($row = mysqli_fetch_row($reqMax)) {
            if ($cur_max_level > $row[0]) {
                mysqli_query($dbh, 'UPDATE table_definitions SET max_level = '.(string) $cur_max_level." WHERE table_name = '".$_SESSION['Catalog']['0']['table']."'");
            }
        } else {
...
{
    switch ($compare_mode) {
        case 0:  return $v_field." LIKE '".$w_text."%'";
            break;
        case 1:  return $v_field." LIKE '%".$w_text."%'";
            break;
        case 2:  return $v_field." = '".$w_text."'";
            break;
        default: return '';
    }
}
...
function MCV($source_value, $m_case)
{
    ...
    if ($m_case) {
        ...
        return mb_strtolower($source_value, 'utf-8');
    }
}
