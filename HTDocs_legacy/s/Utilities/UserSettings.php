...
function InitUserSettings($dbh, $u_id)
{
    ...
    if ($res) {
        if ($row = mysqli_fetch_row($res)) {
            ...
            $arr_data = InitReqUserSettings($dbh, []);
...
function InitReqUserSettings($dbh, $row)
{
    ...
    if (count($row) == 0) {
        $arr_data[] = (string) $_SESSION['portion'];
    } elseif ($row[4]) {
...
function FillUserSettings($row, $u_id)
{
    ...
    $_SESSION['pri'] = $row[3];
    $_SESSION['cur_lang'] = [$row[7], $_SESSION['langs'][(string) $row[7]]];
