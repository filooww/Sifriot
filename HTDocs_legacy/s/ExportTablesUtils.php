...
function ExportToFile()
{
    ...
    if (! $dbh) {
        ...
        if ($chat_set == '') {
            $mes_arr[] = 'no character set database';
        }
        $coll = GetDBParams($dbh, 'SELECT @@collation_database');
        if ($coll == '') {
            $mes_arr[] = 'no collation database';
        }
        ...
        fclose($fh);
        if ($_POST['table'] != '') {
            ExportTable($dbh, $f_name, $_POST['table'], $chat_set);
        } else {
            foreach ($_SESSION['table_list'] as $tb) {
                if ($tb != '') {
                    ExportTable($dbh, $f_name, $tb, $chat_set);
                }
            }
        }
    }
    if (count($mes_arr) == 0) {
        return 'The result is in the file <b>'.$_SERVER['DOCUMENT_ROOT'].'/s/'.$f_name.'.sql</b>';
    } else {
        return implode('; ', $mes_arr);
    }
}
...
function InsertFieldStructure($dbh, $f_name, $table, $chat_set)
{
    ...
    for ($i = 0; $i < count($arr['fields']); $i++) {
        ...
        if (! is_null($arr['fields'][$i][2]) && $arr['fields'][$i][2] != '') {
            $str .= ' CHARACTER SET '.$chat_set.' COLLATE '.$arr['fields'][$i][2];
        }
        if (! is_null($arr['fields'][$i][3]) && $arr['fields'][$i][3] != '') {
            $str .= (($arr['fields'][$i][3] == 'NO') ? ' NOT NULL' : ' NULL');
        }
        if ($arr['fields'][$i][3] == 'YES' && is_null($arr['fields'][$i][5])) {
            $str .= ' DEFAULT NULL';
        } elseif (! is_null($arr['fields'][$i][5]) && $arr['fields'][$i][5] != '') {
            $str .= " DEFAULT '".$arr['fields'][$i][5]."'";
        }
        if (! is_null($arr['fields'][$i][6]) && $arr['fields'][$i][6] != '') {
            $arr['auto_inc'] = $auto_inc = $arr['fields'][$i][0].' '.$arr['fields'][$i][1];
            if (! is_null($arr['fields'][$i][3]) && $arr['fields'][$i][3] != '') {
                if ($arr['fields'][$i][3] == 'NO') {
                    $arr['auto_inc'] .= ' NOT NULL';
                } else {
                    $arr['auto_inc'] .= ' NULL';
                }
            }
        }
        if ($i < count($arr['fields']) - 1) {
            $str .= ',';
        }
        fwrite($fh, '  '.$str."\r\n");
    }
...
function InsertRow($row, $arr_fields)
{
    ...
    foreach ($row as $v) {
        if (is_null($v)) {
            $arr_row[] = 'NULL';
        } elseif (! is_null($arr_fields[$i][2]) && $arr_fields[$i][2] != '') {
            $arr_row[] = "'".$v."'";
        } else {
            $arr_row[] = $v;
        }
...
function InsertIndexInformation($dbh, $f_name, $table, $auto_inc)
{
    ...
    if (count($ind) > 0) {
        ...
        foreach ($ind as $ind_v) {
            if (! in_array($ind_v[2], $ind_names)) {
                $ind_names[$ind_v[2]] = $ind_v[1];
            }
        }
        ...
        foreach ($ind_names as $k => $uniq) {
            ...
            foreach ($ind as $ind_v) {
                if ($ind_v[2] == $k) {
                    $ind_field_arr[$ind_v[6]] = $ind_v[4];
                }
            }
            ksort($ind_field_arr);
            if ($k == 'PRIMARY') {
                $str = 'PRIMARY KEY ('.implode(',', $ind_field_arr).')';
            } elseif ($uniq == 0) {
                $str = 'UNIQUE KEY '.$k.' ('.implode(',', $ind_field_arr).')';
            } else {
                $str = 'KEY '.$k.' ('.implode(',', $ind_field_arr).')';
            }
            if ($k == $last_key) {
                $str .= ';';
            } else {
            ...
            if ($conn) {
                if (IsDB($conn, $dbname)) {
                    ...
                    if ($dbh) {
                        ...
                        $Mes = 'Error connecting to DB';
                    }
                } else {
                    $Mes = 'There is no such DB';
                }
            } else {
                $Mes = 'Failed to establish connection to DB server';
            }
        } else {
            $Mes = 'Invalid content of credential file';
        }
    } else {
        $Mes = 'No credential file';
    }
    if ($fl) {
        return $dbh;
    } else {
