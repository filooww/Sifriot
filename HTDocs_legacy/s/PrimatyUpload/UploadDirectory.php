...
$correct_files = 0;
$load_files = array();
if ($_SESSION['primary_dir'] == "") exit("Source directopy not specified");
for ($i = 0; $i < count($_FILES['filename']['name']); $i++) 
{
	if ($_FILES['filename']['name'][$i] != "") 
	{
		...
		{
			$load_file = array("tmp"=>$_FILES['filename']['tmp_name'][$i], "name"=>$_FILES['filename']['name'][$i], "err_ind"=>$_FILES['filename']['error'][$i]);
			...
			if ($file_struct['id_pub'] != "")
			{
				...
				if ($m['status'] = "statement") $correct_files++;
			}
		}
		else $_SESSION['mes']['u'] = array("time"=>GetMessageDate(), "text"=>"File <b>".$load_file[$i]['name']."</b>: ".UploadErrorMessages($_FILES['filename']['error'][$i]), "status"=>"error", "log"=>true);
...
<table class="table_fix">
	<tr valign="top">
		<td class="cell_fix">
			<p class="line_gap">
				<?php for ($i = 0; $i < count($load_files); $i++) PutToFileList($load_files[$i], $i);?>
			</p>
		</td>
	</tr>
</table>
