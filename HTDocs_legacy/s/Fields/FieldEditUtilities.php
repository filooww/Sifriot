...
function CaseField($post_name, $par_name, $js_on, $f_type, &$even, $td_align, $field_list, $comment, $Mes, $read_only = false, $align_text = '', $add_button = '', $add_class = '', $add_image = '')
{
    ...
    if ($f_type == 'text') {
        echo "<input style='font-weight:bold' size='16' name='".$post_name."' type='text' ".$t_cls."value='".$_SESSION['field_param'][$par_name]."'".(($read_only) ? ' readonly' : '').$dis.'/>';
    } elseif ($f_type == 'check') {
        echo "<button type='submit' name='".$post_name."' class='".$cls."' value='*'".$dis.JSF($js_on, $par_name).'>'.CheckImg($_SESSION['field_param'][$par_name], 16, 15, $_SESSION['user_working_mode'] == 0).'</button>';
    ...
    echo '</td>';
    if ($add_button != '') {
        echo "<td><button name='".$add_button."' type='submit' title='".Title(303)."' class='".$add_class."' value='*'".$dis.'>'.(($add_image == '') ? '' : ImgV($add_image, 10, 16)).'</button></td>';
    } else {
        echo "<td class='".$cls."'></td>";
    }
    echo "<td class='".$cls."'>".$comment.'</td>';
    if (isset($Mes[$post_name])) {
        echo "<td><font color='FF0000'>".CrMes($Mes[$post_name]).'</font></td>';
    } else {
        echo '<td></td>';
    }
    echo '</tr>';
...
{
    if ($js_on) {
        return " onchange='".$field_name."_on();'";
    } else {
        return '';
    }
}
...
{
    if ($mess_err[0] == '*') {
        return substr($mess_err, 1)." <a name='view_err' title='".Title(405)."' href='FieldFormWarn.php' target='_blank'><img src='".$_SESSION['image_dir']."/GoToK.bmp' width='16' height='16'></a>";
    } else {
        return $mess_err;
    }
}
function UsingProc($dbh, &$sw_break)
{
    if ($_POST['usings_s'] == '') {
        ...
        if ($i_com !== false) {
            if ($_SESSION['field_param']['using'] == '') {
                $_SESSION['field_param']['using'] = (string) $i_com;
            } else {
...
{
    if (! is_numeric($tested)) {
        $Mes[$tested] = Title(77);
    } elseif ((int) $tested < 0 || (int) $tested > 0 && $tested != (string) (floor((int) $tested))) {
        $Mes[$tested] = Title(350);
    }
}
function TestChangedPercent($tested, $end_arr, &$Mes)
{
    ...
    if ($i == 0) {
        $m_perc[] = Title(713);
    } elseif (! in_array(substr($perc, $i), $end_arr)) {
        $m_perc[] = Title(714);
    }
    if (count($m_perc) > 0) {
        $Mes = implode(', ', $m_perc);
...
function TestFieldParameters(&$Mes)
{
    ...
    if (! in_array($_POST['type'], $_SESSION['reference_catalogs'])) {
        $Mes['type'] = Title(596).' <b>'.$_POST['type'].'/b>';
    }
    if ($_POST['size'] != '') {
        TestNumberValue($_POST['size'], $Mes);
    }
    if (! in_array($_POST['table'], $_SESSION['reference_catalogs'])) {
        $Mes['table'] = Title(596).' <b>'.$_POST['table'].'/b>';
    }
    if (! in_array($_POST['filter_md'], $_SESSION['compare_mode'])) {
        $Mes['filter_md'] = Title(596).' <b>'.$_POST['filter_md'].'/b>';
    }
    if (! in_array($_POST['sort_sm'], $_SESSION['sort_mode'])) {
        $Mes['sort_sm'] = Title(596).' <b>'.$_POST['sort_sm'].'/b>';
    }
    if ($_POST['using'] != '') {
        ...
        if (count($arr_err) > 0) {
            $Mes['using'] = Title(596).' (<b>'.implode('; ', $arr_err).'</b>)';
        }
    }
    if (! in_array($_POST['f_align'], $_SESSION['field_align'])) {
        $Mes['f_align'] = Title(596).' <b>'.$_POST['f_align'].'/b>';
    }
    if ($_POST['t_prc'] != '') {
        TestChangedPercent($_POST['t_prc'], $Mes);
    }
    if ($_POST['screen_order'] != '') {
        TestNumberValue($_POST['screen_order'], $Mes);
    }
    $test_repeat = TestRepeatValuesForField();
    if (count($arr_rep['name']) > 0) {
        if (isset($Mes['name'])) {
            $Mes['name'] .= ', '.Tilte(490).' <b>'.Title(347).' ('.$_POST['name'].')</b> '.Title(587).' '.((count($test_repeat['name']) == 1) ? Title(655) : Title(656)).' <b>'.implode(', ', $test_repeat['name']);
        } else {
            $Mes['name'] = Tilte(490).' <b>'.Title(347).' ('.$_POST['name'].')</b> '.Title(587).' '.((count($test_repeat['name']) == 1) ? Title(655) : Title(656)).' <b>'.implode(', ', $test_repeat['name']);
        }
    }
    ...
    if (count($arr_rep['screen']) > 0) {
        if (isset($Mes['screen_order'])) {
            $Mes['screen_order'] .= ', '.Tilte(490).' <b>'.Title(347).' ('.$_POST['screen_order'].')</b> '.Title(587).' '.((count($test_repeat['screen']) == 1) ? Title(655) : Title(656)).' <b>'.implode(', ', $test_repeat['screen']);
        } else {
            $Mes['screen_order'] = Tilte(490).' <b>'.Title(347).' ('.$_POST['screen_order'].')</b> '.Title(587).' '.((count($test_repeat['screen']) == 1) ? Title(655) : Title(656)).' <b>'.implode(', ', $test_repeat['screen']);
        }
    }
...
function TestRepeatValuesForField()
{
    ...
    foreach ($_SESSION['field_definitions'][$_SESSION['f_k'][0]] as $field_key => $f_params) {
        if ($field_key != $_SESSION['f_k'][z1]) {
            if ($_POST['name'] == $f_params[4]) {
                $arr_rep['name'][] = $field_key;
            }
            if ($f_params[2] != 0 && is_numeric($_POST['screen_order']) && $f_params[2] == (int) $_POST['screen_order']) {
                $arr_rep['screen'][] = $field_key;
            }
        }
    }
...
function AfterTableChoice(&$sw_break)
{
    if ($_POST['table_s'] == '') {
        ...
        $_SESSION['field_param']['table'] = $_POST['table'];
    }
}
...
function FieldDefinitionExit(&$Mes)
{
    if ($_SESSION['user_working_mode'] == 0 || $_SESSION['user_working_mode'] == 1 && TestFieldParameters($Mes)) {
        ...
        if (isset($Mes['filter_md'])) {
            $test_err[] = $Mes['filter_md'];
        }
        if (isset($Mes['sort_sm'])) {
            $test_err[] = $Mes['sort_sm'];
        }
        if (isset($Mes['using'])) {
            $test_err[] = $Mes['using'];
        }
        if (isset($Mes['f_align'])) {
            $test_err[] = $Mes['f_align'];
        }
        if (isset($Mes['t_prc'])) {
            $test_err[] = $Mes['t_prc'];
        }
        if (isset($Mes['screen_order'])) {
            $test_err[] = $Mes['screen_order'];
        }
        if (count($test_err) == 0) {
            $_SESSION['field_definitions'][$_SESSION['f_k'][0]][$_SESSION['f_k'][1]][20] = '';
        } else {
            $_SESSION['field_definitions'][$_SESSION['f_k'][0]][$_SESSION['f_k'][1]][20] = implode('; ', $test_err);
        }
...
{
    if (isset($_POST['screen_order'])) {
        $_SESSION['field_param']['screen_order'] = $_POST['screen_order'];
    }
    if (isset($_POST['name'])) {
        $_SESSION['field_param']['name'] = $_POST['name'];
    }
    if (isset($_POST['type'])) {
        $_SESSION['field_param']['type'] = $_POST['type'];
    }
    if (isset($_POST['f_align'])) {
        $_SESSION['field_param']['f_align'] = $_POST['f_align'];
    }
    if (isset($_POST['size'])) {
        $_SESSION['field_param']['size'] = $_POST['size'];
    }
    if (isset($_POST['t_prc'])) {
        $_SESSION['field_param']['t_prc'] = $_POST['t_prc'];
    }
    if (isset($_POST['table'])) {
        $_SESSION['field_param']['table'] = $_POST['table'];
    }
    if (isset($_POST['illegals'])) {
        $_SESSION['field_param']['illegals'] = $_POST['illegals'];
    }
    if (isset($_POST['default'])) {
        $_SESSION['field_param']['default'] = $_POST['default'];
    }
    if (isset($_POST['filter_md'])) {
        $_SESSION['field_param']['filter_md'] = $_POST['filter_md'];
    }
    if (isset($_POST['sort_sm'])) {
        $_SESSION['field_param']['sort_sm'] = $_POST['sort_sm'];
    }
}
