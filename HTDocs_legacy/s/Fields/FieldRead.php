...
{
    $PR = ['const' => [], 'var' => []];
    $_SESSION['spec_fld'][1] = ['key' => '', 'URL_file' => '', 'URL_link' => '', 'del_mark' => '', 'refer' => ''];
    $_SESSION['spec_fld'][2] = ['key' => '', 'URL_file' => '', 'URL_link' => '', 'del_mark' => '', 'refer' => ''];
    ...
    if ($res) {
        while ($row = mysqli_fetch_row($res)) {
            ...
            if ($row[2] == 1) {
                $_SESSION['spec_fld']['key'][$row[0]] = $row[1];
            }
            if ($row[4] == 5) {
                $_SESSION['spec_fld']['URL_file'][$row[0]] = $row[1];
            }
            if ($row[4] == 6) {
                $_SESSION['spec_fld']['URL_link'][$row[0]] = $row[1];
            }
            if ($row[4] == 7) {
                $_SESSION['spec_fld']['del_mark'][$row[0]] = $row[1];
            }
            if ($row[4] == 8) {
                $_SESSION['spec_fld']['refer'][$row[0]] = $row[1];
            }
