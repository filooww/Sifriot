...
function GetFieldParameters($dbh, $mandatory_db_tables)
{
    ...
    if ($res) {
        while ($row = mysqli_fetch_row($res)) {
            ...
            $f_param[$row[0]][$row[1]] = ['f_key' => ($row[2] == 1), 'ind_in_t' => $test_err['ind'] + 1, 'screen_order' => $row[20], 'load_order' => $row[21], 'f_using' => $row[17], 'f_name' => $row[3], 'f_type' => $row[4], 'f_unique' => ($row[8] == 1), 'f_default' => $row[12], 'f_size' => $row[5], 'f_interval' => ($row[6] == 1), 'f_blank' => ($row[7] == 1), 'f_s_mode' => ($row[9] == 1), 'f_table' => $row[10], 'f_illegals' => $row[11], 'f_check' => ($row[13] == 1), 'comm' => ($row[14] == 1), 'f_filter_md' => $row[15], 'f_sort_sm' => $row[16], 'f_align' => $row[18], 'table_percent' => $row[19], 'errors' => $test_err['err'], 'auto' => 0];
            if (! isset($_SESSION['all_field_list'][$row[0]][$row[1]])) {
                $f_param[$row[0]][$row[1]]['auto'] = -1;
            }
        }
...
function SetFieldAuto($mandatory_db_tables, $k_own, &$f_param)
{
    if (count($mandatory_db_tables[$k_own]) == 1 && isset($_SESSION['all_field_list'][$mandatory_db_tables[$k_own][0]])) {
        ...
        foreach (array_keys($_SESSION['all_field_list'][$mandatory_db_tables[$k_own][0]]) as $k_field) {
            if (! isset($f_param[$k_own][$k_field])) {
                $f_param[$k_own][$k_field] = ['f_key' => false, 'ind_in_t' => $i, 'screen_order' => 0, 'f_using' => '', 'f_name' => $k_field, 'f_type' => 0, 'f_unique' => false, 'f_default' => '', 'f_size' => 0, 'f_interval' => false, 'f_blank' => false, 'f_s_mode' => false, 'f_table' => '', 'f_illegals' => '', 'f_check' => false, 'comm' => false, 'f_filter_md' => 0, 'f_sort_sm' => 0, 'f_align' => 0, 'table_percent' => '', 'errors' => [], 'auto' => 1];
            }
            $i++;
        }
    }
...
function FieldInitialTest($row, $mandatory_db_tables)
{
    ...
    */
    if ($row[0] > 2) {
        $test_err['err'][] = Title(383);
    } // invalid own table
    if (! isset($_SESSION['all_field_list'][$row[0]][$row[1]]) && ! TestSysString($row[1])) {
        $test_err['err'][] = Title(529);
    } // incorrect field name
    if ($row[2] > 1) {
        $test_err['err'][] = FTM(Title(602)).' '.Title(342).' (<b>'.$row[2].'</b>)';
    } // invalid key field
    if ($row[3] == '' && ($row[20] > 0 || $row[21] > 0)) {
        $test_err['err'][] = Title(630);
    } // invalid field name
    if (! isset($_SESSION['field_types'][$row[4]])) {
        $test_err['err'][] = Title(596).' <b>'.(string) $row[4].'/b>';
    } // invalid field type
    if ($row[6] > 1) {
        $test_err['err'][] = Title(596).' <b>'.(string) $row[6].'/b> ('.Title(577).'<b>0</b>'.Title(567).'<b>1</b>)';
    }

    if ($row[7] < 0 || $row[7] > 1) {
        $test_err['err'][] = Title(596).' <b>'.(string) $row[7].'/b> ('.Title(577).'<b>0</b>'.Title(567).'<b>1</b>)';
    }
    if ($row[8] < 0 || $row[8] > 1) {
        $test_err['err'][] = Title(596).' <b>'.(string) $row[8].'/b> ('.Title(577).'<b>0</b>'.Title(567).'<b>1</b>)';
    }
    if ($row[9] < 0 || $row[9] > 1) {
        $test_err['err'][] = Title(596).' <b>'.(string) $row[9].'/b> ('.Title(577).'<b>0</b>'.Title(567).'<b>1</b>)';
    }
    if (! ($row[4] == 8 && isset($_SESSION['reference_catalogs'][$row[10]]) || $row[4] != 8 && $row[10] == '')) {
        $test_err['err'][] = Title(596).' <b>'.(string) $row[10].'/b>';
    }
    if ($row[13] < 0 || $row[13] > 1) {
        $test_err['err'][] = Title(596).' <b>'.(string) $row[13].'/b> ('.Title(577).'<b>0</b>'.Title(567).'<b>1</b>)';
    }
    if ($row[14] < 0 && $row[14] > 1) {
        $test_err['err'][] = Title(596).' <b>'.(string) $row[14].'/b> ('.Title(577).'<b>0</b>'.Title(567).'<b>1</b>)';
    }
    if (! isset($_SESSION['compare_mode'][$row[15]])) {
        $test_err['err'][] = Title(596).' <b>'.(string) $row[15].'/b>';
    }
    if (! isset($_SESSION['sort_mode'][$row[16]])) {
        $test_err['err'][] = Title(596).' <b>'.(string) $row[16].'/b>';
    }
    if ($row[17] != '') {
        $arr = explode(',', $row[17]);
        $arr_err = [];
        foreach ($arr as $z) {
            if (! isset($_SESSION['field_using'][$z])) {
                $arr_err[] = $z;
            }
        }
        if (count($arr_err) > 0) {
            $test_err['err'][] = Title(596).' (<b>'.implode('; ', $arr_err).'</b>)';
        }
    }
    if (! isset($_SESSION['field_align'][$row[18]])) {
        $test_err['err'][] = Title(596).' <b>'.(string) $row[18].'/b>';
    }
    if (! TestPercent($row[19], ['', '%', 'px'])) {
        $test_err['err'][] = Title(596).' <b>'.(string) $row[19].'/b>';
    } // invalid percent
    if ($row[20] < 0) {
        $test_err['err'][] = Title(490).' '.Title(350).' (<b>'.$row[20].'</b>)';
    }
...
function TestPercent($perc, $end_arr)
{
    ...
    foreach ($end_arr as $z) {
        if (RegularComparison($reg_exp = "'^[1-9][0-9]*".$z."$'", $perc)) {
            return true;
        }
    }
...
function SetRepeatFieldValues(&$f_param)
{
    ...
    if (count($arr_rep_name[1]) > 0) {
        foreach ($arr_rep_name[1] as $rep_value => $rep_list) {
            foreach ($rep_list as $rep_id => $rep_ids) {
                $f_param[1][$rep_id][20][] = Tilte(490).' <b>'.Title(347).' ('.$rep_value.')</b> '.Title(587).' '.((count($rep_ids) == 1) ? Title(655) : Title(656)).' <b>'.implode(', ', $rep_ids);
            }
        }
    }
    if (count($arr_rep_name[2]) > 0) {
        foreach ($arr_rep_name[2] as $rep_value => $rep_list) {
            foreach ($rep_list as $rep_id => $rep_ids) {
                $f_param[2][$rep_id][20][] = Tilte(490).' <b>'.Title(347).' ('.$rep_value.')</b> '.Title(587).' '.((count($rep_ids) == 1) ? Title(655) : Title(656)).' <b>'.implode(', ', $rep_ids);
            }
        }
    }
    if (count($arr_rep_screen[1]) > 0) {
        foreach ($arr_rep_screen[1] as $rep_value => $rep_list) {
            foreach ($rep_list as $rep_id => $rep_ids) {
                $f_param[1][$rep_id][20][] = Tilte(490).' <b>'.Title(333).' ('.$rep_value.')</b> '.Title(587).' '.((count($rep_ids) == 1) ? Title(655) : Title(656)).' <b>'.implode(', ', $rep_ids);
            }
        }
    }
    if (count($arr_rep_screen[2]) > 0) {
        foreach ($arr_rep_screen[2] as $rep_value => $rep_list) {
            foreach ($rep_list as $rep_id => $rep_ids) {
                $f_param[2][$rep_id][20][] = Tilte(490).' <b>'.Title(333).' ('.$rep_value.')</b> '.Title(587).' '.((count($rep_ids) == 1) ? Title(655) : Title(656)).' <b>'.implode(', ', $rep_ids);
            }
        }
    }
...
{
    $arr_values = [];
    foreach ($tested_arr as $f => $f_params) {
        if ($f_params[$par_name] != $empty_value) {
            $arr_values[$f] = $f_params[$par_name];
        }
    }
    $arr_value_groups = array_count_values($arr_values);
    foreach ($arr_value_groups as $id => $value_count) {
        if ($value_count == 1) {
            unset($arr_value_groups[$id]);
        }
    }
    $rep_values = [];
    foreach ($tested_arr as $f => $f_params) {
        if (isset($arr_value_groups[$f_params[$par_name]])) {
            $rep_values[$f_params[$par_name]][] = $f;
        }
    }
    $arr_rep = [];
    foreach ($rep_values as $id => $arr_field) {
        $arr_rep[$id] = WriteRepeatError($arr_field);
    }
...
function WriteRepeatError($arr_field)
{
    ...
    for ($i = 0; $i < count($arr_field); $i++) {
        for ($j = 0; $j < count($arr_field); $j++) {
            if ($j != $i) {
                $arr_rep[$arr_field[$i]][] = $arr_field[$j];
            }
        }
    }
...
function SortFieldDefinitions($sort_by) // ??
{
    ...
    foreach (array_keys($_SESSION['field_definitions']) as $t) {
        foreach ($_SESSION['field_definitions'][$t] as $f_c => $f_def) {
            $arr_sort[(string) $t.'|'.$f_c] = SortColumnValue($sort_by, $t, $f_c, $f_def);
        }
    }
    asort($arr_sort);
...
function ViewFieldUsing($str_using) // ??
{
    ...
    foreach ($arr as $z) {
        if (isset($_SESSION['field_using'][$z])) {
            $arr_view[] = $_SESSION['field_using'][$z];
        }
    }
    ...
    foreach ($arr_table as $table) {
        ...
        if ($res_field) {
            while ($row_field = mysqli_fetch_row($res_field)) {
                $arr_all_field[$table][$row_field[0]] = [$row_field[1], $row_field[2], $row_field[3], $row_field[4], $row_field[5], $row_field[6]];
            }
        }
...
function CatalogFieldDefinitions($table, $separators, &$t_def) // ??
{
    ...
    if ($arr_field[0] == '') {
        $t_def[$table]['tab_err'][] = Title(351);
    } else {
        $t_def[$table]['catalog_id'] = $arr_field[0];
    }
    if ($arr_field[1] == '') {
        $t_def[$table]['tab_err'][] = Title(363);
    }
...
{
    if ($v_auto == -1) {
        return [" class='field_no'", Title(213)];
    }
    if ($v_auto == 1) {
        return [" class='field_auto'", Title(258)];
    }
