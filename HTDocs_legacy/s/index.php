...
if (! $dbh) {
    ExitSession('No connection to system data base|FF0000');
}
if ($no_session) {
    $_SESSION['structure_errors'] = [];
    $_SESSION['alarm'] = false;
    $_SESSION['preliminary_flags'] = ['table_errors' => [], 'no_existed_tables' => []];
    ...
    SystemConfigs($dbh);
    $_SESSION['mandatory_language_errors'] = [];
    ...
    if (count($_SESSION['structure_errors']) == 0) {
        ...
        if ($Mes != '') {
            ExitSession($Mes.'`'.'Contact system administrator.');
        }
        if (UserListEmpty($dbh)) {
            ExitSession('User list is empty. Contact system administrator.');
        }
    ...
    if (count($_SESSION['structure_errors']) == 0) {
        header('Location: UserEnter/UserEnterLogin.php');
    } else {
        $_SESSION['alarm'] = true;
        header('Location: UserEnter/UserEnterAlarmLogin.php');
    }
} else {
    ExitSession('You are already logged in to the site in this browser'.'|FF0000', '0', false);
}
?>
