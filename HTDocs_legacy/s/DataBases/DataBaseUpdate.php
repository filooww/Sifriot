    ...
    foreach ($_SESSION['arr_db'] as $k => $v) {
        if ($k != 0) {
            if ($v['del']) {
                ...
                $_SESSION['arr_db'][$k] = ['db_name' => $v['db_name'], 'db_coding' => $_POST['db_coding|'.$k], 'db_comment' => $_POST['db_comment|'.(string) $k], 'del' => false, 'db_err' => TestDatabase($v['db_name'], $_POST['db_coding|'.$k], (string) $k)];
                $arr_ins[] = '('.(string) $k.",'".$v['db_name']."','".$_POST['db_coding|'.(string) $k]."','".$_POST['db_comment|'.(string) $k]."')";
                if (isset($_SESSION['pre_ref']['user_ident']['p']['db']['v'][$k])) {
                    ChangeByInsertInvalidRefTable('user_ident', 'db', $k);
                }
                if (isset($_SESSION['pre_ref']['visits']['p']['db']['v'][$k])) {
                    ChangeByInsertInvalidRefTable('visits', 'db', $k);
                }
            }
        }
    }
    if (isset($_SESSION['pre_ref']['visits']['p']['db']['v'][0])) {
        ChangeByInsertInvalidRefTable('visits', 'db', 0);
    }
    foreach ($arr_del as $z) {
        unset($_SESSION['arr_db'][$z]);
    }
    ...
    if ($conn) {
        ...
        if ($res) {
            while ($row = mysqli_fetch_row($res)) {
                if ($row[0] != $db_else && ! FindDBname($row[0])) {
                    $arr[] = $row[0];
                }
            }
...
{
    foreach ($_SESSION['arr_db'] as $v) {
        if ($v['db_name'] == $db_name) {
            return true;
        }
    }
...
{
    $arr_ref = ['user_ident' => [], 'visits' => [], 'over' => false];
    ...
    if ($res) {
        while ($row = mysqli_fetch_row($res)) {
            if (count($arr_ref['user_ident']) + count($arr_ref['visits']) == $_SESSION['number_warn']) {
                $arr_ref['over'] = true;
            }
...
function SwitchDBDel($dbh, $db_id)
{
    if ($_SESSION['arr_db'][$db_id]['del']) {
        ...
        if ($_SESSION['alarm']) {
            ...
            if (count($arr_ref['user_ident']) + count($arr_ref['visits']) > 0) {
                $_SESSION['del_ref'][$db_id] = SetDBDelMessage($arr_ref);
            }
        } else {
            unset($_SESSION['del_ref']);
            if (count($arr_ref['user_ident']) + count($arr_ref['visits']) == 0) {
...
{
    $db_mes = [];
    if (count($arr_ref['user_ident']) > 0) {
        $db_mes[] = "<font color='#CC0000'><b>".Title(12).'</b>: '.FTM(Title(259)).': '.implode(', ', $arr_ref['user_ident']).'</font>';
    }
    if (count($arr_ref['visits']) > 0) {
        $db_mes[] = "<font color='#CC00CC'><b>".Title(167).'</b>: '.FTM(Title(259)).': '.implode(', ', $arr_ref['visits']).'</font>';
    }
    if ($arr_ref['over']) {
        $db_mes[] = '...';
    }
    if (count($db_mes) == 0) {
        ...
        return implode(', ', $db_mes);
    }
}
function SelectDBServer($i)
{
    if ($_SESSION['db_sel'] == '') {
        $_SESSION['db_sel'] = (string) NewTableID($_SESSION['arr_db'], 0);
    }
    $_SESSION['arr_db'][(int) $_SESSION['db_sel']] = ['db_name' => $_SESSION['db_server'][$i], 'db_coding' => '', 'db_comment' => $_SESSION['db_server'][$i], 'del' => false, 'db_err' => []];
    $_SESSION['db_server'] = GetServerDatabases('db_manager');
}
function SaveDBPost()
{
    foreach ($_SESSION['arr_db'] as $k => $v) {
        if (isset($_POST['db_coding|'.(string) $k])) {
            $_SESSION['arr_db'][$k]['db_coding'] = $_POST['db_coding|'.(string) $k];
        }
        if (isset($_POST['db_comment|'.(string) $k])) {
            $_SESSION['arr_db'][$k]['db_comment'] = $_POST['db_comment|'.(string) $k];
        }
    }
}
function SortDBList()
{
    $arr_by_name = [];
    foreach ($_SESSION['arr_db'] as $k => $v) {
        $arr_by_name[$v['db_name']] = ['key' => $k, 'db_coding' => $v['db_coding'], 'db_comment' => $v['db_comment'], 'del' => $v['del'], 'db_err' => $v['db_err']];
    }
    ksort($arr_by_name);
    $sort_db = [];
    foreach ($arr_by_name as $k => $v) {
        $sort_db[$v['key']] = ['db_name' => $k, 'db_coding' => $v['db_coding'], 'db_comment' => $v['db_comment'], 'del' => $v['del'], 'db_err' => $v['db_err']];
    }
