...
{
    if (count($_SESSION['arr_db'][$k]['db_err']) == 0) {
        return true;
    } elseif (count($_SESSION['arr_db'][$k]['db_err']) == 1 && isset($_SESSION['pre_db_err'][$k]) && $_SESSION['arr_db'][$k]['db_err'][0] == "<font color='#0000FF'>".$_SESSION['pre_db_err'][$k].'</font>') {
        return true;
    }
...
function TestDBList(&$Mes, $mes_err = false)
{
    ...
    if (! $fl_test && $mes_err) {
        $Mes[] = "<font color='#FF0000'>".Title(471).'</font>';
    }
...
function TestDatabase($db_name, $db_coding, $db_id, $db_del = false)
{
    ...
    for ($i = 0; $i < count($err_arr); $i++) {
        if ($err_arr[$i] == Title(1)) {
            $err_arr[$i] = "<font color='#FF0000'>".FTM($err_arr[$i]).'</font>';
        } else {
            $err_arr[$i] = "<font color='#FF0000'>".$err_arr[$i].'</font>';
        }
    }
    if ((int) $db_id < 0) {
        $err_arr[] = "<font color='#FF0000'>".Title(532).' ('.Title(589).' <b>'.$db_id.'</b>, '.Title(591).") <button name='correct_id|".$db_id."' type='submit' class='i_h' title='".Title(590).' '.FTM(Title(147))."'>...</button>";
    }
    if (isset($_SESSION['del_ref'][$db_id])) {
        $err_arr[] = $_SESSION['del_ref'][$db_id];
        $fl_test = false;
    }
    if (isset($_SESSION['pre_db_err'][$db_id])) {
        $err_arr[] = "<font color='#0000FF'>".$_SESSION['pre_db_err'][$db_id].'</font>';
    }
...
function TestUserDB($db_name, $db_coding, $db)
{
    ...
    if ($dbh) {
        ...
        if (count($_SESSION['structure_errors']) == 0) {
            ...
            if (count($db_pre_flags) > 0) {
                $_SESSION['db_errors'][] = FTM(Title(592)).' '.((count($db_pre_flags) == 1) ? Title(184) : Title(186)).' <b>'.implode(', ', $db_pre_flags).'</b>';
            }
...
function TestDBServiceTableCheck($dbh, $table, $req_str, &$db_pre_flags)
{
    ...
    if ($res) {
        if ($row = mysqli_fetch_row($res)) {
            if ($row[0] > 0 && ! in_array($table, $db_pre_flags)) {
                $db_pre_flags[] = $table;
            }
        }
...
function TestDBFieldCheck($dbh, &$db_pre_flags)
{
    ...
    if ($res) {
        if ($row = mysqli_fetch_row($res)) {
            if ($row[0] > 0 && ! in_array('field_config', $db_pre_flags)) {
                $db_pre_flags[] = 'field_config';
            }
        }
        mysqli_free_result($res);
    }
...
{
    $a = [];
    for ($i = 1; $i < count($_SESSION['field_using']); $i++) {
        $a[] = $i;
    }
    $usings_arr = [];
    $permutations = [];
    ...
    foreach ($permutations as $permutation) {
        $usings_arr[count($_SESSION['field_using'])] = $permutation;
    }
    for ($p_size = count($_SESSION['field_using']) - 1; $p_size > 0; $p_size--) {
        ...
        foreach ($permutations as $permutation) {
            ...
            $x = implode(',', $p_s);
            if (! in_array($x, $usings_arr[$p_size])) {
                $usings_arr[$p_size][] = $x;
            }
        }
    }
    $permutation_arr = $permutations;
    for ($p_size = 3; $p_size > 0; $p_size--) {
        $permutation_arr = array_merge($permutation_arr, $usings_arr[$p_size]);
    }
...
function heapPermutation(&$a, $size, &$result)
{
    if ($size == 1) {
        ...
        for ($i = 0; $i < $size; $i++) {
            ...
            if ($size % 2 == 0) {
                ChangeElements($a, $i, $size - 1);
            } else {
                ChangeElements($a, 0, $size - 1);
            }
        }
    }
}
