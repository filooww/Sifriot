...
{
    $tables = array("algorithms", "coding_table", "db_list", "db_s_configs", "interface_special_texts", "interface_texts", "languages", "translate_table", "user_ident", "visits");
	$all_tables = array();
	...
	foreach ($tables as $k_table) if (!in_array($k_table, $all_tables)) $_SESSION['preliminary_flags']['no_existed_tables'][$k_table] = CreateSystemTable($dbh, $k_table);
}
function CreateSystemTable($dbh, $table)
{
	...
	switch ($table)
	{
        case "algorithms"              : $comm = CreateAlgorithms(); break;
        case "coding_table"            : $comm = CreateCoding(); break;
		case "db_list"                 : $comm = CreateDBList(); break;
		case "db_s_configs"            : $comm = CreateDBSConfig("db_s_configs"); break;
		case "interface_texts"         : $comm = CreateInterfaceTexts(); break;
		case "languages"               : $comm = CreateLanguages(); break;
		case "interface_special_texts" : $comm = CreateInterfaceSpecialTexts(); break;
		case "translate_table"         : $comm = CreateTranslateTable(); break;
		case "user_ident"              : $comm = CreateUserIdent(); break;
		case "visits"                  : $comm = CreateVisits(); break;
	}
	if (count($comm) > 0)
    {
