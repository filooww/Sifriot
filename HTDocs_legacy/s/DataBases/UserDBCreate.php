...
{
    $tables = ['db_configs', 'field_config', 'table_definitions'];
    $all_tables = [];
    ...
    if (count($add_tables) > 0) {
        $_SESSION['db_errors'][] = ((count($add_tables) == 1) ? FTM(Title(365)) : FTM(Title(115))).' <b>'.implode(', ', $add_tables).'</b> '.((count($add_tables) == 1) ? Title(536) : Title(712));
    }
...
function CreateServiceTable($dbh, $table)
{
    $comm = [];
    switch ($table) {
        case 'db_configs': $comm = CreateDBSConfig('db_configs');
            break;
        case 'field_config': $comm = CreateDataBaseFieldConfigs();
            break;
        case 'table_definitions': $comm = CreateDataBaseTableConfigs();
            break;
    }
    if (count($comm) > 0) {
...
function CreateBasicTable($f_type)
{
    ...
    foreach ($_SESSION['field_definitions'][$f_type] as $k => $v) {
        $arr_field[] = $k;
        switch ($v[6]) {
            case 0: $arr_field[] .= ' int unsigned';
                break;
            case 1: $arr_field[] .= ' varchar(255) CHARACTER SET utf8 COLLATE utf8mb3_bin';
                break;
            case 2: $arr_field[] .= ' date';
                break;
            case 3: $arr_field[] .= ' int unsigned';
                break;
            case 4: $arr_field[] .= ' tinyint unsigned';
                break;
            case 5: $arr_field[] .= ' varchar(255) CHARACTER SET utf8 COLLATE utf8mb3_bin';
                break;
            case 6: $arr_field[] .= ' varchar(255) CHARACTER SET utf8 COLLATE utf8mb3_bin';
                break;
            case 7: $arr_field[] .= ' tinyint unsigned';
                break;
            case 8: $arr_field[] .= ' int unsigned';
                break;
...
function CreateCatalog($table)
{
    ...
    if ($_SESSION['table_definitions'][$table]['second_catalog'] == '') {
        $crt .= $_SESSION['table_definitions'][$table]['catalog_value'].'_low varchar(255) CHARACTER SET utf8 COLLATE utf8mb3_bin NOT NULL) ';
    }
    $crt .= 'ENGINE=InnoDB DEFAULT CHARSET=utf8;';
