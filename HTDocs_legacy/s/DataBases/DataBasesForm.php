...
foreach ($_POST as $str_key => $str_v) {
    ...
    switch ($s_k) {
        ...
        case 'user_lang_s': TestDBList($Mes);
            AfterLangChoice($dbh, 'user_lang_s', 'user_lang', $sw_break);
            break;
        case 'DB_exit': $goto = DataBaseListExit($dbh, $Mes);
            if ($goto != '') {
                header('Location: '.$goto.'.php');
            } break;
        case 'DB_save': RestDBValues();
            if (TestDBList($Mes, (! $_SESSION['alarm']))) {
                RewriteDB($dbh, $Mes);
            } break;
        case 'DB_new': RestDBValues();
            $_SESSION['server_table'] = true;
            break;
        case 'db_mark_del': RestDBValues();
            SwitchDBDel($dbh, (int) $a[1]);
            TestDBList($Mes);
            break;
        case 'db_select': RestDBValues();
            $_SESSION['server_table'] = true;
            $_SESSION['db_sel'] = $a[1];
            break;
        case 'server_db_select': SaveDBPost();
            SelectDBServer((int) $a[1]);
            $_SESSION['server_table'] = false;
            TestDBList($Mes);
            break;
        case 'exit_server': RestDBValues();
            SaveDBPost();
            $_SESSION['server_table'] = false;
            TestDBList($Mes);
            break;
        case 'correct_id': RestDBValues();
            CorrectDBID($dbh, (int) $a[1]);
            TestDBList($Mes);
            break;
	...
	<table>
		<tr>
            ...
            if ($_SESSION['user_working_mode'] == 1) {
                ...
                if (count($_SESSION['db_server']) > 0) {
                    echo "<td><button name='DB_new' type='submit' class='add_button'".$f_other.'>'.Title(158).'</button></td>';
                }
            }
?>
		</tr>
...
if ($_SESSION['server_table']) {
    require_once 'DataBasesSelectTable.php';
} else {
    require_once 'DataBasesTable.php';
}
?>
<form>
