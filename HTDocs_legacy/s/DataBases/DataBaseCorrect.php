...
function CorrectDBList($dbh_sys)
{
    ...
    if ($fl_change) {
        ...
        foreach ($_SESSION['arr_db'] as $k => $v) {
            $ins_arr[] = '('.(string) $k.",'".$v['db_name']."','".$v['db_coding']."','".$v['db_comment']."')";
        }
...
function AddInvalidDB($v)
{
    ...
    $_SESSION['arr_db'][$new_id] = ['db_name' => $v.'~', 'db_coding' => $_SESSION['arr_db'][0]['db_coding'], 'db_comment' => $_SESSION['arr_db'][0]['db_comment'], 'del' => false, 'db_err' => TestDatabase($v.'~', $_SESSION['arr_db'][0]['db_coding'], (string) $new_id)];
...
{
    $_SESSION['arr_db'][0] = ['db_name' => 'db_manager', 'db_coding' => 'utf8', 'db_comment' => 'system DB', 'del' => false, 'db_err' => []];
    ksort($_SESSION['arr_db']);
