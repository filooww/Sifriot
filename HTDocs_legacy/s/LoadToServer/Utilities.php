...
function NoHTNLChar($arr_piece, &$arr_code, $last_index)
{
	...
	{
		if (substr($arr_piece[$j], 0, 2) == "&#" && is_numeric(substr($arr_piece[$j], 2))) return $j;
		$arr_code[] = ord($arr_piece[$j]);
	}
	if (!$last_index) $arr_code[] = ord(";");
	return -1;
}
function UploadFileNames($file_name, $file_tmp = "", $file_error = 0)
{
	...
	for ($n = 0; $n < count($arr_str); $n++)
	{
		...
		elseif (implode("", array_slice($arr_str, $n, 2)) == "&#")
		{
			...
			{
				$num_code = (integer)$str_code;
				$ind_code = array_search($num_code, $_SESSION['trans_codes']['html_code']);
				if ($ind_code === false) $arr_code[] = ord($arr_str[$n]);
				else
				{
	...
	if ($file_tmp == "") return $arr_code;
	else return array("tmp"=>$file_tmp, "name"=>$arr_code, "err_ind"=>$file_error);
}
function SingleFileUpload($s_file, $d_file, $file_name)
{
	if (ResExists($s_file))
	{
		...
		else return array("time"=>GetMessageDate(), "text"=>"Error while file <b>".$file_name."</b> uploaded into destination<br>(on the server this file has the name <b>".$d_file."</b>)<br>", "status"=>"error");
	}
	else return array("time"=>GetMessageDate(), "text"=>"File <b>".$file_name."</b> not exists<br>", "status"=>"error");
}
function Transit($mess, $act_script)
{
	...
	echo "<p class='row_invisible' align='center'>S</p>";
	echo "<p class='row_invisible' align='center'>S</p>";
	if (is_array($mess)) foreach ($mess as $v) echo "<p align='center'><font color='#FF0000'><b>".$v."</b></font></p>";
	else echo "<p align='center'><font color='#FF0000'><b>".$mess."</b></font></p>";
	echo "<p class='row_invisible' align='center>S</p>";
	echo "<p class='row_invisible' align='center'>S</p>";
