...
session_start();
$_SESSION['trans_mess'] = array();
if ($_SESSION['mult'])
{
	$s = array_sum($_FILES['filename']['size']);
	$m = TransSize(ini_get('upload_max_filesize'));
	$l = TransSize(ini_get('max_file_uploads'));
	if ($s > $m) $_SESSION['trans_mess'][] = "Total size of uploaded files exceeds the directive in php.ini (".(string)$m.")";
	if (CountOfNoEmptyItems($_FILES['filename']['name']) > $l) $_SESSION['trans_mess'][] = "Number of loaded files exceeds the directive in php.ini (".(string)$l.")";
}
	...
	if ($_SESSION['mult'])
	{
		...
		for ($i = 0; $i < count($_FILES['filename']['name']); $i++) 
		{
			if ($_FILES['filename']['name'][$i] != "") 
			{
				...
				if ($_FILES['filename']['error'][$i] == 0) $correct_files++;
				else $incorrect_files++;
			}
		}
	}
	else $_SESSION['load_files'] = UploadFileNames($_FILES['filename']['name'], $_FILES['filename']['tmp_name'], $_FILES['filename']['error']);
	if ($correct_files == 0)
	{
...
else header("Location: Transit.php");
?>
