...
function DeletePub($dbh, $main_table, $attach, $PR, &$start_pos, $conf, &$p_code, &$p_count, &$item_arr, $URL_path)
{
	...
	if ($k_f != "" && $d_m != "")
	{
		...
		if ($i < count($arr_keys) - 1) $n_code = (integer)$arr_keys[$i + 1];
		elseif (count($arr_keys) < $_SESSION['portion'])
		{
			if ($i > 0) $n_code = (integer)$arr_keys[$i - 1];
			else 
			{
				$n_code = 0;
				$start_pos--;
		...
		else $n_code = (integer)$arr_keys[$i - 1];
		...
		{
			DeleteFilesDir($URL_path, $p_code, array());
			$p_count['total']--;
...
function DeleteAllMarked($dbh, $main_table, $attach, $p_con, $URL_path, &$p_count, &$item_arr, &$p_code)
{
	...
	if ($k_f != "" && $d_m != "")
	{
		...
		{
			$arr_del = array();
			while ($row = mysqli_fetch_row($res)) $arr_del[] = (string)$row[0];
...
function DeleteFilesList($del_dir, $item_id_arr)
{
	...
	if ($dir)
	{
		while (($file = readdir($dir)) !== false) 
		{
			...
			$cnt = count($arr_f);
			if ($cnt > 1 && in_array($arr_f[0], $item_id_arr)) unlink($del_dir."/".$file);
		}
		closedir($dir);
	}
}
