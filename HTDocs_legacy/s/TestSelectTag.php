...
{
    if (isset($_SESSION['titles'][(string) $title_id])) {
        return $_SESSION['titles'][(string) $title_id];
    } else {
        return 'Missing text '.(string) $title_id;
    }
}
...
function AfterLangChoice($dbh_sys, $lang_name)
{
    ...
    if ($lang_key !== false) {
        ...
        $_SESSION['titles'] = GetTitlesByLanguage($dbh_sys, (int) $lang_key);
    }
}
function ArrayToFile($f_name, $k, $a, $o)
{
    ...
    if (is_array($a)) {
        ...
        $os = $o + 2;
        foreach ($a as $ka => $va) {
            ArrayToFile($f_name, $ka, $va, $os);
        }
    } else {
        $f = false;
        switch (gettype($a)) {
            case 'boolean': $b = TestBool($a);
                $f = true;
                break;
            case 'integer': $b = (string) $a;
                $f = true;
                break;
            default: $b = $a;
                $f = true;
        ...
        if ($f) {
            fwrite($log_file, str_repeat('--', $o).$k.' => '.$b."\r\n");
        }
        fclose($log_file);
    }
}
...
$v = ['use_type' => 0, 'illegals' => '', 'max_level' => 0, 'separators' => '', 'catalog_id' => '', 'catalog_value' => '', 'group_type' => 0, 'second_catalog' => '', 'table_title' => '', 'low_fields' => '', 'tab_err' => []];
$dis = '';
?>
