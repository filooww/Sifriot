...
{
    if ($w_mode < -1 || $w_mode > 1) {
        return (string) $w_mode;
    }
    if ($w_mode == 0) {
        return $v;
    }
...
function UpdateVisitSet($working_mode, $v_count = 0)
{
    ...
    if ($working_mode >= 0) {
        $str_update[] = 'visit_count='.(string) ($v_count + 1);
    }
...
function VisitParameters($dbh, $id_db, $working_mode, $priority)
{
    if ($working_mode == -1) {
        if ($priority > 0) {
            mysqli_query($dbh, 'UPDATE visits SET '.UpdateVisitSet($working_mode).' WHERE id_db = '.(string) $id_db.' AND id_user = '.$_SESSION['user_id']);
        } else {
            CancelGuest($dbh, $id_db, $working_mode);
        }
    } else {
        $v_c = VisitAnalysis($dbh, $id_db);
        if ($v_c == 0) {
            mysqli_query($dbh, 'INSERT INTO visits VALUES ('.(string) $id_db.','.$_SESSION['user_id'].',NOW(),1,'.(string) $working_mode.",'".session_id()."',0)");
        } else {
            mysqli_query($dbh, 'UPDATE visits SET '.UpdateVisitSet($working_mode, $v_c).' WHERE id_db = '.(string) $id_db.' AND id_user = '.$_SESSION['user_id']);
        }
    }
}
...
function ReplaceVisitIDs($dbh, $k_type, $k_r)
{
    if ($k_type == 'db') {
        ...
        if (! $fl) {
            SetVisitReplaceMessage($dbh, $k_type, $k_r, 'id_db', 'id_user');
        }
    } else {
        ...
        if ($fl) {
            ...
            if (! $fl) {
                SetVisitReplaceMessage($dbh, $k_type, $k_r, 'id_user', 'id_db');
            }
        } else {
            $_SESSION['replace_mes'] = ['#FF0000', -602, ' ', 544, ' <b>'.$_POST['replacing_id|ui|'.$k_r], '</b>'];
        }
    }
    if ($fl) {
...
{
    $fl = false;
    if (is_numeric($_POST['replacing_id|ui|'.$k]) && (int) ($_POST['replacing_id|ui|'.$k]) > 0) {
...
function SetVisitReplaceMessage($dbh, $k_type, $k, $field1, $field2)
{
    ...
    if (count($no_changed) > 0) {
        ...
        $_SESSION['replace_mes'] = ['#FF0000', 569, ' <b>'.$no_change_list.'</b> ', 673, ' <b>'.$err_list.'</b> ', 674, ' <b>'.$_POST['replacing_id|'.$k_type.'|'.$k].'</b>'];
    }
}
...
function ResetFilter()
{
    ...
    $_SESSION['expired_date'] = [1, 1, (int) $_SESSION['start_year']];
    $_SESSION['ses_type'] = 0;
