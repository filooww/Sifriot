...
function ChangeVisitScreen($dbh, $offs, &$Mes)
{
	if (gettype($offs) == "integer")
	{
		...
		{
			if ($_SESSION['portion'] > count($_SESSION['visit_list'])) $_SESSION['portion']--;
			elseif ($_SESSION['portion'] == 1) $Mes[] = "<font color='#FF0000'>".Title(513)."</font>";
			elseif (end($_SESSION['visit_list'])[7] > 0) $Mes[] = "<font color='#FF0000'>".Title(513)."</font>";
			else
			{
				if (count($_SESSION['visit_list']) == $_SESSION['portion']) array_pop($_SESSION['visit_list']);
				$_SESSION['portion']--;
			}
		}
	}
	...
	{
		if (!is_numeric($offs)) $Mes[] = "<font color='#FF0000'>".Title(177)." ".Title(77)." (".Title(299)." <b>".$offs."</b>)</font>";
		elseif ((integer)$offs > $_SESSION['portion'])
		{
			$old = $_SESSION['portion'];
			$_SESSION['portion'] = (integer)$offs;
			if (count($_SESSION['visit_list']) >=  $old) $_SESSION['visit_list'] = GetVisitPortion($dbh, $_SESSION['visit_list']);
		}
		elseif ((integer)$offs < 1 || strpos($offs, "-") !== false || strpos($offs, ".") != false) $Mes[] = "<font color='#FF0000'>".Title(177)." ".Title(512)." (".Title(299)." <b>".$offs."</b>)</font>";
		elseif ((integer)$offs > $_SESSION['portion'])
		{
			$_SESSION['portion'] = (integer)$offs;
			$_SESSION['visit_list'] = GetVisitPortion($dbh, $_SESSION['visit_list']);
		}
		elseif ((integer)$offs < count($_SESSION['visit_list'])) SetVisitPortionSize((integer)$offs, $Mes);
		else $_SESSION['portion'] = (integer)$offs; 
	}
}
function SetVisitPortionSize($int_offs, &$Mes)
{
	if (count($_SESSION['visit_list']) < $_SESSION['portion']) $_SESSION['portion'] = count($_SESSION['visit_list']);
	...
	if ($_SESSION['portion'] > $int_offs) $Mes[] = "<font color='#FF0000'>".Title(513)."</font>";
}
function IsMarkedSessions($mark_type)
{
	foreach ($_SESSION['visit_list'] as $k => $v) if ($v[7] == $mark_type) return true;
    return false;
}
	...
	foreach ($_SESSION['visit_list'] as $k => $v)
	{
		if ($v[7] == 2)
		{
			...
			$deleted_set[] = "id_db = ".$arr_key[0]." AND id_user = ".$arr_key[1];
 		    if (isset($_SESSION['pre_ref']['visits']['p']['db']['v'][(string)$arr_key[0]])) ChangeInvalidRefTable($dbh, "visits", "db", (string)$arr_key[0], (string)$arr_key[1]);
		    if (isset($_SESSION['pre_ref']['visits']['p']['ui']['v'][(string)$arr_key[1]])) ChangeInvalidRefTable($dbh, "visits", "ui", (string)$arr_key[1], (string)$arr_key[0]);
		}
	}
...
function DeleteSessionFromUserList($dbh, $del_arr, $id_user)
{
    ...
    else
    {
	    ...
	    {
	        if (isset($_SESSION['pre_ref']['visits']['p']['db']['v'][(string)$db])) ChangeInvalidRefTable($dbh, "visits", "db", (string)$db, (string)$id_user);
	        if (isset($_SESSION['pre_ref']['visits']['p']['ui']['v'][(string)$id_user])) ChangeInvalidRefTable($dbh, "visits", "ui", (string)$id_user, (string)$db);
	    }
	}
}
...
function MarkVisit($dbh, $db_id, $user_id)
{
    ...
    {
        case  0: $_SESSION['visit_list'][$k][7] = (strpos($_SESSION['visit_list'][$k][9]['str'], "<font color='#0000FF'>".Title(516)."</font>") === false) ? 2 : 1; break;
        case  1: $_SESSION['visit_list'][$k][7] = 2; break;
        case  2: $_SESSION['visit_list'][$k][7] = 0; break;
        default: $fl = false;
    }
    if ($fl && $_SESSION['cancel_started'])
	{
		...
		else mysqli_query($dbh, "UPDATE visits SET rest_time = 0 WHERE id_db = ".$db_id." AND id_user = ".$user_id);
	}
}
function ExpiredView($dis_suspend)
{
    for ($i = 1; $i < 4; $i++)
    {
        ...
        {
            case "d" : CalendSelect("expired_day", $_SESSION['expired_date'][0], 1, CalendMaxDay($_SESSION['expired_date'][1], $_SESSION['expired_date'][2]), "exp_d_s","class_day", "exp_d_on", $dis_suspend, FTM(Title(29))); break;
            case "m" : CalendSelect("expired_month", $_SESSION['expired_date'][1], 1, 12, "exp_m_s", "class_month", "exp_m_on", $dis_suspend, FTM(Title(28))); break;
            case "y" : CalendSelect("expired_year", $_SESSION['expired_date'][2], (integer)$_SESSION['start_year'], (integer)$_SESSION['current_year'], "exp_y_s","class_year", "exp_y_on", $dis_suspend, FTM(Title(27))); break;
        }
    }
}
function AfterVisitLangChoice($dbh, &$sw_break)
{
	...
	if (AfterLangChoice($dbh, "user_lang_s", "user_lang", $sw_break))
	{
		foreach ($_SESSION['visit_list'] as $k => $v)
		{
			...
			if ($v[9]['str'] != "") $_SESSION['visit_list'][$k][9]['str'] = $visit_errors['str'];
		}
...
function VisitExit($dbh, &$Mes)
{
	...
	if ($_SESSION['alarm'])
	{
		...
		if (isset($_SESSION['replace_mes'])) unset($_SESSION['replace_mes']);
        return "../Alarm/CommonAlarmForm";
	}
   	if ($_SESSION['user_working_mode'] == 1)
    {
        $err_flags = TestVisitList();
        if ($err_flags['del']) $Mes[] = "<font color='#FF0000'><b>".FTM(Title(555))."</b></font>";
        if ($err_flags['err']) $Mes[] = "<font color='#FF0000'><b>".FTM(Title(223))."</b></font>";
        if ($err_flags['del'] || $err_flags['err']) return "";
    }
    return "../Administrator/MainForm";
}
function VisitSavePost()
{
	if (isset($_POST['visit_db']) && isset($_POST['visit_db_id'])) $_SESSION['filter_db'] = array($_POST['visit_db_id'], $_POST['visit_db']);
	if (isset($_POST['visit_user'])) $_SESSION['filter_user'] = $_POST['visit_user'];
	if (isset($_POST['visit_user_id'])) $_SESSION['filter_user_id'] = $_POST['visit_user_id'];
	if (isset($_POST['visit_category'])) $_SESSION['category'] = SavePostSelect($_SESSION['categories'], $_POST['visit_category'], array(169, 135, 136, 137, 232));
	if (isset($_POST['expired_day']) && isset($_POST['expired_month']) && isset($_POST['expired_year'])) $_SESSION['expired_date'] = array($_POST['expired_day'], $_POST['expired_month'], $_POST['expired_year']);
	if (isset($_POST['session_type_radio'])) $_SESSION['ses_type'] = (integer)$_POST['session_type_radio'];
}
function ResetSessionParameters($dbh, $id_db, $id_user)
{
    ...
    if (count($set_update) > 0)
    {
        ...
        if ($fl_work) $_SESSION['admin_mes'] = array(0, "");
...
function TimeCancelSessions($dbh) // inner
{
	...
	if (count($all_sessions) > 0)
	{
		if ($_SESSION['t_rest'] == "0")
		{
	        ...
	        $cancel_sessions = array();
            for ($i = 0; $i < count($all_sessions); $i++)
            {
                if ($sessions_type[$i] == 0) $delete_sessions[] = $all_sessions[$i];
                else $cancel_sessions[] = $all_sessions[$i];
            }
            if (count($cancel_sessions) > 0)
            {
			    mysqli_query($dbh, "UPDATE visits SET ".UpdateVisitSet(-1)." WHERE ".implode(" OR ", $cancel_sessions));
			    foreach ($_SESSION['visit_list'] as $k => $v) if ($v[7] == 1) RestoreSession($k);
            }
		...
		else mysqli_query($dbh, "UPDATE visits SET rest_time = ".$_SESSION['t_rest']." WHERE ".implode(" OR ", $all_sessions));
    }
	return $fl;
}
function CalcRestTime()
{
	if (isset($_POST['t_curr']) && $_POST['t_curr'] != "")
	{
		...
		if (strlen($rest_time) < 4) $_SESSION['t_rest'] = "0";
		else
		{
			$_SESSION['t_rest'] = substr($rest_time, 0, -3);
			if (substr($rest_time, -3) >= "500") $_SESSION['t_rest'] = bcadd($_SESSION['t_rest'], 1, 0);
			if ((integer)$_SESSION['t_rest'] % (integer)$_SESSION['reminder_interval'] != 0) $_SESSION['t_rest'] = (string)(round((integer)$_SESSION['t_rest'] / (integer)$_SESSION['reminder_interval']) * (integer)$_SESSION['reminder_interval']);
		}
		if (bccomp($rest_time, 1000) < 0) $_SESSION['t_rest'] = "0";
	}
}
