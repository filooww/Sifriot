...
if (count($_POST) == 0) VisitInitial($dbh);
else
{
	$_SESSION['t_start'] = $_POST['t_start'];
	$_SESSION['t_curr'] = $_POST['t_curr'];
	if ($_SESSION['cancel_started'] && TimeCancelSessions($dbh)) $_SESSION['visit_list'] = GetVisitPortion($dbh, array());
	VisitSavePost();
}
foreach ($_POST as $str_key => $str_v)
{
	...
	switch ($s_k)
	{
		...
		case "user_lang_s"			: if (AfterVisitLangChoice($dbh, $sw_break)) VisitFilter($dbh, false); break;
		case "replace_s"		    : if (AfterReplaceSelect(array("db"=>$_SESSION['visit_db_list'], "ui"=>array()), $sw_break)) VisitFilter($dbh, false); break;
		case "category_s"			: if (AfterFilterSelect("category_s", $sw_break)) VisitFilter($dbh); break;
		case "visit_db_s"			: AfterDBFilterSelect($sw_break); break;
		case "session_type_s"		: if (AfterFilterSelect("session_type_s", $sw_break)) VisitFilter($dbh); break;
        case "exp_d_s"		        : if (AfterFilterSelect("exp_d_s", $sw_break)) VisitFilter($dbh); break;
        case "exp_m_s"              : if (AfterFilterSelect("exp_m_s", $sw_break)) VisitFilter($dbh); break;
        case "exp_y_s"              : if (AfterFilterSelect("exp_y_s", $sw_break)) VisitFilter($dbh); break;
		case "visit_exit"			: $goto = VisitExit($dbh, $Mes); if ($goto != "" && count($Mes) == 0) header("Location: ".$goto.".php"); break;
		case "visit_height_minus"	: ChangeVisitScreen($dbh, -1, $Mes); break;
		case "visit_height_plus"	: ChangeVisitScreen($dbh, 1, $Mes); break;
		case "visit_height_button"	: if (isset($_POST['visit_height'])) ChangeVisitScreen($dbh, $_POST['visit_height'], $Mes); break;
        case "visit_navigation"		: VisitListNavigation($dbh, $arr_key[1], $Mes); break;
        case "visit_filter"			: VisitFilter($dbh); break;
		case "reset_session"		: ResetSessionParameters($dbh, $arr_key[1], $arr_key[2]); break;
		case "suspend_session"		: MarkVisit($dbh, $arr_key[1], $arr_key[2]); break;
		case "visit_cancel"			: StartCancelSessions($dbh, $Mes); break;
		case "visit_delete"			: if (DeleteSessions($dbh, $Mes)) $_SESSION['visit_list'] = GetVisitPortion($dbh, $_SESSION['visit_list']); break;
        case "visit_references"	    : GetInvRef("visits", array("db"=>$_SESSION['visit_db_list'], "ui"=>array())); VisitFilter($dbh, false); break;
        case "replace_id"           : $_SESSION['replace_mes'] = array(); ReplaceVisitIDs($dbh, $arr_key[1], $arr_key[2]); break;
...
if ($_SESSION['cancel_started'] && $is_suspend) echo "<font color='#FF0000'>".Title(416)." <b>".Title(415)."</b>. ".Title(417)." (<b>".$_SESSION['t_rest']."</b> ".Title(414).")</font>";
if (!$is_suspend) $_SESSION['cancel_started'] = false;
...
<form method="post" id="visit_form" name="visit_form">
    ...
    if ($_SESSION['inv_ref'] && PermitInvRef("visits", array("db", "ui")) && !IsMarkedSessions(2) && $_SESSION['visit_filter'] == "") InvRefTable("visits", "emph_data", array("db"=>$_SESSION['visit_db_list'], "ui"=>array()));
    ?>
	<table>
		<tr>
			...
			<td><font class="cell_inv">X</font></td>
			<?php if (PermitInvRef("visits", array("db", "ui"))) echo "<td><button name='visit_references' type='submit' value='*' class='i_r'".$dis_filter_suspend.">".Title(541)."</button></td>";?>
		</tr>
	</table>
	<hr align="left" size="1" noshade="noshade" color="#000000" >
	<table>
		<tr>
			...
			<td>
                <?php
                echo Title(425).": ";
                RadioTag("session_type_radio", $_SESSION['ses_type'], array(Title(524), Title(518), Title(526)), "hidden_button", false, "session_type_radio_on", "session_type_s", $_SESSION['cancel_started']);
    ...
    <table>
        <tr>
			<td><?php echo FTM(Title(163));?> <?php SelectTag("visit_db", array_merge(array(""=>""), $_SESSION['visit_db_list']), $_SESSION['filter_db'][0], "visit_db_s", true, "", "visit_db_on", $_SESSION['cancel_started']);?></td>
    ...
    <hr align="left" size="1" noshade="noshade" color="#000000" >
	<?php
		if (count($_SESSION['visit_list']) == 0) echo "<table align='center'><tr><td><font size='+2'><b>".Title(227)." ".Title(226)."</b></font></td></tr></table>";
		else require_once("VisitTable.php");
	?>
