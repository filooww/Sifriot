...
function SetCollapse($dbh, $collapse_id, $collapse_set, $collapse_value, $settings_pad)
{
    ...
    if (count($arr_err) > 0) {
        $_SESSION['mes']['0'][] = ['time' => '', 'text' => IsChangesAndErrors('0', Title(486).' ', ' '.Title(470)), 'status' => 'error', 'now' => true];
    } else {
        ...
        if ($collapse_value == '-') {
            if (! SingleNode($dbh, $collapse_set)) {
                ...
                $_SESSION['collapse'][$_SESSION['Catalog']['0']['table']][(string) $collapse_id] = ['set' => $collapse_set, 'count' => GetCollapseCount($dbh, $collapse_set)];
            ...
            if ($res) {
                while ($row = mysqli_fetch_row($res)) {
                    if ($son_count > 0) {
                        $_SESSION['collapse'][$_SESSION['Catalog']['0']['table']][(string) $row[0]] = ['set' => $row[1], 'count' => GetCollapseCount($dbh, $row[1])];
                    }
                }
                mysqli_free_result($res);
                $_SESSION['catalog_param']['0']['tree_total_count'] = TreeCounts($_SESSION['catalog_param']['0']['total_count']);
...
{
    if (in_array($node_id, array_keys($arr_collapse))) {
        return 1;
    }
    $arr = explode(',', $ref_ids);
    for ($i = 0; $i < count($arr) - 1; $i++) {
        if (in_array(implode(',', array_slice($arr, 0, $i + 1)), $arr_collapse)) {
            return -1;
        }
    }
...
function GetSonsArray($cat_arr, $v_set)
{
    $sons_inner = [];
    foreach ($cat_arr as $k => $v) {
        if (strpos($v[3], $v_set.',') !== false && strpos($v[3], $v_set.',') == 0) {
            $sons_inner[$k] = [$v[3], $v[4]];
        }
    }
...
function GetNodeSons($dbh, $sons_inner)
{
    ...
    if (count($screen_sons) > 0) {
        $q_text .= ' AND '.$_SESSION['Catalog']['0']['id'].' NOT IN ('.implode(',', $screen_sons).')';
    }
    $res = mysqli_query($dbh, $q_text);
    if ($res) {
        while ($row = mysqli_fetch_row($res)) {
            $sons_outer[(string) $row[0]] = [$row[1], GetQueryValueSet(2, $row, $max_lev)];
        }
        mysqli_free_result($res);
    }
