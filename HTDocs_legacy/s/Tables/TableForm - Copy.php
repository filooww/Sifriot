...
foreach ($_POST as $str_key => $str_v) {
    ...
    switch ($s_k) {
        ...
        case 'group_symbol': $_SESSION['table_definitions'][$k[1]]['illegals'] .= $_SESSION['char_group'];
            break;
        ...
        case 'table_OK': $t_flags = TestTables(false);
            if (SetPreliminaryTableErrors($t_flags)) {
                RewriteDefinition($dbh, $Mes);
            } break;
        case 'max_level': ReduceMaxLevel($dbh, $k[1]);
            $_SESSION['table_definitions'][$k[1]]['tab_err'][] = "<font color='#0000FF'>".Title(407).'</font>';
            break; // change
...
foreach ($_SESSION['table_definitions'] as $k => $v) {
    ...
    echo '<td>';
    if ($v['second_catalog'] != '') {
        echo "<button name='max_level-".$k."' type='submit' title='".Title(325)."' class='i_h' value='*'".$dis.'>'.ImgV('LineUp', 10, 16).'</button>';
    } echo '</td>';
