...
foreach ($_POST as $str_key => $str_v) {
    ...
    switch ($s_k) {
        ...
        case 'user_lang_s': if (SetUserLangTable($dbh_sys, $sw_break)) {
            $t_flags = TestTables($_SESSION['user_working_mode'] == 0);
        } break;
        case 'use_type_s': if (AfterUseTypeChoice($sw_break, $Mes)) {
            $t_flags = TestTables(false);
        } break;
        case 'second_catalog_s': if (AfterSecondCatalogChoice($sw_break)) {
            $t_flags = TestTables(false);
        } break;
        case 'group_symbol': if ($_SESSION['table_definitions'][$k[1]]['illegals'] != '' && substr($_SESSION['table_definitions'][$k[1]]['illegals'], -1) != chr($_SESSION['char_group'])) {
            $_SESSION['table_definitions'][$k[1]]['illegals'] .= chr($_SESSION['char_group']);
        } break;
        case 'table_exit': if (ActionTableExit($Mes)) {
            header('Location: ../Administrator/DataBaseActions.php');
        } break;
        case 'table_OK': $t_flags = TestTables(false);
            if (! SetPreliminaryTableErrors($t_flags, $Mes)) {
                RewriteDefinition($dbh, $Mes);
            } break;
        case 'max_level': if (ReduceMaxLevelAll($dbh)) {
            $Mes[] = "<font color='#0000FF'>".Title(407).'</font>';
        }  break;
        case 'auto_increment': AutoIncrementAll($dbh);
            $Mes[] = "<font color='#0000FF'><b>".Title(406).'</b></font>';
            break;
...
<form method="post" id="table_form" name="table_form">
	...
	<?php foreach ($Mes as $z) {
	    echo "<br><font color='#FF0000'>".$z.'</font>';
	} ?>
...
foreach ($_SESSION['table_definitions'] as $k => $v) {
    ...
    echo '<td>';
    SelectTag('use_type-'.$k, $_SESSION['table_types'], $v['use_type'], '', true, '', 'use_type_on(this)', $_SESSION['user_working_mode'] == 0);
    echo '</td>';
    ...
    if ($v['use_type'] == 3) {
        ...
        echo '<td>';
        SelectTag('second_catalog-'.$k, SetSecondCatalogList($k), $v['second_catalog'], '', false, '', 'second_catalog_on(this)', $dis);
        echo '</td>';
