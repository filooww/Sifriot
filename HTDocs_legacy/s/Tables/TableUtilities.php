...
function GetTableDefinitions($dbh, $db)
{
    ...
    if ($res) {
        while ($row = mysqli_fetch_row($res)) {
            $t_def[$row[0]] = ['use_type' => $row[1], 'illegals' => $row[2], 'max_level' => $row[5], 'separators' => $row[4], 'catalog_id' => '', 'catalog_value' => '', 'group_type' => $row[3], 'second_catalog' => $row[6], 'table_title' => (($row[7] == '') ? $row[0] : $row[7]), 'tab_err' => []];
            if (! in_array($row[0], $DB_all_tables)) {
                $t_def[$row[0]]['tab_err'][] = Title(427);
                $no_DB[] = $row[0];
            }
            if ($row[1] == 3) {
                CatalogFieldDefinitions($row[0], $row[4], $t_def);
            }
        }
        mysqli_free_result($res);
    }
    foreach ($DB_all_tables as $table) {
        if (! in_array($table, array_keys($t_def))) {
            $t_def[$table] = ['use_type' => 0, 'illegals' => '', 'max_level' => 0, 'separators' => '', 'catalog_id' => '', 'catalog_value' => '', 'group_type' => 0, 'second_catalog' => '', 'table_title' => '', 'tab_err' => [Title(536)]];
            $no_definition[] = $table;
        }
    }
    if (count($no_definition) > 0) {
        $_SESSION['db_errors'][] = Title(707).' <b>table_definitions</b> '.Title(708).' '.Title(705).' '.((count($no_definition) == 1) ? FTM(Title(115)) : Title(706)).' <b>'.implode(', ', $no_definition).'</b>';
    }
    if (count($no_DB) > 0) {
        $_SESSION['db_errors'][] = FTM(Title(709)).' <b>'.$_SESSION['arr_db'][$db]['db_name'].'</b> '.Title(708).' '.((count($no_DB) == 1) ? FTM(Title(115)) : Title(706)).' <b>'.implode(', ', $no_DB).'</b>';
    }
    ksort($t_def);
...
function AfterUseTypeChoice(&$sw_break, &$Mes)
{
    if ($_POST['use_type_s'] == '') {
        ...
        if ($new_table_type == 3 && isset($_SESSION['all_field_list'][$a_p[1]])) {
            CatalogFieldDefinitions($a_p[1], $_SESSION['table_definitions'][$a_p[1]]['separators'], $_SESSION['table_definitions']);
        } else {
            ...
            if ($new_table_type == 1 || $new_table_type == 2 || $new_table_type == 5) {
                TestThisType($a_p[1], $new_table_type, $Mes);
            }
        }
...
function GetCatalogList($dbh)
{
    ...
    if ($res) {
        while ($row = mysqli_fetch_row($res)) {
            $arr_catalogs[$row[0]] = [$row[1], $row[2]];
            if ($row[1] != '') {
                $second_catalogs[] = $row[1];
            }
        }
        mysqli_free_result($res);
    }
    foreach ($second_catalogs as $z) {
        unset($arr_catalogs[$z]);
    }
...
{
    $C = ['0' => ['name' => '', 'tbl' => '', 'id' => '', 'vl' => '', 'cat_type' => 0, 'spr' => '', 'max_level' => 0, 'invs' => ''], '1' => ['name' => '', 'tbl' => '', 'id' => '', 'vl' => '', 'cat_type' => 0, 'spr' => '', 'max_level' => 0, 'invs' => '']];
...
function SetSecondCatalogList($else_catalog)
{
    ...
    foreach ($_SESSION['table_definitions'] as $k => $v) {
        if ($v['use_type'] == 3 && $k != $else_catalog && $v['second_catalog'] == '') {
            $second_catalogs[] = $k;
        }
    }
