    ...
    <div class="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 sticky top-0 z-10">
        <div class="px-4 py-2">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Metadata Review Dashboard') }}
                    </h1>
                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                        {{ __('Manage and review file metadata extraction') }}
                    </p>
                </div>
                <div class="flex gap-2">
                    @if (
                            $search || $statusFilter !== 'all' || $formatFilter !== 'all' || $dateFilter !== 'all' ||
            ...
            <div class="mt-4 flex flex-wrap gap-2">
                <button wire:click="$set('statusFilter', 'all')"
                    class="px-4 py-2 rounded-lg text-sm font-medium transition {{ $statusFilter === 'all' ? 'bg-gray-600 text-white ring-2 ring-gray-400' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700' }}">
                    📊 {{ __('All') }}
                    ({{ $stats['pending'] + $stats['processed'] + $stats['confirmed'] + $stats['failed'] + $stats['rejected'] }})
                </button>
                <button wire:click="$set('statusFilter', 'pending')"
                    class="px-4 py-2 rounded-lg text-sm font-medium transition {{ $statusFilter === 'pending' ? 'bg-yellow-600 text-white ring-2 ring-yellow-400' : 'bg-yellow-50 dark:bg-yellow-900/20 text-yellow-700 dark:text-yellow-400 hover:bg-yellow-100 dark:hover:bg-yellow-900/40' }}">
                    ⏳ {{ __('Pending') }} ({{ $stats['pending'] }})
                </button>
                <button wire:click="$set('statusFilter', 'processed')"
                    class="px-4 py-2 rounded-lg text-sm font-medium transition {{ $statusFilter === 'processed' ? 'bg-blue-600 text-white ring-2 ring-blue-400' : 'bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/40' }}">
                    📋 {{ __('Ready for Review') }} ({{ $stats['processed'] }})
                </button>
                <button wire:click="$set('statusFilter', 'confirmed')"
                    class="px-4 py-2 rounded-lg text-sm font-medium transition {{ $statusFilter === 'confirmed' ? 'bg-green-600 text-white ring-2 ring-green-400' : 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 hover:bg-green-100 dark:hover:bg-green-900/40' }}">
                    ✅ {{ __('Confirmed') }} ({{ $stats['confirmed'] }})
                </button>
                <button wire:click="$set('statusFilter', 'failed')"
                    class="px-4 py-2 rounded-lg text-sm font-medium transition {{ $statusFilter === 'failed' ? 'bg-red-600 text-white ring-2 ring-red-400' : 'bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/40' }}">
                    ❌ {{ __('Failed') }} ({{ $stats['failed'] }})
                </button>
                <button wire:click="$set('statusFilter', 'rejected')"
                    class="px-4 py-2 rounded-lg text-sm font-medium transition {{ $statusFilter === 'rejected' ? 'bg-gray-600 text-white ring-2 ring-gray-400' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700' }}">
                    🚫 {{ __('Rejected') }} ({{ $stats['rejected'] }})
                </button>

                @if ($orphanedCount > 0)
                    <button wire:click="toggleOrphanedView"
                        class="px-4 py-2 rounded-lg text-sm font-medium transition {{ $showOrphanedPublications ? 'bg-orange-600 text-white ring-2 ring-orange-400' : 'bg-orange-50 dark:bg-orange-900/20 text-orange-700 dark:text-orange-400 hover:bg-orange-100 dark:hover:bg-orange-900/40' }}">
            ...
            <!-- Search Bar -->
            <div class="mt-4">
                <input type="text" wire:model.live="search" placeholder="{{ __('Search by filename or title...') }}"
            ...
            @if ($showOrphanedPublications && $orphanedPublications->count() > 0)
                <div
                    class="mt-4 p-4 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-700 rounded-lg">
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <div class="flex gap-2">
                            <button type="button" wire:click="deleteAllOrphaned"
                            ...
                            <button type="button" wire:click="toggleOrphanedView"
                    ...
                    <div class="space-y-2 max-h-64 overflow-y-auto">
                        @foreach ($orphanedPublications as $pub)
                            <div
                                class="flex items-center justify-between p-2 bg-white dark:bg-gray-800 rounded border border-orange-200 dark:border-orange-600">
                                <div>
                                    <span
                                        class="text-sm font-medium text-gray-900 dark:text-white">{{ Str::limit($pub->title, 60) }}</span>
                                    <span class="text-xs text-gray-500 dark:text-gray-400 ml-2">ID:
                                        {{ $pub->id_publication }}</span>
                                    <span
                                        class="text-xs text-gray-500 dark:text-gray-400 ml-2">{{ $pub->created_at->format('M d, Y') }}</span>
                                </div>
                                <button type="button" wire:click="deleteOrphanedPublication({{ $pub->id_publication }})"
    ...
    <div class="flex flex-1 overflow-hidden">
        ...
        <main class="flex-1 overflow-y-auto">
            ...
            @if (count($selectedItems) > 0)
                <div
                    class="p-2 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded-lg flex items-center justify-between">
                    ...
                    <div class="flex gap-2 flex-wrap">
                        ...
                        <button type="button" wire:click="rejectAllSelected"
                            class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-medium rounded-lg transition">
                            🚫 {{ __('Reject All') }}
                        </button>
                        <div x-data="{ showDeleteModal: false }" class="relative">
                            <button type="button" @click="showDeleteModal = true"
                                class="px-4 py-2 bg-red-800 hover:bg-red-900 text-white text-sm font-medium rounded-lg transition">
                                🗑️ {{ __('Delete All') }}
                            </button>
                            ...
                            <div x-show="showDeleteModal" x-transition:enter="transition ease-out duration-300"
                                ...
                                <div x-show="showDeleteModal" x-transition:enter="transition ease-out duration-300"
                                    ...
                                    <div class="flex items-start gap-4">
                                        ...
                                        <div class="flex-1">
                                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                                                {{ __('Delete Selected Publications?') }}
                                            </h3>
                                            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                                                {{ __('This will permanently delete :count selected publication(s) and their metadata. This action cannot be undone.', ['count' => count($selectedItems)]) }}
                                            </p>
                                        </div>
                                    </div>
                                    <div class="mt-6 flex justify-end gap-3">
                                        ...
                                        <button type="button" @click="showDeleteModal = false; $wire.deleteAllSelected()"
                        ...
                        <div x-data="{
                            ...
                            class="relative">
                            <button type="button" wire:click="extractWithAISelected" wire:loading.attr="disabled"
                                wire:target="extractWithAISelected" @if (!$geminiConfigured) disabled
                                @mouseenter="showTooltip = true" @mouseleave="showTooltip = false" @endif
                                ...
                                @if($geminiConfigured)
                                style="background-color: rgb(147, 51, 234) !important; color: white !important;" @endif>
                                ...
                                <span wire:loading.remove
                                    wire:target="extractWithAISelected">{{ __('Extract with AI') }}</span>
            ...
            <!-- File Metadata Table -->
            <div
                class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 overflow-hidden flex flex-col min-h-96 flex-1 dark:bg-gray-900">
                <table class="w-full">
                    <thead class="bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left">
                                <button wire:click="sort('file_name')"
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700 bg-white dark:bg-gray-900 flex-1">
                        @forelse ($fileMetadataList as $metadata)
                            ...
                            @endphp
                            <tr wire:key="metadata-{{ $metadata->id }}"
                                class="hover:bg-gray-100 dark:hover:bg-gray-800 transition">
                                <td class="px-6 py-3">
                                    <input type="checkbox" wire:model.live="selectedItems" value="{{ $metadata->id }}"
                                        class="w-4 h-4 text-blue-600 rounded border-gray-300" />
                                </td>
                                <td class="px-6 py-3">
                                    <div class="flex flex-col">
                                        <span
                                            class="text-sm font-medium text-gray-900 dark:text-white">{{ Str::limit($metadata->file_name, 40) }}</span>
                                ...
                                <td class="px-6 py-3">
                                    <span
                                        class="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-200">
                                ...
                                <td class="px-6 py-3">
                                    ...
                                    @else
                                        <span
                                            class="text-xs text-gray-500 dark:text-gray-400 px-3 py-1.5 inline-block bg-gray-100 dark:bg-gray-800 rounded-lg">{{ __('No Publication') }}</span>
                                    @endif
                                </td>
                                    ...
                                    <div class="flex justify-end gap-2 items-center">
                                        ...
                                        @elseif ($metadata->status === 'failed')
                                            <button type="button" wire:click="reExtractSingle({{ (int) $metadata->id }})"
                                                ...
                                                <span wire:loading.remove
                                                    wire:target="reExtractSingle({{ (int) $metadata->id }})">{{ __('Retry') }}</span>
                                                <span wire:loading
                                                    wire:target="reExtractSingle({{ (int) $metadata->id }})">{{ __('Processing...') }}</span>
                                            </button>
                                        @endif
                                        <button type="button" wire:click="deleteMetadata({{ $metadata->id }})"
