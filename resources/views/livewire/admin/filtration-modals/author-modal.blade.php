            ...
            <div class="flex items-center gap-3">
                ...
                @else
                    <div class="flex h-10 w-10 items-center justify-center rounded-full bg-indigo-100 dark:bg-indigo-500/20">
                        <x-heroicon-o-user-plus class="h-5 w-5 text-indigo-600 dark:text-indigo-400" />
                    </div>
                @endif
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                    {{ $editingAuthor ? __('Edit Author') : __('Create Author') }}
                </h3>
            </div>
        ...
        <form wire:submit.prevent="saveAuthor">
            <div class="px-6 py-5">
                <div class="space-y-5">
                    {{-- Author Name --}}
                    <div>
                        <label for="author_name" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">
                            <span class="flex items-center gap-1.5">
                                <x-heroicon-o-user class="h-4 w-4" />
                                {{ __('Author Name') }} <span class="text-red-500">*</span>
                            </span>
                        </label>
                        <input type="text" wire:model="author_name" id="author_name"
                               x-on:input="$wire.author_name_low = $event.target.value.toLowerCase()"
                        ...
                        @error('author_name') <span class="mt-1 text-xs text-red-500 dark:text-red-400">{{ $message }}</span> @enderror
                    </div>

                    {{-- Author Name Lowercase (readonly) --}}
                    <div>
                        <label for="author_name_low" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">
                            <span class="flex items-center gap-1.5">
                                <x-heroicon-o-code-bracket class="h-4 w-4" />
                                {{ __('Author (lowercase)') }}
                            </span>
                        </label>
                        <input type="text" wire:model="author_name_low" id="author_name_low" readonly
                        ...
                        <p class="mt-1.5 flex items-center gap-1 text-xs text-gray-500 dark:text-gray-400">
                            ...
                            {{ __('Auto-generated from author name') }}
                        </p>
                    </div>
                </div>
            </div>
