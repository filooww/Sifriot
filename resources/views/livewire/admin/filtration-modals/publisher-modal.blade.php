    ...
    <div class="relative z-10 w-full max-w-xl rounded-2xl bg-white shadow-2xl ring-1 ring-gray-900/5 dark:bg-gray-800 dark:ring-white/10"
        ...
        <div class="flex items-center justify-between border-b border-gray-200 px-6 py-4 dark:border-gray-700">
            <div class="flex items-center gap-3">
                ...
                @else
                    <div class="flex h-10 w-10 items-center justify-center rounded-full bg-teal-100 dark:bg-teal-500/20">
                        <x-heroicon-o-building-office class="h-5 w-5 text-teal-600 dark:text-teal-400" />
                    </div>
                @endif
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                    {{ $editingPublisher ? __('Edit Publisher') : __('Create Publisher') }}
                </h3>
            </div>
        ...
        {{-- Modal Body --}}
        <form wire:submit.prevent="savePublisher">
            <div class="px-6 py-5">
                <div class="space-y-5">
                    {{-- Name Fields Group --}}
                    <div class="rounded-xl border border-gray-200 bg-gray-50/50 p-4 dark:border-gray-700 dark:bg-gray-900/30">
                        ...
                        <div class="grid gap-4 sm:grid-cols-3">
                            {{-- Name EN --}}
                            <div>
                                <label for="publisher_name_en" class="mb-1.5 block text-xs font-medium text-gray-600 dark:text-gray-400">
                                    {{ __('English') }} <span class="text-red-500">*</span>
                                </label>
                                <input type="text" wire:model="publisher_name_en" id="publisher_name_en"
                                       x-on:input="$wire.publisher_slug = $event.target.value.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '')"
                                ...
                                @error('publisher_name_en') <span class="mt-1 text-xs text-red-500 dark:text-red-400">{{ $message }}</span> @enderror
                            </div>

                            {{-- Name RU --}}
                            <div>
                                <label for="publisher_name_ru" class="mb-1.5 block text-xs font-medium text-gray-600 dark:text-gray-400">
                                    {{ __('Russian') }}
                                </label>
                                <input type="text" wire:model="publisher_name_ru" id="publisher_name_ru"
                                       class="block w-full rounded-lg border-gray-300 bg-white text-sm shadow-sm transition focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 dark:focus:border-blue-400 dark:focus:ring-blue-400">
                                @error('publisher_name_ru') <span class="mt-1 text-xs text-red-500 dark:text-red-400">{{ $message }}</span> @enderror
                            </div>

                            {{-- Name HE --}}
                            <div>
                                <label for="publisher_name_he" class="mb-1.5 block text-xs font-medium text-gray-600 dark:text-gray-400">
                                    {{ __('Hebrew') }}
                                </label>
                                <input type="text" wire:model="publisher_name_he" id="publisher_name_he" dir="rtl"
                                       class="block w-full rounded-lg border-gray-300 bg-white text-sm shadow-sm transition focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 dark:focus:border-blue-400 dark:focus:ring-blue-400">
                                @error('publisher_name_he') <span class="mt-1 text-xs text-red-500 dark:text-red-400">{{ $message }}</span> @enderror
                            </div>
                        </div>
                    </div>
                    ...
                    {{-- Slug --}}
                    <div>
                        <label for="publisher_slug" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">
                            <span class="flex items-center gap-1.5">
                                <x-heroicon-o-link class="h-4 w-4" />
                                {{ __('Slug') }} <span class="text-red-500">*</span>
                            </span>
                        </label>
                        <input type="text" wire:model="publisher_slug" id="publisher_slug"
                        ...
                        @error('publisher_slug') <span class="mt-1 text-xs text-red-500 dark:text-red-400">{{ $message }}</span> @enderror
                    </div>

                    {{-- Website --}}
                    <div>
                        <label for="publisher_website" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">
                            <span class="flex items-center gap-1.5">
                                <x-heroicon-o-globe-alt class="h-4 w-4" />
                                {{ __('Website') }}
                            </span>
                        </label>
                        <input type="url" wire:model="publisher_website" id="publisher_website" placeholder="https://example.com"
                               class="block w-full rounded-lg border-gray-300 bg-white text-sm shadow-sm transition focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 dark:focus:border-blue-400 dark:focus:ring-blue-400">
                        @error('publisher_website') <span class="mt-1 text-xs text-red-500 dark:text-red-400">{{ $message }}</span> @enderror
                    </div>
                </div>
            </div>
