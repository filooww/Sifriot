...
$expDz = "1";
$expDdz = EX($D, $eps, $big_scale);
$ttr = 22;
for ($t = 1; $t <= $ttr; $t++)
{
    $expDz = bcmul($expDz, $expDdz);
    $M = bcdiv($Mm, bcadd("1", bcdiv(bcsub($Mm, "1"), $expDz)));
    $R[$t] = Calc_k($t, $k_max, $ln_v, $h_order, $lndec, $M, $eps_k);
    $df = bcsub(time(), $tt);
    $tt = time();
    WriteTable("f_store", $res_format, $t, $M, "", "", (string)$R[$t], $df);
}

$dkdt = ($R[$ttr - 2] - 4 * $R[$ttr - 1] + 3 * $R[$ttr]) / 2;
$x = $R[$ttr] - 3 - ($dkdt / $R[$ttr]);
$N = bcadd((string)$x, bcdiv(bcmul("3", $M), (string)$R[$ttr]));

$fh = fopen("f_store.txt", "a");
fwrite($fh, "\r\nt = ".$ttr."; k[] = ".$R[$ttr - 2].", ".$R[$ttr - 1].", ".$R[$ttr]);
fwrite($fh, "\r\ndk/dt = ".$dkdt);
fwrite($fh, "\r\nk - 3 - (dk/dt / k) = ".$x);
fwrite($fh, "\r\nN = ".NormNum($N, 20));
fclose($fh);
...
$i_max = $tend * $di; //$i_max = 2 * $J; //$i_max = 5 * $J + 1;
$max_info = array(0, "0");
$flag_end = false;
...
$k_max = (float)$max_info[1];
$fh = fopen("f_store.txt", "a");
fwrite($fh, "\r\n".(string)$dif_t);
fwrite($fh, "\r\n".(string)$max_info[0]."   ".(string)$sec."   ".(string)$k_max)."    time = ".time();
fwrite($fh, "\r\nconcave points:");
