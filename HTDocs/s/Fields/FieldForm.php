...
foreach ($_POST as $str_key => $str_v)
{
	...
	switch ($s_k)
	{
		...
		case "user_lang_s"	: if (AfterLangChoice($dbh_sys, "user_lang_s", "user_lang", $sw_break)) $_SESSION['field_using'] = GetSpecialTexts($dbh_sys, "field_using"); break;
		case "field_exit"	: if ($_SESSION['field_change']) $_SESSION['all_field_list'] = GetAllFieldList($dbh, $Mes, -1); header("Location: ../Administrator/DataBaseActions.php"); break;
		case "sort_field"	: $_SESSION['field_sort'] = SortFieldDefinitions("field"); break;
		case "sort_id"		: $_SESSION['field_sort'] = SortFieldDefinitions("table_name"); break;
		case "sort_table"	: $_SESSION['field_sort'] = SortFieldDefinitions("in_table"); break;
		case "sort_screen"	: $_SESSION['field_sort'] = SortFieldDefinitions("in_screen"); break;
		case "sort_all"	    : $_SESSION['field_sort'] = SortFieldDefinitions("table_field"); break;
		case "field_edit"	: $_SESSION['f_k'] = array((integer)$k[1], $k[2]); header("Location: FieldEditForm.php"); break;
