...
function ParseAlgButtons($dbh)
{
	foreach ($_POST as $str_key => $str_v)
	{
		...
		switch ($s_k)
		{
			case "alg_exit"     : return array("act"=>"alg_exit");
			case "alg_create"   : return array("act"=>"alg_create");
			case "alg_minus"    : return array("act"=>"scr_h", "scr_corr"=>-1);
			case "alg_plus"     : return array("act"=>"scr_h", "scr_corr"=>1);
			case "alg_beg"      : return array("act"=>"nav", "nv"=>"beg");
			case "alg_pg_up"    : return array("act"=>"nav", "nv"=>"pgup");
			case "alg_ln_up"    : return array("act"=>"nav", "nv"=>"lnup");
			case "alg_ln_dn"    : return array("act"=>"nav", "nv"=>"lndn");
			case "alg_pg_dn"    : return array("act"=>"nav", "nv"=>"pgdn");
			case "alg_end"      : return array("act"=>"nav", "nv"=>"end");
			case "delete_alg"   : return array("act"=>"alg_del", "code"=>$arr_key[1]);
			case "edit_alg"     : return array("act"=>"alg_ed", "code"=>$arr_key[1]);
			case "copy_alg"     : return array("act"=>"alg_copy", "code"=>$arr_key[1]);
			case "select_alg"   : return array("act"=>"alg_sel", "code"=>$arr_key[1]);
			case "cancel_alg"   : return array("act"=>"alg_can", "code"=>$arr_key[1]);
			case "yes_name"     : return array("act"=>"del_act", "ans"=>true);
			case "cancel_name"  : return array("act"=>"del_act", "ans"=>false);
			case "selected_alg" : return array("act"=>"selected_alg");
...
{
	if ($_SESSION['portion'] == 1 && $offs == -1) return;
	$_SESSION['portion'] += $offs;
	$alg_arr = GetAlgorithmPortion($dbh);
}
...
{
	if (isset($arr_alg_seq[$alg_code])) unset($arr_alg_seq[$alg_code]);
...
{
	if (isset($radio_name) == $v_post) return "1";
	else return "0";
}
function InsertAlgorithm($dbh, $bsl)
{
	if (!is_numeric($_POST['alg_offset'])) return "Offset in path is not numeric";
...
function AlgField($alg_copy, $alg_code, &$arr_v, $v_name, $str = true)
{
	if (count($alg_copy) == 0)
	{
		if ($alg_code == "") return ($str) ? "" : 0;
		elseif (isset($arr_v[$alg_code][$v_name]))
		{
			if ($v_name == "remarks") return FillAlgFields($arr_v[$alg_code][$v_name]);
			else return $arr_v[$alg_code][$v_name];
		}
		else return "";
	}
	else return $alg_copy[$v_name];
}
function FillAlgFields($str)
{
	if (strpos($str, "Publishing;") !== false) return $str;
	if (strpos($str, "Series;") !== false) return $str;
	if (strpos($str, "IssueYear;") !== false) return $str;
	if (strpos($str, "IssueType;") !== false) return $str;
	if (strpos($str, "Magazine;") !== false) return $str;
	if (strpos($str, "Authors;") !== false) return $str;
	if (strpos($str, "Themes;") !== false) return $str;
...
{
	if ($alg_code == "") return ($check_mode == "N") ? " checked" : "";
	if ($arr_alg[$alg_code][$param] == $check_mode) return " checked";
	else return "";
}
...
function AddDelSymb($del_sy, $alg_del_symb, $arr_codes)
{
	...
	if ($alg_del_symb == "") return $arr_code[1]." ".(string)$arr_code[0];
	else
	{
...
function AlgorithmErrors()
{
    ...
    $w_str .= "reg_scr < 0 OR reg_scr > 1 OR ";
    $w_str .= "alg_remarks = ''";
    return $w_str;
}
function GetCheck($d) {return ($d == "Y") ? "Y" : "N";}
?>
