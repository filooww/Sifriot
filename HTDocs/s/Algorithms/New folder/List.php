...
if (isset($_POST['alg_exit'])) header("Location: ../".$_SESSION['return_point']);
...
$_SESSION['alg_copy'] = array();
$aA = ParseAlgButtons($dbh);
if (isset($aA['act']))
{
	switch ($aA['act'])
	{
		case "alg_create"   : $_SESSION['alg_edit_code'] = ""; header("Location: Form.php"); break;
		case "scr_h"        : ChangeAlgScreenHeight($dbh, $aA['scr_corr'], $_SESSION['arr_alg'], $_SESSION['alg_start_pos'], $_SESSION['alg_filter']); break;
		case "alg_del"      : $_SESSION['del_question'] = true; $_SESSION['alg_edit_code'] = $aA['code']; break;
		case "alg_ed"       : $_SESSION['alg_edit_code'] = $aA['code']; header("Location: Form.php"); break;
		case "alg_copy"     : CopyAlg($_SESSION['arr_alg'][$aA['code']], $_SESSION['alg_copy']); header("Location: Form.php"); break;
		case "alg_sel"      : $_SESSION['arr_alg_seq'][$aA['code']] = $_SESSION['arr_alg'][$aA['code']]; break;
		case "alg_can"      : if (isset($_SESSION['arr_alg_seq'][$aA['code']])) unset($_SESSION['arr_alg_seq'][$aA['code']]); break;
		case "del_act"      : if ($aA['ans']) DeleteAlg($dbh, $_SESSION['alg_edit_code'], $_SESSION['arr_alg'], $_SESSION['alg_start_pos'], $_SESSION['alg_count'], $_SESSION['arr_alg_seq'], $_SESSION['alg_filter']); break;
		case "selected_alg" : header("Location: ../".$_SESSION['return_point']); break;
	}
}
?>
...
<form method="post">
	...
	?>
	<?php if ($_SESSION['del_question']) QuestionForm("modal_form", "Do You sure You want to delete this algoritm?", array("yes_name", "cancel_name"), array("Yes", "No"));?>
