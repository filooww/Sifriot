...
if (!$dbh) ExitSession(Title(1)." <b>".$_SESSION['db_info']['db_name']."</b>`".implode("`", $Mes)."|FF0000", $_SESSION['db_key']);
$Mes = "";
if (isset($_POST['alg_save']))
{
	if (count($_SESSION['alg_copy']) == 0)
	{
		...
		{
			$Mes = InsertAlgorithm($dbh, array(chr(92), chr(92).chr(92)));
			if ($Mes == "") $_SESSION['alg_count']++;
		}
		else $Mes = UpdateAlgorithm($dbh, $_SESSION['alg_edit_code'], array(chr(92), chr(92).chr(92)));
	}
	else
	{
		$Mes = InsertAlgorithm($dbh, array(chr(92), chr(92).chr(92)));
	...
	if ($Mes == "") header("Location: List.php");
}
elseif (isset($_POST['yes_name']) || isset($_POST['alg_form_exit'])) header("Location: List.php");
elseif (isset($_POST['path_parse'])) $_SESSION['path_example'] = PathOutPut($_POST['file_example'], $_SESSION['file_example']);
?>
...
<hr noshade="noshade" />
<?php if ($Mes != "" && $Mes != "-") QuestionForm("modal_form", array($Mes, "Do You want to stop editing?"), array("yes_name", "cancel_name"), array("Yes", "No"));?>
</form>
