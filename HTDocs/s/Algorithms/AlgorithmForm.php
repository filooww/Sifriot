...
$Mes = array();
if (count($_POST) == 0) AlgorithmInitial($dbh);
$mes_screen = "";
$mark_del_mes = "";
foreach ($_POST as $str_key => $str_v)
{
	...
	switch ($s_k)
	{
		...
		case "algorithm_exit"		  : $_SESSION['algorithm_info'] = ""; $goto = AlgorithmExit($dbh, $Mes); if ($goto != "") header("Location: ".$goto.".php"); break;
		case "algorithm_height_minus" : $_SESSION['algorithm_info'] = ""; $mes_screen = ChangeAlgorithmScreen($dbh, -1); break;
		case "algorithm_height_plus"  : $_SESSION['algorithm_info'] = ""; $mes_screen = ChangeAlgorithmScreen($dbh, 1); break;
		case "algorithm_height_button": $_SESSION['algorithm_info'] = ""; $mes_screen = ChangeAlgorithmScreen($dbh, $_POST['algorithm_height']); break;
		case "algorithm_navigation"   : $sw_break = false; $_SESSION['algorithm_info'] = ""; AlgorithmNavigation($dbh, $arr_key[1], $Mes); break;
		case "yes_del"			      : $sw_break = false; DeleteMarkedAlgorithms($dbh); $_SESSION['algorithm_delete'] = false; break;
		case "no_del"				  : $sw_break = false; $_SESSION['alg_del'] = array(); $_SESSION['algorithm_delete'] = false; break;
		case "algorithm_mark_del"	  : $sw_break = false; $_SESSION['algorithm_info'] = ""; AlgorithmMarkDelete($arr_key[1]); break;
		case "algorithm_edit"		  : $sw_break = false; if (!in_array($arr_key[1], $_SESSION['alg_del'])) EditAlgorithm($dbh, $arr_key[1], $_SESSION['arr_alg'][$arr_key[1]], $Mes); break;
		case "algorithm_info"	      : $sw_break = false; HideUnhideAlgorithmInfo($dbh, $arr_key[1]); break;
		case "add_new_algorithm"	  : AddNewAlgorithm($dbh, "", $Mes); break;
		case "algorithm_copy"	      : AddNewAlgorithm($dbh, $_SESSION['algorithm_info'], $Mes); break;
		case "algorithm_delete"	      : $_SESSION['algorithm_delete'] = true; break;
        case "algorithm_parse"	      : $_SESSION['path_parse'] = !$_SESSION['path_parse']; break;
        case "path_parse"	          : $_SESSION['perform_parse'] = true; $_SESSION['path_example'] = PathOutPut($_POST['file_example'], $_SESSION['file_example']); break;
...
<form method="post" id="algorithm_form" name="algorithm_form">
	...
	if ($_SESSION['algorithm_delete'] && count($_SESSION['alg_del']) > 0) DBMDelQuestion("modal_form", array("yes_del", "no_del"), AlgorithmQuestionString(), 208, 692, 693);
	if (!isset($_SESSION['arr_alg']) || isset($_SESSION['arr_alg']) && count($_SESSION['arr_alg']) == 0) echo "<div><center><b><font size=+1>".Title(76)."</b></font></center></div>";
	else require_once("AlgorithmTable.php");
	?>
</form>
