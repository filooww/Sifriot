...
{
    $_SESSION['arr_alg'] = array();
    $_SESSION['alg_flag'] = array();
...
{
    foreach ($_SESSION['alg_flag'][$k] as $v_name => $flag) if (in_array($v_name, $_SESSION['algorithm_err'][$err_types][0]) && $flag) return false;
    if (isset($_SESSION['update_replace'][$k])) foreach ($_SESSION['update_replace'][$k] as $v_name => $f_name) if (in_array($v_name, $_SESSION['algorithm_err'][$err_types][1]) && $f_name) return false;
    return true;
}
