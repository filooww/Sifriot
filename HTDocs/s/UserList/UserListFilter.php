...
function SetUserFilterPosition($dbh)
{
	...
	{
        GetUserCounts($dbh);
        if ($old_filter == "") $_SESSION['start'] = 0;
        else
        {
            $first_key = FindFirstUser();
            if ($first_key == "") $_SESSION['start'] = 0;
            else GetUserStartPosition($dbh, $first_key);
        }
    }
}
function FindFirstUser()
{
    foreach ($_SESSION['user_list'] as $k => $v) if (UserRowComparison($k, $v)) return $k;
    return "";
}
function UserRowComparison($k, $v)
{
    if ($_SESSION['filter_id'] != "" && $k != $_SESSION['filter_id']) return false;
    if ($_SESSION['filter_name'] != "" && strpos($v[0], FilterToUserName()) === false) return false;
    if ($_SESSION['category'][0] != "all" && $_SESSION['category'][0] != "invalid" && (integer)$v[2] < $_SESSION['categories'][$_SESSION['category'][0]][1] && (integer)$v[2] > $_SESSION['categories'][$_SESSION['category'][0]][2]) return false;
    if ($_SESSION['category'][0] == "invalid" && (integer)$v[2] >= 0 && (integer)$v[2] <= 99) return false;
    if ($_SESSION['filter_error']) return ComparisonErrors($k, $v);
    return true;
}
function FilterToUserName()
{
    if (strpos($_SESSION['filter_name'], chr(39)) !== false) return str_replace(chr(39), $_SESSION['apostrophe_replace'], $_SESSION['filter_name']);
    else return $_SESSION['filter_name'];
}
function ComparisonErrors($k, $v)
{
    if ((integer)($k) <= 0) return true;
    if ((integer)$v[2] < 0 || (integer)$v[2] > 99) return true;
    if ((integer)$v[2] = 0 && ($v[0] != "_".$k || $v[1] != "#####".$k)) return true;
    if (!TestUserName($v[0]) || !TestUserPassword($v[1])) return true;
    if (!isset($_SESSION['user_langs'][$v[3]])) return true;
    if ((integer)$v[4] < 1) return true;
    if ((integer)$v[2] == 0 && gettype($v[5]) == "string" || (integer)$v[2] > 0 && !isset($_SESSION['list_db'][$v[5]])) return true;
    if (substr($v[6], 1) != "dmy" && substr($v[6], 1) != "mdy" && substr($v[6], 1) != "ydm" && substr($v[6], 1) != "ymd") return true;
    if ($v[7] != 0 && $v[7] != 1) return true;
    if ($v[8] != 0 && $v[8] != 1) return true;
    return false;
}
function GetUserStartPosition($dbh, $k)
{
	...
	if ($_SESSION['update_replace'][$k][1]) $_SESSION['start'] = max($_SESSION['start'] - 1, 0);
}
...
function UserFilterNameSettings()
{
    ...
    if ($_SESSION['double_quote_fix']) str_replace(chr(34).chr(34), chr(34), $w_str); // "
    $esc_flag = false;
    if (strpos($w_str, chr(37)) !== false) $esc_flag = ChangeFilterString(37, $w_str); // %
    if (strpos($w_str, chr(95)) !== false) $esc_flag = ChangeFilterString(95, $w_str); // _
    if (strpos($w_str, chr(92)) !== false) $esc_flag = ChangeFilterString(92, $w_str); // \
    if (strpos($w_str, chr(39)) !== false) $w_str = str_replace(chr(39), chr(39).chr(39), $w_str); // '
    $w_str = "name LIKE '%".$w_str."%'";
    if ($esc_flag) $w_str .= " ESCAPE '!'";
    return $w_str;
}
function GetUserListFilter()
{
    $arr_ww = array();
    if ($_SESSION['filter_id'] != "") $arr_ww[] = "id_user >= ".$_SESSION['filter_id'];
   	if ($_SESSION['filter_name'] != "") $arr_ww[] = UserFilterNameSettings();
    if ($_SESSION['category'][0] != "all" && $_SESSION['category'][0] != "invalid") $arr_ww[] = "user_priority >= ".$_SESSION['categories'][$_SESSION['category'][0]][1]." AND user_priority <=".$_SESSION['categories'][$_SESSION['category'][0]][2];
    elseif ($_SESSION['category'][0] == "invalid") $arr_ww[] = "(user_priority < 0 OR user_priority > 99)";
    if ($_SESSION['filter_error']) $arr_ww[] = "(".UserListErrorCondition().")";
