...
function RewriteDefinition($dbh, &$Mes)
{
	...
	foreach ($_SESSION['table_definitions'] as $table_name => $table_params)
	{
		...
		if ($table_params['illegals'] != "" && substr($table_params['illegals'], -1) == chr($_SESSION['char_group'])) $_SESSION['table_definitions'][$table_name]['illegals'] = substr($_SESSION['table_definitions'][$table_name]['illegals'], 0, -1);
...
function ReduceMaxLevelAll($dbh)
{
    ...
    foreach ($_SESSION['table_definitions'] as $table => $t_params)
    {
        ...
        {
            $max_level_arr = array();
            if ($t_params['use_type'] == 3) $max_level = GetMaxLevel($dbh, $table);
		    else $max_level = 0;
    ...
    foreach ($_SESSION['table_definitions'] as $t_name => $v)
    {
	    ...
	    if ($v['use_type'] == 3)
	    {
		    ...
		    if ($_SESSION['group_types'][$v['group_type']] != $_POST["group_type-".$t_name])
		    {
                ...
                if ($i !== false) $_SESSION['table_definitions'][$t_name]['group_type'] = $i;
            }
	    }
    }
}
