...
<table>
	<tr valign="top">
		...
		<td>
			<table>			
				<tr>
					<?php if ($_SESSION['user_working_mode'] != 0) echo "<td></td>";?>
					<td><b><?php echo Title(163);?></b></td>
					<?php if ($_SESSION['alarm']) echo "<td></td>";?>
				...
				foreach ($_SESSION['visit_list'] as $k => $v)
				{
					...
					echo "<tr valign='top'>";
						if ($_SESSION['user_working_mode'] != 0)
						{
							if (!$u_this)
							{
								if ($v[7] == 0) $b_image = SysImage("BlankBorder", 16, 16, $_SESSION['user_working_mode'] == 0);
								elseif ($v[7] == 1) $b_image = SysImage("CheckBorder", 16, 16, $_SESSION['user_working_mode'] == 0);
								else $b_image = SysImage("Delete", 16, 16, $_SESSION['user_working_mode'] == 0);
						...
						if ($_SESSION['alarm']) echo "<td><input size='5' disabled type='text' name='db_id|".$k."' value='".$v[1]."' class='".(($v[0] == "") ? "data_numeric_bold_red" : "data_numeric_bold")."'></td>";
						echo "<td><input size='15' disabled type='text' name='user_name|".$k."' value='".$v[2]."'></td>";
						if ($_SESSION['alarm']) echo "<td><input size='5' disabled type='text' name='user_id|".$k."' value='".$v[3]."' class='".(($v[2] == "") ? "data_numeric_bold_red" : "data_numeric_bold")."'></td>";
						echo "<td><input size='20' disabled type='text' class='data_center_bold' name='work_start|".$k."' value='".ToDate($v[4], ".", true)."'></td>";
						echo "<td><input size='18' disabled type='text' name='visit_count|".$k."' value='".$v[5]."' class='".((!is_numeric($v[5]) || is_numeric($v[5]) && (integer)$v[5] < 1) ? "data_numeric_bold_red" : "data_numeric_bold")."'></td>";
						...
						if ($u_this || CompareSessionParameters($v)) echo "<td></td>";
						else echo "<td><button name='reset_session|".$k."' title='".Title(194)."' type='submit' class='w_b' value='*'>...</button></td>";
						echo "<td>".$v[9]['str']."</td>";
					echo "</tr>";
				}
				for ($i = count($_SESSION['visit_list']); $i < $_SESSION['portion']; $i++) echo "<tr><td align='right' class='dis_text'>".(string)($i + 1)."</td></tr>";
				?>
			</table>
		</td>
	</tr>
