...
function GetVisitPortion($dbh, $old_visit_param)
{
	...
	if ($res)
	{
		while ($row = mysqli_fetch_row($res))
		{
			...
			$visit_param[$k] = array($row[1], $row[0], $row[3], $row[2], $row[4], (string)$row[5], $row[6], (array_key_exists($k, $old_visit_param)) ? $old_visit_param[$k][7] : 0, $row[7], SetVisitErrors($row[1], $row[3], $row[4], $row[5], $row[6], $row[7], $k));
		}
...
function VisitListNavigation($dbh, $act, &$Mes)
{
	...
	{
		if (isset($_SESSION['pre_ref']['visits'])) $Mes[] = "<font color='#FF0000'>".Title(240)." ".Title(570)."</font>"; // !@#$%^&
		else $Mes[] = "<font color='#FF0000'>".Title(240)." ".Title(571)."</font>";
	}
}
