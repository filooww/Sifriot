...
function TestCatalogs($dbh, $init = false, $clear_mes = true, $common_mes = "")
{
	...
	if (!$init)
	{
		if ($_SESSION['Catalog']['1']['table'] != "") CatalogNewContent("1");
		else CatalogNewContent("0");
	}
	TestCatalogRows("0");
	TestCatalogRows("1");
	DublicateSearch($dbh, "0");
	if ($_SESSION['Catalog']['1']['table'] != "") DublicateSearch($dbh, "1");
	if (count($_SESSION['del_row']['0']) > 0) PublicationReferences($dbh, $_SESSION['Catalog']['0']['in_pub']);
	if (count($_SESSION['del_row']['1']) > 0) CoupledReferences($dbh);
	if ($common_mes != "")
    {
        $_SESSION['mes']['c'][] = array("time"=>"", "text"=>CatalogMessage($common_mes), "status"=>"statement", "now"=>true);
...
function CatalogNewContent($n)
{
	foreach ($_SESSION['cat_arr'][$n] as $arr_key => $arr_el)
	{
		...
		{
			if ($_POST[$post_key] == "" && is_numeric($arr_key) && end($arr_el) != "") $_SESSION['del_row'][$n][$arr_key] = array("old_id"=>$arr_key, "old_value"=>end($arr_el)); // new??
...
function TestCatalogRows($n)
{
	...
	{
		if (is_numeric($k) && end($v) == "") $_SESSION['mes'][$n][] = array("time"=>"", "text"=>Title(225)." <b>".$k."</b> ".Title(160), "status"=>"warning");
		...
		if ($illegal_mes != "") $_SESSION['mes'][$n][] = array("time"=>"", "text"=>$illegal_mes, "status"=>"error");
		if ($n == 0 && $_SESSION['Catalog']['1']['table'] != "" && $v[3] != "" && is_numeric($k) && !$_SESSION['cat_arr']['0'][$k][2] && TestSetValue($k, $v[3], $v[4])) $_SESSION['cat_arr']['0'][$k][2] = true; // new??
        if (is_numeric($k) && (integer)$k = 0)
        {
            $_SESSION['mes'][$n][] = array("time"=>"", "text"=>Title(225)." <b>".$k."</b>: ".Title(627), "status"=>"error");
...
function TestSetValue($k, $code_str, $value_str)
{
	...
	$arr_empty = array();
	for ($i = 0; $i < count($arr_values); $i++) if ($arr_values[$i] == "") $arr_empty[] = $arr_codes[$i];
	if (count($arr_empty) > 0)
	{
		$title_link = (count($arr_empty) == 1) ? Title(445) : Title(446);
		$title_del = (count($arr_empty) == 1) ? Title(447) : Title(448);
		$_SESSION['mes']['0'][] = array("time"=>"", "text"=>Title(443)." <b>".$value_str."</b> (<b>".$k."</b>) ".$title_link." <b>".implode(", ", $arr_empty)."</b> ".$title_del." <b>".$_SESSION['Catalog']['1']['name']."</b>.", "status"=>"error");
		$fl = true;
	}
	if ($_SESSION['Catalog']['0']['cat_type'] == 2)
	{
		...
		{
			$_SESSION['mes']['0'][] = array("time"=>"", "text"=>Title(443)." <b>".$value_str."</b> (<b>".$k."</b>) ".Title(475), "status"=>"error");
			$fl = true;
		}
	}
	return $fl;
...
function DublicateSearch($dbh, $n)
{
	...
	if ($req)
	{
		while ($row = mysqli_fetch_row($req))
		{
			...
			foreach ($arr as $z)
			{
				if ($single) $doubled[$row[0]][] = $z;
				else $pre_ids[$row[0]][] = $z;
			}
		}
		mysqli_free_result($req);
		if (!$single) $doubled = GetSetValues($dbh, $pre_ids);
		if (count($doubled) > 0) DublicateMessages($dbh, $doubled, $n);
	}
}
function DublicateMessages($dbh, $doubled, $n)
{
	$_SESSION['mes'][$n][] = array("time"=>"", "text"=>Title(489).":", "status"=>"error");
	foreach ($doubled as $doubled_value => $doubled_id_arr)
	{
		foreach ($doubled_id_arr as $z) if (array_key_exists($z, $_SESSION['cat_arr'][$n])) $_SESSION['cat_arr'][$n][$z][2] = true;
		$doubled_id_arr = array_unique($doubled_id_arr);
		sort($doubled_id_arr);
		$_SESSION['mes'][$n][] = array("time"=>"", "text"=>"-- ".Title(490)." <b>".$doubled_value."</b> ".Title(491)." <b>".implode(", ", $doubled_id_arr)."</b>", "status"=>"error");
	}
}
function GetSetValues($dbh, $pre_ids)
{
	...
	{
		$arr[$k] = explode(",", $k);
		foreach ($arr[$k] as $z) if (!in_array($z, $set_keys)) $set_keys[] = $z;
	}
	$set_values = array();
	...
	{
		$arr_new = array();
		for ($i = 0; $i < count($arr[$k]); $i++) $arr_new[] = $set_values[$arr[$k][$i]];
...
function PublicationReferences($dbh, $in_pub)
{
	...
	{
		$_SESSION['mes']['c'][] = array("time"=>"", "text"=>"The references to deleted values found in publications.", "status"=>"error");
		foreach ($item_for_del as $k => $v)
		{
			$_SESSION['cat_arr']['0'][$k][2] = true;
			$s = (count($v) == 1) ? "" : "s";
			$_SESSION['mes']['c'][] = array("time"=>"", "text"=>"-- reference to value <b>".end($_SESSION['cat_arr']['0'][$k])."</b> (<b>".$k."</b>) found in publication".$s.":", "status"=>"error");
			foreach ($v as $z) $_SESSION['mes']['c'][] = array("time"=>"", "text"=>"-- -- <b>".GetFieldByID($dbh, "publication", "title", "id_publication", $z)."</b> (<b>".$z."</b>)", "status"=>"error");
		}
		$_SESSION['mes']['c'][] = array("time"=>"", "text"=>"<b>-- Deletion canceled</b>", "status"=>"warning");
	}
	foreach ($_SESSION['del_row']['0'] as $k => $v) $_SESSION['mes']['c'][] = array("time"=>"", "text"=>"Row <b>".$v['old_value']."</b> (<b>".$k."</b>) will be deleted", "status"=>"warning");
}
function CancelDeletion($key_del, $id_row, &$item_for_del)
{
	OldValuesToCat($key_del, "0");
	$arr_pub = explode(",", $id_row);
	$item_for_del[$key_del] = array();
	foreach ($arr_pub as $z) if ($z != "") $item_for_del[$key_del][] = $z;
}
function CoupledReferences($dbh)
{
	...
	{
		$_SESSION['mes']['c'][] = array("time"=>"", "text"=>"The following deleting values have links to catalog <b>".$_SESSION['Catalog']['0']['name']."</b>:", "status"=>"error");
		foreach ($del_values as $k_del => $v_del)
		{
			OldValuesToCat($k_del, "0");
			$_SESSION['cat_arr']['1'][$k_del][2] = true;
			$_SESSION['mes']['c'][] = array("time"=>"", "text"=>"-- row <b>".end($_SESSION['cat_arr']['1'][$k_del])."</b> (<b>".$k_del."</b>) refers to", "status"=>"error");
			foreach ($v_del as $k => $v) $_SESSION['mes']['c'][] = array("time"=>"", "text"=>"-- -- <b>".$v."</b> (<b>".$k.")</b>", "status"=>"error");
			unset($_SESSION['del_row']['1'][$k_del]);
		}
		if (count($del_values) > 0) $_SESSION['mes']['c'][] = array("time"=>"", "text"=>"<b>-- Deletion canceled</b>. You must first make the appropriate changes to the catalog <b>".$_SESSION['Catalog']['0']['name']."</b>", "status"=>"warning");
		foreach ($_SESSION['del_row']['1'] as $k => $v) $_SESSION['mes']['c'][] = array("time"=>"", "text"=>"Row <b>".$v['old_value']."</b> (<b>".$k."</b>) will be deleted", "status"=>"warning");
	}
}
function DelScreenValues($n, $del_row_keys, &$test_values)
{
	foreach ($_SESSION['cat_arr'][$n] as $k => $v) SetDelValues($del_row_keys, $k, $v[3], $v[4], $test_values);
	return $test_values;
}
function DelOutScreenValues($dbh, $del_row_keys, $screen_ids, &$test_values)
{
	$adel = array();
	foreach ($del_row_keys as $k) $adel[] = $_SESSION['Catalog']['0']['value']." = '".$k."' OR ".$_SESSION['Catalog']['0']['value']." LIKE '".$k.",%' OR ".$_SESSION['Catalog']['0']['value']."  LIKE '%,".$k.",%' OR ".$_SESSION['Catalog']['0']['value']." LIKE '%,".$k."'";
	if (count($adel) > 0)
	{
		...
		if (!$_SESSION['match_case']) $qt .= ", ".VF($ml, $_SESSION['Catalog']['0']['table'], $_SESSION['Catalog']['0']['id'], $_SESSION['Catalog']['0']['value'], $_SESSION['Catalog']['1']['table'], $_SESSION['Catalog']['1']['id'], $_SESSION['Catalog']['1']['value']."_low", "", chr(34).chr(34), "f_l");
...
function SetDelValues($del_row_keys, $k, $str_codes, $str_value, &$test_values)
{
	...
	foreach ($del_row_keys as $k_del) if (in_array($k_del, $ed_codes)) $test_values[$k_del][$k] = $str_value;
}
...
{
	$flag_test = 0;
	if ($n != "") $flag_test = ChangesAndErrorsTest($_SESSION['cat_arr'][$n]);
	else
	{
	...
	if ($flag_test == 0) return false;
	else
	{
		if ($set_mes)
		{
			if ($flag_test == 1) $_SESSION['mes']['c'][] = array("time"=>"", "text"=>$pref_text.Title(239).$post_text, "status"=>"warning", "now"=>true);
			else $_SESSION['mes']['c'][] = array("time"=>"", "text"=>$pref_text." ".Title(471).$post_text, "status"=>"warning", "now"=>true);
		}
		return true;
	}
}
function ChangesAndErrorsTest($arr_tested)
{
	...
	{
		if ($v[1] && !$v[2] && $flag_test == 0) $flag_test = 1;
		elseif ($v[2]) $flag_test = 2;
		$v = next($arr_tested);
	}
	return $flag_test;
}
