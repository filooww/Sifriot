...
function GetAllConfigs($dbh, $conf_table)
{
	...
	if ($res)
	{
		...
		{
			$i++;
			$arr_site[(string)$i] = array(false, false, false, false, false, $row[0], $row[1], $row[2], $row[3]);
		}
		mysqli_free_result($res);
	}
	if ($_SESSION['user_working_mode'] == 1) $arr_site['new'] = array(false, false, false, false, false, "", "", false, "", "");
	return $arr_site;
}
function ActionConfigExit($dbh, &$Mes)
{
	if ($_SESSION['user_working_mode'] == 1)
	{
     	...
     	if (count($arr_warn['changed']) + count($arr_warn['illegal']) + count($arr_warn['numeric']) + count($arr_warn['doubled']) + count($arr_warn['deleted']) == 0)
	    {
		    ...
		    if ($v !== false) $_SESSION['image_dir'] = $v;
	    ...
	    {
		    if (count($arr_warn['illegal']) + count($arr_warn['numeric']) + count($arr_warn['doubled']) > 0) $Mes[] = "<b>".Title(223)."</b>";
		    else $Mes[] = "<b>".Title(235)."</b>";
            if ($_SESSION['alarm']) return "../Alarm/CommonAlarmForm" ;
            return "";
        }
	}
	if ($_SESSION['common_config']) return "../Administrator/MainForm";
	return "../Administrator/DataBaseActions";
}
function FindNumberByConfigName($conf_name)
{
    foreach ($_SESSION['config_list'] as $k => $v) if ($v[5] == $conf_name) return $k;
    return "";
}
function RewriteConfigs($dbh, $conf_table, &$Mes)
{
	...
	foreach ($_SESSION['config_list'] as $k => $v) 
	{
		if ($v[5] != "")
		{
			mysqli_query($dbh, "INSERT INTO ".$conf_table." VALUES ('".$v[5]."','".$v[6]."',".(integer)$v[7].",'".$v[8]."')");
			$arr_sort[$v[5]] = array($v[6], $v[7], $v[8]);
			$fl = true;
		}
	}
	if ($fl)
	{
		...
		{
			$i++;
			$arr_list[(string)$i] = array(false, false, false, false, false, $k, $v[0], $v[1], $v[2]);
		}
		$arr_list['new'] = array(false, false, false, false, false, "", "", false, "", "");
		$Mes[] = "<b>".Title(192)."</b>";
		if ($_SESSION['common_config']) SystemConfigs($dbh);
		else DBConfigs($dbh);
	}
...
{
	foreach ($_SESSION['config_list'] as $k => $v) for ($i = 1; $i < 4; $i++) $_SESSION['config_list'][$k][$i] = false;
}
function TestConfigs(&$Mes, $init = false, $numeric_flag_key = "")
{
	if ($numeric_flag_key == "") ClearConfigFlags();
	if (!$init)
	{
		foreach ($_SESSION['config_list'] as $k => $v)
		{
	...
	foreach ($_SESSION['config_list'] as $k => $v)
	{
		if ($v[5] != "")
		{
			...
			{
				if ($v[7] > 1) $_SESSION['config_list'][$k][7] = 1;
				else $_SESSION['config_list'][$k][7] = 1 - $_SESSION['config_list'][$k][7];
	...
	foreach ($dubl_codes as $k => $v) for ($i = 0; $i < count($v); $i++) $_SESSION['config_list'][(string)$v[$i]][3] = true;
	$arr_warn = WarningConfigArrays();
	if (count($arr_warn['changed']) > 0) $Mes[] = "<font color='#0000FF'><b>".((count($arr_warn['changed']) == 1) ? Title(116) : Title(108))."</b></font>: <b>".implode(", ", $arr_warn['changed'])."</b>";
	if (count($arr_warn['illegal']) > 0) $Mes[] = "<font color='#FF0000'><b>".Title(191)."</b></font> (".((count($arr_warn['illegal']) == 1) ? Title(419) : Title(222))." <b>".implode(", ", $arr_warn['illegal'])."</b>)";
	if (count($arr_warn['numeric']) > 0) $Mes[] = "<font color='#FF0000'><b>".Title(410)." ".Title(77)."</b></font> (".((count($arr_warn['numeric']) == 1) ? Title(419) : Title(222))." <b>".implode(", ", $arr_warn['numeric'])."</b>)";
	if (count($arr_warn['doubled']) > 0) DoubledMessages(190, $arr_warn['doubled'], array(), 0, $Mes);
	if (count($arr_warn['deleted']) > 0) $Mes[] = "<font color='#FF00FF'><b>".Title(204)." ".((count($arr_warn['deleted']) == 1) ? Title(221) : Title(222))."</font> ".implode(", ", $arr_warn['deleted'])."</b>";
	return $arr_warn;
}
function ChangeConfigFromPost($conf_type, $k, $v, $param_str_ind)
{
	if ($_POST["conf_".$conf_type."-".$k] != $v[$param_str_ind])
	{
		...
		else $_SESSION['config_list'][$k][0] = true;
	}
}
function WarningConfigArrays()
{
	$arr_warn = array("changed"=>array(), "illegal"=>array(), "numeric"=>array(), "doubled"=>array(), "deleted"=>array());
	foreach ($_SESSION['config_list'] as $k => $v)
	{
		if ($v[0] || $v[4]) $arr_warn['changed'][] = $k;
		if ($v[1]) $arr_warn['illegal'][] = $k;
		if ($v[2]) $arr_warn['numeric'][] = $k;
		if ($v[3]) $arr_warn['doubled'][$v[5]][] = $k;
		if ($v[5] == "" && $k != "new") $arr_warn['deleted'][] = $k;
	}
	return $arr_warn;
}
function DoubleConfigSearch()
{
	$no_del = array("codes"=>array(), "values"=>array());
	foreach ($_SESSION['config_list'] as $k => $v)
	{
		if ($v[5] != "")
		{
	...
	$dubl_codes = array();
	for ($i = 0; $i < count($no_del['values']); $i++) if ($arr_groups[$no_del['values'][$i]] > 1) $dubl_codes[$no_del['values'][$i]][] = $no_del['codes'][$i];
	return $dubl_codes;
}
...
function GetConfigValue($dbh, $conf_table, $config_name, $conf_type, $config_description = "", $default_value = "")
{
	...
	if ($res)
	{
        ...
        {
            $gv = ($conf_type) ? (integer)$row[0] : $row[0];
            if ($config_description != "" && $row[1] != $config_description) SetConfigDescription($dbh, $conf_table, $config_name, $config_description);
        }
		else
		{
			mysqli_query($dbh, "INSERT INTO ".$conf_table." VALUES ('".$config_name."','".VV($default_value)."',".(($conf_type) ? "1" : "0").",'".VV($config_description)."')");
			if ($row[1] == "" && $config_description != "") SetConfigDescription($dbh, $conf_table, $config_name, $config_description);
			$gv = ($conf_type) ? (integer)VV($default_value) : VV($default_value);
		}
		mysqli_free_result($res);
	}
	else
...
function SetConfigValue($dbh, $conf_table, $config_name, $conf_type, $config_value, $config_description = "")
{
	...
	{ 
		...
		else mysqli_query($dbh, "INSERT INTO ".$conf_table." VALUES ('".$config_name."','".$config_value."','".$config_description."')");
		mysqli_free_result($res);
	}
	else mysqli_query($dbh, "INSERT INTO ".$conf_table." (".$f_set.") VALUES (".$v_set.")");
}
function TitleConfig()
{
	if ($_SESSION['common_config']) return Title(130);
	return Title(202)." <font color='0000FF'>".$_SESSION['db_info']['name']."</font>";
}
function GetValueByConfigName($c_n)
{
	foreach ($_SESSION['config_list'] as $v) if ($v[5] == $c_n) return $v[6];
	return false;
}
...
function BeforeSave(&$Mes, $init)
{
	...
	if (count($arr_warn['illegal']) + count($arr_warn['numeric']) + count($arr_warn['doubled']) > 0) $Mes[] = "<br><font color='0000FF'><b>".Title(420)."</b></font>";
	return $arr_warn;
}
function InvalidConfigType()
{
	$arr_err = array();
	foreach ($_SESSION['config_list'] as $k => $v) if ($v[7] > 1) $arr_err[] = $k;
	return $arr_err;
}
function SystemConfigs($dbh_sys)
{
    ...
    $_SESSION['char_group'] = (integer)GetConfigValue($dbh_sys, "db_s_configs", "char_group", true, "Code to separate of special symbol groups", "19");
    $_SESSION['apostrophe_replace'] = GetConfigValue($dbh_sys, "db_s_configs", "apostrophe_replace", false, "Apostrophe replacement symbol", "`");
    $_SESSION['number_warn'] = (integer)GetConfigValue($dbh_sys, "db_s_configs", "number_warning", true, "Number of values in warning row", "10");
    ...
    $_SESSION['try_limit'] = (integer)GetConfigValue($dbh_sys, "db_s_configs", "try_limit", true, "Maximum number of login attempts", "3");
...
//    $_SESSION['conf']['portion_item'] = (integer)GetConfigValue($dbh, "db_configs", "portion_item", true, "Number of records reading from the main list at one time", "100");
    ...
    $_SESSION['conf']['screen_saver_height'] = (integer)GetConfigValue($dbh, "db_configs", "screen_saver_height", true, "Height of site screen saver");
