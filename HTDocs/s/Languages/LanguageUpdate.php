...
function SetDoubleLangParameters($v)
{
	for ($i = 0; $i < count($v); $i++)
	{
		...
		if ($_SESSION['lang_param'][$v[$i]][2] == "") $_SESSION['lang_param'][$v[$i]][2] = "<font color='#FF0000'>".Title(538)." ".FTM(Title(181))." ".((count($arr_v) == 1) ? Title(151) : Title(152))." <b>".implode(", ", $arr_v)."</b></font>";
		else $_SESSION['lang_param'][$v[$i]][2] .= "; <font color='#FF0000'>".Title(538)." ".FTM(Title(181))." ".((count($arr_v) == 1) ? Title(151) : Title(152))." <b>".implode(", ", $arr_v)."</b></font>";
	}
}
function TestLangList($dbh, $init = false)
{
    $_SESSION['del_langs'] = array();
	foreach ($_SESSION['lang_param'] as $k => $v) TestLangRow($dbh, $k, $v[0], $init);
	if (!$init)
	{
	    $dd = TestLanguageDouble();
	    if (count($dd) > 0) foreach ($dd as $v) SetDoubleLangParameters($v);
	}
}
function TestLangRow($dbh, $k, $v0, $init)
{
    ...
    if ((string)$k == "1" && $v0 == "English") $arr_mark[] = "<font color='#0000FF'><b>".Title(270)."</b></font>";
	if ($v0 == "" && gettype($k) == "integer") TestEmptyLanguage($dbh, $k, $init, $arr_mark);
    ...
    if (isset($_SESSION['pre_lang_err'][$k])) $arr_mark[] = "<font color='#0000FF'>".DigitsToTitle($_SESSION['pre_lang_err'][$k])."</font>";
	if (count($arr_mark) > 0) $_SESSION['lang_param'][$k][2] = implode("; ", $arr_mark);
	else $_SESSION['lang_param'][$k][2] = "";
}
...
function InvalidLangDelMessage($k, $lang_refs)
{
	...
	if ($_SESSION['lang_param'][$k][0] != $_SESSION['lang_param'][$k][1]) $str .= " <button name='lang_restore|".(string)$k."' type='submit'>".FTM(Title(224))." ".Title(490)."</button>";
	return $str;
}
function RewriteLanguages($dbh, &$Mes)
{
    ...
    foreach ($_SESSION['lang_param'] as $k => $v)
    {
        if ($v[0] != "" && !$v[2])
        {
            if (gettype($k) == "string" && $v[0] != "") InsertNewLanguage($dbh, $v[0], $fl_change);
            elseif ((integer)$k > 1 && $v[0] != $v[1]) UpdateLanguage($dbh, $k, $v[0], $fl_change);
        }
	}
    if ($fl_change)
    {
...
function InsertNewLanguage($dbh, $v0, &$fl_change)
{
    ...
    $_SESSION['lang_param'][$new_lang_id] = array($v0, $v0, "", false);
    unset($_SESSION['lang_param']['new']);
    ksort($_SESSION['lang_param']);
    $_SESSION['lang_param']['new'] = array("", "", "", false);
...
function UpdateLanguage($dbh, $k, $v0, &$fl_change)
{
    ...
    if ($k == $_SESSION['user_lang'][0]) $_SESSION['user_lang'][1] = $_SESSION['lang_param'][$k][0];
    $fl_change = true;
}
function LangDel($dbh)
{
    if (isset($_SESSION['pre_lang_err'])) unset($_SESSION['pre_lang_err']);
    ...
    foreach ($_SESSION['del_langs'] as $z) unset($_SESSION['lang_param'][$z]);
}
function LangSetQuestionString()
{
    $arr = array();
    foreach ($_SESSION['del_langs'] as $k_del) $arr[] = $_SESSION['lang_param'][$k_del][1]." (".$k_del.")";
    return $arr;
}
function LanguageReferencesForDelete($dbh, $id_lang)
{
	...
	if ($res)
	{
		...
		{
            $arr = explode(", ", $row[1]);
            if (count($arr) > $_SESSION['number_warn']) $lang_refs[$row[0]] = implode(", ", array_slice($arr, 0, $_SESSION['number_warn'])).", ...";
            else $lang_refs[$row[0]] = $row[1];
		}
...
{
	$str_arr = array();
	foreach ($lang_refs as $k => $v) $str_arr[] = "<b>".(string)$k."</b> (".$v.")";
	return implode(", ", $str_arr);
}
function IsLanguageErrors()
{
	foreach ($_SESSION['lang_param'] as $v) if ($v[3]) return true;
	return false;
}
function LanguageExit($dbh, &$Mes)
{
    ...
    if ($ret_addr != "")
    {
        ...
        if (isset($_SESSION['del_langs'])) unset($_SESSION['del_langs']);
        if (isset($_SESSION['pre_lang_err'])) unset($_SESSION['pre_lang_err']);
    }
 	return $ret_addr;
}
...
{
    $_SESSION['common_langs'] = array();
    $_SESSION['user_langs'] = array();
    foreach ($_SESSION['lang_param'] as $k => $v)
    {
        if (gettype($k) == "integer")
        {
            $_SESSION['common_langs'][$k] = $v[0];
            if ($k != 0) $_SESSION['user_langs'][$k] = $v[0];
        }
    }
}
...
{
	$arr_dd = array();
	foreach ($_SESSION['lang_param'] as $k => $v) if ($v[0] != "") $arr_dd[$v[0]][] = (string)$k;
	foreach ($arr_dd as $k => $v)
    {
        if (count($v) == 1) unset($arr_dd[$k]);
        else foreach ($v as $z) $_SESSION['lang_param'][$z][3] = true;
    }
	return $arr_dd;
}
function LanguageForbid($k, $v0)
{
    if ((string)$k == "0" && $v0 == "(special)" || (string)$k == "1" && $v0 == "English") return " disabled";
    else return "";
}
function DelLangListRestore()
{
    if (isset($_SESSION['pre_lang_err'])) unset($_SESSION['pre_lang_err']);
    foreach ($_SESSION['del_langs'] as $k) if ($_SESSION['lang_param'][$k][0] != $_SESSION['lang_param'][$k][1]) $_SESSION['lang_param'][$k][0] = $_SESSION['lang_param'][$k][1];
    $_SESSION['del_langs'] = array();
}
