...
function UnusedTitles($dbh)
{
	...
	if ($res)
	{
		while ($row = mysqli_fetch_row($res))
        {
            if (!in_array($row[0], $_SESSION['scripts_title_ids']) && !in_array($row[0], $_SESSION['ex_title_ids'])) $unused_titles[] = (string)$row[0];
        }
		mysqli_free_result($res);
	}
...
{
    $arr_filter = array();
    if ($_SESSION['selected_title_lang'][0] != 0) $arr_filter[] = "id_language = ".(string)$_SESSION['selected_title_lang'][0];
    if ((integer)$_SESSION['title_filter_id'] > 0) $arr_filter[] = "id_title >= ".$_SESSION['title_filter_id'];
    if ($_SESSION['title_filter_text'] != "") $arr_filter[] = "title_text LIKE '%".$_SESSION['title_filter_text']."%'";
	switch ($_SESSION['view_title_mode'][0])
	{
		case 1: $arr_filter[] = "title_text = ''"; break;
		case 2: $d_list = TitleDoubleList($dbh); if (count($d_list) > 0) $arr_filter[] = "(".implode(" OR ", $d_list).")"; break;
		case 3: $unused_titles = UnusedTitles($dbh); if (count($unused_titles) > 0) $arr_filter[] = "id_title IN (".implode(",", $unused_titles).")"; break;
		case 4: $arr_filter[] = "((SELECT languages.id_language FROM languages WHERE languages.id_language = interface_texts.id_language) IS NULL OR interface_texts.id_language = 0 OR id_title = 0 OR id_title > 99999)"; break;
	}
    if (count($arr_filter) == 0) return "";
    else return " WHERE ".implode(" AND ", $arr_filter);
}
function TitleResetFilter($dbh, $set_title_count = true)
{
	...
	if ($set_title_count) $_SESSION['title_count'] = TitleCounts($dbh);
}
...
function StartTitlePositionByFilter($dbh)
{
    ...
    if (count($first_key) > 0) GetTitleStartPosition($dbh, $first_key[0], $first_key[1]);
    else $_SESSION['start'] = 0;
}
...
function GetFilterValue($search_value, $value_offset, $terminate_symbol, $value_separator = "")
{
    ...
    if ($start_pos + strlen($filter_string) == strlen($_SESSION['title_filter'])) $result_string = $filter_string;
    else
    {
    ...
    if ($value_separator == "") return $result_string;
    else return explode($value_separator, $result_string);
}
function FindFirstTitle($filter_param)
{
    foreach ($_SESSION['title_param'] as $v) if (TitleRowComparison($filter_param, $v[0], $v[2], $v[3])) return array($v[2], $v[3]);
    return array();
}
function TitleRowComparison($filter_param, $title_text, $title_id, $title_lang)
{
    foreach ($filter_param as $key_param => $v_param)
    {
        if ($v_param !== false)
        {
            switch ($key_param)
            {
                case "language_value" : if ((string)$title_lang != $v_param) return false; break;
                case "empty_pos"      : if ($title_text != "") return false; break;
                case "doubled_arr"    : if (!in_array("id_title = ".$title_id." AND id_language = ".(string)$title_lang, $v_param)) return false; break;
                case "unused_arr"     : if (!in_array($title_id, $v_param)) return false; break;
                case "errors_pos"     : if (isset($_SESSION['title_langs'][$title_lang]) && (integer)$v[2] > 0 && (integer)$title_id <= 99999) return false; break;
                case "id_value"       : if ((string)$title_lang != $v_param) return false; break;
                case "text_value"     : if (strpos($title_text, substr($v_param, 2, strlen($v_param) - 4)) === false) return false;
            }
        }
    }
    return true;
