...
{
	$item_row = array("e"=>array(), "i"=>array());
	if ($k == 0)
	{
...
{
	$p_files = array("e"=>array(), "i"=>array());
	$p_files['i'] = ReadItemFiles($dbh, "files", $p_id, $URL_path, $p_con);
	foreach ($p_files['i'] as $k => $v) foreach ($p_con as $k_c => $v_c) $p_files['e'][$k][$k_c] = $v[$k_c];
...
function EmptyRow($cury)
{
	...
	{
		if ($v['type'] == "integer") $c_row[$k] = 0;
		elseif ($v['type'] == "date") $c_row[$k] = GetCurrentDate();
		else $c_row[$k] = "";
		if ($v['table'] != "") $c_row[$k."_code"] = 0;
	}
	return $c_row;
}
...
function AddRefCatalog($act, $ref_field, $ref_value, &$arr_fields, &$arr_values)
{
	...
	else $arr_values[] = $ref_field." = ".(string)$ref_value;
}
function AddPostValue($act, $source_post_value, $k, $p_con, $ref_field, &$arr_fields, &$arr_values)
{
	...
	else $arr_values[] = $ref_field." = ".$ref_value;
	if ($p_con[$k]['low'])
	{
		...
		else $arr_values[] = $ref_field."_low = ".$ref_value;
	}
}
function ItemQueryText($act, $item_row, $p_con)
{
	...
	foreach ($item_row['e'] as $k => $v_row)
	{
		if (strpos($k, "_code") === false)
		{
			...
			if (isset($p_con[$k]['table']) && ($act == "insert" || $item_row['e'][$k] != $item_row['i'][$k])) AddRefCatalog($act, $p_con[$k]['ref'], $item_row['e'][$k], $arr_fields, $arr_values);
			elseif ($act == "insert" || $v != $item_row['i'][$k]) AddPostValue($act, $v, $k, $p_con, $p_con[$k]['ref'], $arr_fields, $arr_values);
		}
	}
	if (count($arr_values) > 0)
	{
		if ($act == "insert") return "(".implode(", ", $arr_fields).") VALUES (".implode(",", $arr_values).")";
		else return implode(", ", $arr_values);
	}
	else return "";
}
...
function AfterItemEdit($dbh)
{
	...
	if (!$in_filter) ResetSettings($dbh, array(false, true), $_SESSION['PR']['var']);
	...
	if (!$in_filter) $_SESSION['mes']['m'] = array("time"=>"", "text"=>"The item (<b>".(string)$_SESSION['p_code']."</b>) does not meet the selection criteria. The selection was canceled.", "status"=>"warning");
	if (!$no_sort) $_SESSION['mes']['m'] = array("time"=>"", "text"=>"The list on the screen has changes due to editing. Sorting by name is set.", "status"=>"warning");
}
function NoSort($p_var)
{
	foreach ($p_var as $v) if ($v['sort']['sort_order'] > 0) return false;
	return true;
}
function ItemFormExit($dbh, &$_SESSION, $exit_mode = "")
{
	...
	{
		if ($_SESSION['p_code'] == 0) $_SESSION['p_code'] = $_SESSION['old_p_code'];
		$_SESSION['block']['item_exit'] = false;
		$_SESSION['mes']['m'] = array();
		return true;
	}
}
function PubMarkDelete($dbh, &$_SESSION)
{
	...
	if ($_SESSION['item_view_mode'] != "all") $_SESSION['item_arr'] = GetMTPortion($dbh, $_SESSION['db_info']['t_main'], $_SESSION['p_start'], $_SESSION['portion'], $_SESSION['PR'], $_SESSION['item_view_mode']);
}
function SetViewMode($dbh, &$_SESSION, $post_view_mode)
{
	if ($_SESSION['item_view_mode'] != $post_view_mode)
	{
		...
		if ($_SESSION['p_count']['total'] > 0 && $_SESSION['p_count']['active'] > 0)
		{
			...
			if ($old_mode == "all" && $_SESSION['item_view_mode'] == "active") $_SESSION['p_code'] = FindNewPubID($_SESSION['item_arr'], false);
			elseif ($old_mode == "all" && $_SESSION['item_view_mode'] == "inactive") $_SESSION['p_code'] = FindNewPubID($_SESSION['item_arr'], true);
			elseif ($old_mode == "active" && $_SESSION['item_view_mode'] == "inactive") $_SESSION['p_code'] = FindNewPubID($_SESSION['item_arr'], true);
			elseif ($old_mode == "inactive" && $_SESSION['item_view_mode'] == "active") $_SESSION['p_code'] = FindNewPubID($_SESSION['item_arr'], false);
		}
	}
}
function FindNewPubID($item_arr, $del_mark_flag)
{
	foreach ($item_arr as $k => $v) if ($v['del_mark'][0] == $del_mark_flag) return (integer)$k;
	if (count($item_arr) == 0) return 0;
	return (integer)GetFirstKey($item_arr);
}
?>
