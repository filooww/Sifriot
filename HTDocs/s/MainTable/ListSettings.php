...
function SetModeSorting($key_sort, &$p_var)
{
	if ($p_var[$key_sort]['sort']['sort_order'] < 0)
	{
		...
		foreach (array_keys($p_var) as $k) if ($p_var[$k]['sort']['sort_order'] > $n) $p_var[$k]['sort']['sort_order']--;
	}
	...
	elseif (isset($_POST["sort-".$key_sort])) $p_var[$key_sort]['sort']['sm'] = (($_POST["sort-".$key_sort] == "sort_asc-".$key_sort) ? 1 : -1);
}
function SetMainFilter($k, &$p_var)
{
	if (isset($_POST["filter_text-".$k])) $p_var[$k]['filter']['text'] = $_POST["filter_text-".$k];
	if (isset($_POST["filter_max-".$k]) && $p_var[$k]['filter']['iv']) $p_var[$k]['filter']['to'] = $_POST["filter_max-".$k];
	if (isset($_POST["filter_radio-".$k]) && $p_var[$k]['filter']['md'] > -1)
	{
		$v_radio = str_replace("filter_radio-".$k."_", "", $_POST["filter_radio-".$k]);
		if (is_numeric($v_radio)) $p_var[$k]['filter']['md'] = (integer)$v_radio;
		else $p_var[$k]['filter']['md'] = 1;
	}
}
function MaxSortOrderNumber($p_var)
{
	$n_max = 0;
	foreach ($p_var as $v) if ($v['sort']['sort_order'] > $n_max) $n_max = $v['sort']['sort_order'];
	return $n_max + 1;
}
function OrderSortingButton($key_sort, &$p_var)
{
	...
	{
		$p_var[$key_sort]['sort']['sort_order'] = MaxSortOrderNumber($p_var);
		if ($p_var[$key_sort]['sort']['sm'] == 0) $p_var[$key_sort]['sort']['sm'] = 1;
	}
}
...
{
	if ($v['type'] == "integer") $compare = array("test"=>(integer)$test_value, "filter"=>(integer)$filter_value, "filter_to"=>(integer)$filter_to_value);
	elseif ($v['type'] == "date") $compare = array("test"=>StringToDateFormat($test_value, $Mes), "filter"=>StringToDateFormat($filter_value, $Mes), "filter_to"=>StringToDateFormat($filter_to_value, $Mes));
	else $compare = array("test"=>$test_value, "filter"=>$filter_value, "filter_to"=>$filter_to_value);
	return $compare;
}
function CaseCompare($filter_value, $filter_to_value)
{
	if ($filter_value == "" && $filter_to_value != "") return 1;
	elseif ($filter_value != "" && $filter_to_value == "") return 2;
	elseif ($filter_value != "" && $filter_to_value != "") return 3;
	else return 0;
}
	...
	foreach ($main_params['const'] as $k => $v)
	{
		if (TestedValue($k, $v, $PR['var']))
		{
			...
			if ($v['interval'] && $main_params['var'][$k]['filter']['iv'])
			{
				...
				{
					case 1:	if ($compare['test'] > $compare['filter_to']) return false; break;
					case 2:	if ($compare['test'] < $compare['filter']) return false; break;
					case 3: if ($compare['test'] < $compare['filter'] || $compare['test'] > $compare['filter_to']) return false; break;
				}
			}
			elseif ($compare['test'] != $compare['filter']) return false;
		}
	}
	foreach ($date_mes as $v) $Mes[] = array("time"=>"", "text"=>$v, "status"=>"warning");
	return true;
}
function ApplySettings($dbh, $main_table, &$p_code, &$item_arr, &$start_pos, &$p_count_filter, &$Mes, $user_id = "")
{
	...
	if (isset($item_arr[(string)$p_code]))
	{
		...
		else
		{
			$start_pos = 0;
			$item_arr = GetMTPortion($dbh, $main_table, $start_pos, $_SESSION['portion'], $_SESSION['main_params'], ($user_id == "") ? "all" : "active");
			if (count($item_arr) > 0) $p_code = (integer)GetFirstKey($item_arr);
		}
	}
	else
	{
...
function InputCase($k, $v, $v_con_k, $block, $sel_align)
{
	...
	if ($v_con_k['type'] == "date") $str .= "<button name='to_calendar-".$k."' type='submit' title='".Title(57)."' value='*'".$block.">".ImgV("Calendar", 13, 10)."</button>";
	if ($v_con_k['s_mode'])
	{
		if ($v_con_k['table'] == "") $str .= "<button name='dummy-".$k."' type='submit' value='*'".$block." disabled>...</button>";
		else $str .= "<button name='to_catalog-".$k."' type='submit' title='".Title(58)."' value='*'".$block.">...</button>";
	}
	if ($v_con_k['interval'])
	{
		...
		if ($v['filter']['iv']) 
		{
			...
			if ($v_con_k['type'] == "date") $str .= "<button name='to_calendar_to-".$k."' type='submit' title='".Title(57)."' value='*'".$block.">".ImgV("Calendar", 13, 10)."</button>";
		}
	}
	return $str;
}
...
function FilterPart($block, $k, $v, $v_con_k, $mode_texts, $sel_align)
{
	echo "<table>";
		echo "<tr valign='middle'>";
			...
			if ($v_con_k['s_mode']) for ($i = 0; $i < count($mode_texts); $i++) echo "<td><input type='radio' ".RadioNameValue($k, $i)."'".(($v['filter']['md'] == $i) ? " checked" : "").$block.">".$mode_texts[$i]."</td>"; //RadioTag
		echo "</tr>";
	echo "</table>";
}
					...
					foreach ($_SESSION['main_params']['var'][1] as $k => $v)
					{
						if ($_SESSION['main_params']['const'][1][$k]['screen_order'] > 0 && ($user_id == "" || $user_id != "" && in_array("4", explode(",", $_SESSION['main_params']['const'][1][$k]['using'])))) // view
						{
							echo "<tr>";
								...
								echo "<td class='".$row_cl."'>"; SortPart($block, $k, $v['sort'], $_SESSION['sort_mode']); echo "</td>";
								echo "<td class='".$row_cl."'>"; FilterPart($block, $k, $v, $_SESSION['PR']['const'][$k], $_SESSION['compare_mode'], $_SESSION['field_align']); echo "</td>";
								$row_cl = ($row_cl == "pad_colors1") ? "pad_colors2" : "pad_colors1";
							echo "</tr>";
							if ($_SESSION['main_params']['const'][1][$k]['type'] == "date" && $_SESSION['calend_date'][$k]['id_filter'] != "") CalendPart($_SESSION['conf']['start_year'], $_SESSION['cury'], $k, $_SESSION['calend_date'][$k], $_SESSION['calend_mes']);
						}
					}
				echo "</table>";
			echo "</td>";
			echo "<td>";
			if ($_SESSION['cat_flag']) ViewCatalogForSelect($_SESSION['cat_vis'], $_SESSION['conf'], $_SESSION['block'], $_SESSION['cat_tree'], $_SESSION['radio_modes'], $_SESSION['compare_mode'], $_SESSION['cur_value'], "cur_text");
			echo "</td>";
		echo "</tr>";
	echo "</table>";
}
...
function GetInitItemCode($dbh, $main_table, $p_con, $inact_mode = false)
{
	...
	{
		if ($inact_mode) $res = mysqli_query($dbh, "SELECT ".$_SESSION['spec_fld'][1]['key']." FROM ".$main_table." WHERE ".$_SESSION['spec_fld'][1]['del_mark']." = 1 LIMIT 1");
...
function SortFind()
{
	...
	$_SESSION['cur_cat'] = "";
	if (!$_SESSION['set_pad']) RememberSettings($s['PR']['var']);
}
