...
$l_selected = array("value"=>"", "code"=>0);
$smes = "";
$aA = ParseItemFormButtons($dbh);
if (count($aA) == 0) $aA = ParseCatalogButtons($dbh);
...
if (count($aA) > 0)
{
	...
	if ($_SESSION['cur_cat'] != "" && ToTest($aA)) TestCatalogs($dbh);
	ItemSavePost($_SESSION['PR']['const'], $_SESSION['item_row']['e'], $_SESSION['p_files']['e']);
	$sw_break = true;
	switch ($aA['act'])
	{
		case "db_save"			: $sw_break = false; if (SaveItem($dbh, $_SESSION)) $smes = "Item saved"; break;
		case "line_select"		: $sw_break = false; LineUpdateSelect($dbh, $aA['code'], $_SESSION['cur_cat'], $l_selected, $_SESSION['item_row']['e'], $_SESSION['mes']['m']); break;
		case "delete_file"		: $sw_break = false; unset($_SESSION['p_files']['e'][$aA['file_number']]); break;
		case "upload_file"		: $sw_break = false; if (FileUploading($_SESSION, $aA['file_number'], $_SESSION['mes']['m'])) {$_SESSION['file_upload_return'] = "FileUpload.php"; header("Location: FileSelection.php");} break;
		case "db_form_exit"		: $sw_break = false; $_SESSION['item_exit_question'] = (ItemFormCompare($_SESSION['item_row'], $_SESSION['p_files'])) ? 1 : 2; break;
		case "item_exit_save"	: $sw_break = false; if (ItemFormExit($dbh, $_SESSION, "save")) header("Location: List.php"); break;
		case "item_exit_no_save": $sw_break = false; if (ItemFormExit($dbh, $_SESSION)) header("Location: List.php"); break;
		case "continue_edit"	: $sw_break = false; $_SESSION['block']['item_exit'] = false; break;
		default					: $sw_break = false;
	}
	if (!$sw_break) CatalogActions($dbh, $n, $aA, CatalogSelectedValueUpdate($aA), $_SESSION['cur_value'], $_SESSION['mes']);
}
?>
<!-- <form action="ItemForm.php" method="post"> -->
<form method="post">
	<?php
	if ($_SESSION['item_exit_question'] == 1) {QuestionForm("modal_form", "You haven't made any changes to the data", array("item_exit_no_save", "continue_edit"), array("Exit", "Continue editing")); $_SESSION['block']['item_exit'] = true;}
	elseif ($_SESSION['item_exit_question'] == 2) {QuestionForm("modal_form", "Save changes?", array("item_exit_save", "item_exit_no_save", "continue_edit"), array("Yes", "No", "Continue editing")); $_SESSION['block']['item_exit'] = true;}
	echo "<table>";
		echo "<tr>";
	...
	if ($_SESSION['cat_view'])
	{
		echo "<tr valign='top'>";
			echo "<td>";
				echo "<table"; ItemHTMLForm($_SESSION['t_main'], $_SESSION['PR']['const'], $_SESSION['item_row']['e'], $_SESSION['block'], $_SESSION['conf']['start_year'], $_SESSION['cury']); echo "</table>";
				if (isset($aA['act'])) ViewMessages(false, "", $_SESSION['number_warn'], $_SESSION['Catalog'], $_SESSION['del_row'], $_SESSION['block'], ToSave($aA['act']));
			echo "</td>";
			echo "<td>"; ViewCoupleCatalog(); echo "</td>";
		echo "</tr>";
	}
	else
	{
		ItemHTMLForm($_SESSION['t_main'], $_SESSION['PR']['const'], $_SESSION['item_row']['e'], $_SESSION['block'], $_SESSION['conf']['start_year'], $_SESSION['cury']);
		echo "<tr><td>"; require_once("ItemFiles.php"); echo "</td></tr>";
		echo "<tr><td>"; if (isset($aA['act'])) ViewMessages(false, "", $_SESSION['number_warn'], array(), array(), $_SESSION['block'], ToSave($aA['act'])); echo "</td></tr>";
	}
	echo "</table>";
	?>
</form>
