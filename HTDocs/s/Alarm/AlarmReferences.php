...
function SetInvalidRows($res, $table, $types, &$pre_ref)
{
    ...
    while ($row = mysqli_fetch_row($res))
    {
        ...
        {
            if (!isset($pre_ref[$table]['p'][$type[0]]['ts'])) $pre_ref[$table]['p'][$type[0]]['ts'] = array($type[3], $type[4]);
...
{
    if (!isset($pre_arr['v'][$pre_key])) return true;
    if (!in_array($pre_value, $pre_arr['v'][$pre_key])) return true;
    return false;
}
function IncludeInReferenceList($table, $t_type, $row)
{
    ...
    {
        if ($t_type == "cd") return !isset($_SESSION['coding_list'][$row[0]]);
        else return $row[1] == 1 || !isset($_SESSION['user_langs'][$row[1]]);
    }
    if ($table == "user_ident")
    {
        if ($t_type == "li") return !isset($_SESSION['user_langs'][$row[1]]);
        else return DBFilter($row[2], $row[3]);
    }
    if ($table == "visits")
    {
        if ($t_type == "db") return DBFilter($row[0], $row[2]);
        else return !is_numeric($row[2]);
    }
    return true;
}
function DBFilter($db_id, $priority)
{
    if (is_null($db_id)) return false;
    if (!array_key_exists($db_id, $_SESSION['arr_db'])) return true;
    if (!is_numeric($priority)) return false;
    if ((integer)$priority > -1 && (integer)$priority < 11) return ($db_id == 0);
    return false;
}
function InvalidReferenceSum($pre_ref_table)
{
    $s = 0;
    foreach ($pre_ref_table['p'] as $k_type => $v_type) foreach ($v_type['v'] as $k => $v) $s += count($v);
    return $s;
}
function InvalidReferenceTable($dbh)
{
	...
	SetReferenceTable($dbh, "interface_texts", 128, QueryReferencesTitles(), array(array("li", 0, 1, 542, 543)), $pre_ref);
	SetReferenceTable($dbh, "user_ident", 12, QueryReferencesUsers(), array(array("li", 1, 0, 542, 544), array("db", 2, 0, 545, 544)), $pre_ref);
	SetReferenceTable($dbh, "visits", 167, QueryReferencesVisits(), array(array("db", 0, 1, 545, 544), array("ui", 1, 0, 544, 545)), $pre_ref);
	ksort($pre_ref);
	return $pre_ref;
}
function IsInvalidTableReferences($k_table)
{
    if (isset($_SESSION['pre_ref'][$k_table]['p'])) foreach ($_SESSION['pre_ref'][$k_table]['p'] as $v_type) if (isset($v_type['v']) && count($v_type['v']) > 0) return true;
    return false;
}
function ChangeInvalidRefTable($dbh, $table, $ref_type, $ref_id, $code_id)
{
    ...
    if ($i !== false)
    {
	    if (count($_SESSION['pre_ref'][$table]['p'][$ref_type]['v'][$ref_id]) == 1) unset($_SESSION['pre_ref'][$table]['p'][$ref_type]['v'][$ref_id]);
	    else unset($_SESSION['pre_ref'][$table]['p'][$ref_type]['v'][$ref_id][$i]);
    }
}
function ChangeByInsertInvalidRefTable($table, $ref_type, $ref_id)
{
	...
	{
	    if (count($_SESSION['pre_ref'][$table]['p'][$ref_type]['v']) == 0) unset($_SESSION['pre_ref'][$table]['p'][$ref_type]);
	    if (count($_SESSION['pre_ref'][$table]['p']) == 0) unset($_SESSION['pre_ref'][$table]);
	}
}
function IsInvalidReferences()
{
    if (!isset($_SESSION['pre_ref']) || count($_SESSION['pre_ref']) == 0) return false;
    foreach (array_keys($_SESSION['pre_ref']) as $k_table) if (IsInvalidTableReferences($k_table)) return true;
    return false;
}
function InvRefTable($table, $emph, $cor_ref_arr)
{
        ...
        if (isset($v_type['v']) && count($v_type['v']) > 0)
        {
		    ...
		    foreach ($v_type['v'] as $k => $v)
		    {
			    ...
			    echo "<tr valign='top'>";
                    ...
                    if (count($cor_ref_arr[$k_type]) == 0) echo "<td></td>";
                    else
                    {
                        echo "<td>";
	...
	if ($_SESSION['pre_ref'][$table]['over']) echo "<tr valign='top'><td><font color='#0000FF'><i>".Title(359)." ... </i></font></td><td></td><td></td><td></td><td></td></tr>";
	echo "</table>";
	if (isset($_SESSION['replace_mes']) && count($_SESSION['replace_mes']) > 0) PrintReplaceMes();
...
{
    if ($old_ref_id == $new_ref_id) $_SESSION['replace_mes'] = array("#FF0000", 244);
    else
    {
        mysqli_query($dbh, "UPDATE ".$table." SET ".$ref_field." = ".$new_ref_id." WHERE ".$ref_field." = ".$old_ref_id);
        if (mysqli_errno($dbh) == 1062) return false;
        unset($_SESSION['pre_ref'][$table]['p'][$k_type]['v'][(integer)$old_ref_id]);
        $_SESSION['replace_mes'] = array("#0000FF", 602, " ", $_SESSION['pre_ref'][$table]['p'][$k_type]['ts'][0], " <b>".$old_ref_id."</b> ", 469, " <b>".$new_ref_id."</b>");
    }
    return true;
}
function GetInvRef($table, $cor_ref_arr)
{
    ...
    if ($_SESSION['inv_ref'] && isset($_SESSION['pre_ref'][$table]['p'])) SetReplacedID($table, $cor_ref_arr);
    else
    {
        $_SESSION['replacing_id'] = array();
...
function AfterReplaceSelect($select_arr, &$sw_break)
{
    ...
    if ($r_id === false) return false;
    $_SESSION['repl_id'][$k[1]][$k[2]] = (string)$r_id;
    return true;
}
function PermitInvRef($table, $type_arr)
{
    if (!isset($_SESSION['pre_ref'][$table]['p'])) return false;
    foreach ($_SESSION['pre_ref'][$table]['p'] as $k_type => $v_type) if (isset($v_type['v']) && count($v_type['v']) > 0) return true;
    foreach ($type_arr as $type) if(isset($_SESSION['pre_ref'][$table]['p'][$type]['v']) && count($_SESSION['pre_ref'][$table]['p'][$type]['v']) > 0) return true;
    return false;
}
function SetInitReplaceIDs($table, $id_types)
{
    $_SESSION['repl_id'] = array();
    foreach ($id_types as $type_k => $arr_name)
    {
	    if (isset($_SESSION['pre_ref'][$table]['p'][$type_k]['v']))
        {
            ...
            foreach (array_keys($_SESSION['pre_ref'][$table]['p'][$type_k]['v']) as $k) $_SESSION['repl_id'][$type_k][$k] = (string)$first_k;
        }
    }
}
...
{
    $str_print = "";
    if ($_SESSION['replace_mes'][0] != "") $str_print .= "<font color='".$_SESSION['replace_mes'][0]."'>";
    for ($i = 1; $i < count($_SESSION['replace_mes']); $i++)
    {
        if ($i % 2 == 0) $str_print .= $_SESSION['replace_mes'][$i];
        else
        {
            if ($_SESSION['replace_mes'][$i] != 0)
            {
                if ($_SESSION['replace_mes'][$i] < 0) $str_print .= FTM(Title(-$_SESSION['replace_mes'][$i]));
                else $str_print .= Title($_SESSION['replace_mes'][$i]);
            }
        }
    }
    if ($_SESSION['replace_mes'][0] != "") $str_print .= "</font>";
    if ($str_print != "")
    {
