...
{
	if (strpos($str, chr(39)) == false) $rs = $str;
	else $rs = str_replace(chr(39), "&#39", $str);
	if ($to_space) $rs = "";
	return $rs;
}
function StringTest($illegal_str, $str, $cl)
{
	$rr = array("error"=>false, "symb"=>"");
	if ($str == "" || $illegal_str == "") return $rr;
	...
	if (count($arr_symb) > 0) $rr['symb'] = implode($_SESSION['char_group'], $arr_symb);
	return $rr;
}
function SetTestMessages($rr, $source_string, $row_id = "")
{
	$Mes = "";
	if ($rr['symb'] != "") $Mes = Title(443)." <b>".$source_string."</b> ".(($row_id == "") ? "" : "(<b>".$row_id."</b>)")." ".Title(444).": ".$rr['symb'];
	return $Mes;
}
function GetCorrectEditedValue($ill_arr, $str)
{
	$rr = $str;
	foreach ($ill_arr as $v) if (count($v) > 1) $rr = str_replace($v[0], $v[1], $rr);
	return $rr;
}
function CountOfNoEmptyItems($arr)
{
	$i = 0;
	foreach ($arr as $v) if ($v != "") $i++;
	return $i;
}
function TransSize($str)
{
	switch (substr($str, -1))
	{
		case "K" : return (is_numeric(substr($str, 0, strlen($str) -1))) ? intval(substr($str, 0, strlen($str) -1)) * 1024 : 0; break;
		case "M" : return (is_numeric(substr($str, 0, strlen($str) -1))) ? intval(substr($str, 0, strlen($str) -1)) * 1024 * 1024 : 0; break;
		case "G" : return (is_numeric(substr($str, 0, strlen($str) -1))) ? intval(substr($str, 0, strlen($str) -1)) * 1024 * 1024 * 1024 : 0; break;
...
{
	if (substr($res_adr, 0, 4) == "http") return $res_adr;
	else
	{
...
function MaxWidth($using_code)
{
	...
	foreach ($_SESSION['main_params']['const'][1] as $k => $v)
	{
		if ($v['screen_order'] > 0 && in_array($using_code, explode(",", $v['using'])))
		{
			...
			if ($n_str > $max_width) $max_width = $n_str;
		}
	}
	return $max_width;
}
	...
	if ($ses_destr)
	{
	    if (isset($_SESSION['priority']))
	    {
            $dbh_sys = GetOnlyDB("db_manager");
   	        if ($dbh_sys) VisitParameters($dbh_sys, $id_db, -1, $_SESSION['priority']);
        }
	    $tt = Title(16);
	    if (substr($tt, 0, 13) == "Missing text ") $tt = "Session completed";
...
function TestSysString($str)
{
	...
	for ($i = 1; $i < count($arr_str); $i++) if (ord($arr_str[$i]) < 48 || ord($arr_str[$i]) > 57 && ord($arr_str[$i]) < 95 || ord($arr_str[$i]) == 96 || ord($arr_str[$i]) > 122) return false;
	return true;
}
function GetIDArraySelect($arr_select, $value, $n = -1)
{
	...
	foreach ($arr_select as $k => $v) if ($v[$n] == $value) return $k;
	return "";
}
...
function ChangeTimerInterval($interval_file, $interval_value)
{
	...
	$pos = strpos($str, ", ");
	$str = substr($str, 0, $pos + 2).(string)((integer)$interval_value * 1000).");";
...
function SaveUserScreenPortion($dbh_sys, $post_portion)
{
	...
	if ($res)
	{
		...
		{
			if ($row[0] != (integer)$post_portion) mysqli_query($dbh_sys, "UPDATE user_ident SET user_list_portion = ".$post_portion." WHERE id_user = ".$_SESSION['user_id']);
		}
...
function GetCorrectDBValue($ini_v, $arr_conv)
{
	...
	if (count($arr_conv) > 0)
	{
		if (count($arr_conv) > 1) $s = str_replace($arr_conv[0], $arr_conv[1], $s);
		elseif (is_array($arr_conv[0]) && count($arr_conv[0]) > 1) foreach ($arr_conv as $v) $s = str_replace($v[0], $v[1], $s);
	}
	return $s;
}
...
function IsReferenceErrors()
{
    foreach ($_SESSION['pre_ref'] as $table => $t_info)
    {
        if (isset($t_info['p']))
        {
            ...
            {
                if (isset($type_params['v'])) return true;
            }
        }
    }
    return false;
...
{
    $arr_ord = array(ord(substr($k, 0)), ord(substr($k, 1)), ord(substr($k, 2)));
    return 1000000 * $arr_ord[0] + 1000 * $arr_ord[1] + $arr_ord[2];
}
function EmphSymb($v, $emph)
{
    $vv = array();
    foreach ($v as $z) $vv[] = "<font class='".$emph."'><b>".$z."</b></font>";
    return $vv;
}
...
function ChangeMesAfterLangChoice($mes_arr)
{
    ...
    for ($i = 0; $i < count($mes_arr); $i++)
    {
        ...
        {
            if ($mes_arr[$i] > 0) $mes .= Title($mes_arr[$i]);
            else $mes .= FTM(Title($mes_arr[$i]));
        }
        else $mes .= $mes_arr[$i];
    }
    return $mes;
